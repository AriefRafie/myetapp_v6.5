 ...
 <script>
 var carian = $jquery('#Form_id_negeri').val(); 
 //alert("carian : "+carian+" total rekod : "+'$totalRecords'+" nama list : "+'$namaList');
// highlight('$listBahagianHQ.size()',carian,'listBahagianHQ'); 
 </script>
 ...
 <input type="hidden" name="scrolPosition$command" id="scrolPosition$command" >
           ...
           <table width="100%" border="0" cellpadding="0" cellspacing="0">
                <tr>
                    <td  align="right"><strong>Jumlah Daerah: $totalRecords</strong></td>
                   <td  align="right">
                    	#if($totalRecords != 0)
                        
                        <select class="smallselect" id="itemsPerPage$command"  name="itemsPerPage$command" 
                        onChange="doDivAjaxCall$formname('$div','$command','action=doChangeItemPerPage&showSenaraiPejabat='+$jquery('#showSenaraiPejabat').val()+'&scrolPosition='+getPageLocation('$command'));"
                        >
	  ...
	  <table width="100%" cellpadding="0" cellspacing="0" border="0">
		...
		<td width="70%" align="left" valign="middle">
		   ...
		   onClick="doDivAjaxCall$formname('$div','$command','action=getPage&value=1&showSenaraiPejabat='+$jquery('#showSenaraiPejabat').val()+'&scrolPosition='+getPageLocation('$command'));"
		   src="../img/paging/page-first.gif">
		   #if ($page > 10)
		   	<img title="Previous 10" style="vertical-align=bottom;cursor:hand;cursor:pointer;" 
		   	onClick="doDivAjaxCall$formname('$div','$command','action=getPrevious&page=$previous10&showSenaraiPejabat='+$jquery('#showSenaraiPejabat').val()+'&scrolPosition='+getPageLocation('$command'));"
		...
		&nbsp;
			...
			#foreach ( $i in [$page_mula..$totalPages] )
					...
					#else
					<a 
					href="javascript:doDivAjaxCall$formname('$div','$command','action=getPage&value=$i&showSenaraiPejabat='+$jquery('#showSenaraiPejabat').val()+'&scrolPosition='+getPageLocation('$command'));"
		    		>[$i]</a>
					#end
				#end
			#end
		...
		&nbsp;
		    ...
		    onClick="doDivAjaxCall$formname('$div','$command','action=getNext&page=$next10&showSenaraiPejabat='+$jquery('#showSenaraiPejabat').val()+'&scrolPosition='+getPageLocation('$command'));"
		    
		    src="../img/paging/page-next.gif">
		    <img title="Last page:$totalPages" style="vertical-align=bottom;cursor:hand;cursor:pointer;" 
		    onClick="doDivAjaxCall$formname('$div','$command','action=getPage&value=$totalPages&showSenaraiPejabat='+$jquery('#showSenaraiPejabat').val()+'&scrolPosition='+getPageLocation('$command'));"
