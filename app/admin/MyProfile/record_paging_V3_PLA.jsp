 ...
 <script>
 var carian = $jquery('#carianTerperinciPLA_$internalType$USER_ID').val(); 
           ...
           <table width="100%" border="0" cellpadding="0" cellspacing="0">
                <tr>
                    ...
                    <td  align="right">
                        ...
                        onChange="doDivAjaxCall$formname('$div','$command','action=doChangeItemPerPage&carianTerperinciPLA='+$jquery('#carianTerperinciPLA_$internalType$USER_ID').val()+'&scrolPosition='+getPageLocation('$command')+'&TARIKH_MULA_PLA='+$jquery('#TARIKH_MULA_PLA_$internalType$USER_ID').val()+'&TARIKH_AKHIR_PLA='+$jquery('#TARIKH_AKHIR_PLA_$internalType$USER_ID').val()+'&USER_ID=$USER_ID&internalType=$internalType');"
                        >
	  ...
	  <table width="100%" cellpadding="0" cellspacing="0" border="0">
		...
		&nbsp;
			...
			#foreach ( $i in [$page_mula..$totalPages] )
					...
					#else
					<a 
					href="javascript:doDivAjaxCall$formname('$div','$command','action=getPage&value=$i&carianTerperinciPLA='+$jquery('#carianTerperinciPLA_$internalType$USER_ID').val()+'&scrolPosition='+getPageLocation('$command')+'&TARIKH_MULA_PLA='+$jquery('#TARIKH_MULA_PLA_$internalType$USER_ID').val()+'&TARIKH_AKHIR_PLA='+$jquery('#TARIKH_AKHIR_PLA_$internalType$USER_ID').val()+'&USER_ID=$USER_ID&internalType=$internalType');"
		    		>[$i]</a>
					#end
				#end
			#end
