 ...
 highlight('$totalRecords',carian,'$namaList','$MODULE_GROUP'); 
 </script>
 ...
 <input type="hidden" name="scrolPosition$command" id="scrolPosition$command" >
           ...
           <table width="100%" border="0" cellpadding="0" cellspacing="0">
                <tr>
                    ...
                    <td  align="right">
                        ...
                        onChange="doDivAjaxCall$formname('$div','$command','action=doChangeItemPerPage&BIL=$BIL&MODULE_GROUP=$MODULE_GROUP&carianTerperinci='+$jquery('#carianTerperinci').val()+'&scrolPosition='+getPageLocation('$command'));"
                        >
	  ...
	  <table width="100%" cellpadding="0" cellspacing="0" border="0">
		...
		<td width="70%" align="left" valign="middle">
		   ...
		   onClick="doDivAjaxCall$formname('$div','$command','action=getPage&value=1&BIL=$BIL&MODULE_GROUP=$MODULE_GROUP&carianTerperinci='+$jquery('#carianTerperinci').val()+'&scrolPosition='+getPageLocation('$command'));"
		   src="../img/paging/page-first.gif">
		   #if ($page > 10)
		   	<img title="Previous 10" style="vertical-align=bottom;cursor:hand;cursor:pointer;" 
		   	onClick="doDivAjaxCall$formname('$div','$command','action=getPrevious&page=$previous10&BIL=$BIL&MODULE_GROUP=$MODULE_GROUP&carianTerperinci='+$jquery('#carianTerperinci').val()+'&scrolPosition='+getPageLocation('$command'));"
		...
		&nbsp;
			...
			#foreach ( $i in [$page_mula..$totalPages] )
					...
					#else
					<a 
					href="javascript:doDivAjaxCall$formname('$div','$command','action=getPage&value=$i&carianTerperinci='+$jquery('#carianTerperinci').val()+'&scrolPosition='+getPageLocation('$command')+'&MODULE_GROUP=$MODULE_GROUP&BIL=$BIL');"
		    		>[$i]</a>
					#end
				#end
			#end
		...
		&nbsp;
		    ...
		    onClick="doDivAjaxCall$formname('$div','$command','action=getNext&page=$next10&BIL=$BIL&MODULE_GROUP=$MODULE_GROUP&carianTerperinci='+$jquery('#carianTerperinci').val()+'&scrolPosition='+getPageLocation('$command'));"
		    
		    src="../img/paging/page-next.gif">
		    <img title="Last page:$totalPages" style="vertical-align=bottom;cursor:hand;cursor:pointer;" 
		    onClick="doDivAjaxCall$formname('$div','$command','action=getPage&value=$totalPages&BIL=$BIL&MODULE_GROUP=$MODULE_GROUP&carianTerperinci='+$jquery('#carianTerperinci').val()+'&scrolPosition='+getPageLocation('$command'));"
