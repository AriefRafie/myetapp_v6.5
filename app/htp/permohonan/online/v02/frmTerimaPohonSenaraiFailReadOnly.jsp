...
<table width="100%" border="0">
  ...
  <tr class="$row">
    <td align="center">
		<a href="javascript:viewMaklumatPermohonan('$senarai.id')" >
    	$cnt.
 		</a>
    </td>
    <td >
...
function viewMaklumatPermohonan(id){
	...
	function maklumatPermohonan(id){
		var mode = 'maklumatpermohonanonline';
...
function Hapus(){
	...
	function Simpan(i){	
		...
		if ( document.${formName}.socDaerah.value == "" ) { 
			alert('Sila pilih daerag terlebih dahulu.');
...
function kembaliBorangK(){
	...
	function failKemaskini(){
		doAjaxCall${formName}("kemaskinipermohonanonline","mode=MakAsasTanah&pagemode=0&hittButton=kemaskini");
	
	}
	
	function failKemaskiniSimpan(){
		doAjaxCall${formName}("kemaskinipermohonanonline","mode=MakAsasTanah&pagemode=0&hittButton=kemaskinisimpan");
