...
<table border="0" width="100%">
    <tbody>
       <tr >
        <td align="right" width="29%"><div align="right">Negeri</div></td>
     ...
     <tr >
        ...
        <td ><div align="center">:</div></td>
        <td ><input name="txtNoFail" id="txtNoFail" type="text" value="$!txtNoFail" size="45" maxlength="50" style="text-transform:uppercase;" > </td>
      </tr>   
      <tr >
        <td align="right" ><div align="right">Nama Pemilik</div></td>
        <td ><div align="center">:</div></td>
        <td ><input name="txtNamaPemilik" id="txtNamaPemilik" type="text" value="$!txtNamaPemilik" size="45" maxlength="50" style="text-transform:uppercase;" > </td>
      </tr> 
      <!--<tr>
...
<table border="0" width="100%">
	...
	<!-- <tr class="table_header">
	  <td width="3%"><strong>BIL.</strong></td>
   	  <td width="20%"><strong>NO. FAIL</strongb></td>
  	  <td width="47%"><strong>TAJUK FAIL</strong></td>
   	  <td width="15%"><strong>NO. FAIL PTG/JTS/JTU</strong></td>
  	  <td width="15%"><strong>NO. FAIL PTD</strong></td>
	</tr> -->
	<tr class="table_header">
	  <td width="3%"><strong>Bil.</strong></td>
   	  <td width="20%"><strong>No. Fail</strong></td>
      <td width="20%"><strong>No. Fail KPKT/PP</strong></td>
  	  <td width="37%"><strong>Nama Pemilik</strong></td>
   	  <td width="20%"><strong>Status</strong></td>
	</tr>
	
#set ($count = 0)
#foreach ($permohonan in $permohonans)
	#set ($count = $count+1)
  	#set( $i = $velocityCount )
    #if ( ($i % 2) != 1 )
       #set( $row = "row2" )
    ...
    #end
   <tr class="$row">
	   	<td height="20">$!permohonan.bil.</td> 
	    <td>$permohonan.noFail</td>
	 	<td>$permohonan.noFailLain</td>
	    <td><a href="javascript:detail('$permohonan.idPermohonan');" class="style1">$permohonan.nama</a></td>
...
<input type="hidden" name="semakantanah"/>
...
*/
	...
	if(document.${formName}.txdTarikhTerimaSelesai.value == ""){
		alert('Sila pilih " Tarikh Penghantaran Selesai " terlebih dahulu.');
  		document.${formName}.txdTarikhTerimaSelesai.focus(); 
		return; 
	}
	...
	function fGPHA_Kembali() {
		doAjaxCall${formName}("SenaraiPemilik");
	}
	
	//penambahan yati
	function detail(id_){
		document.${formName}.mode = "";
		doAjaxCall${formName}("paparDetailHakmilik2","idPermohonan="+id_);
	}
	...
	*/
	...
	function hakmilik_detail2P(idp){
		document.${formName}.mode = "";
		//doAjaxCall${formName}("paparDetailHakmilik2","idPermohonan="+id);
		document.${formName}.idPermohonan.value = idp;			
		doAjaxCall${formName}("papardetailhakmilik2kembali");
	}
	...
	function rizab_detail2P(idp){
		document.${formName}.idPermohonan.value = idp;		
		doAjaxCall${formName}("barukembali");
	...
	function doChangeKementerianRizabEdit() {simpanRizab
...
// Pilihan skrin Pendaftaran Hakmilik/Rizab
	/** Dibuat pada 10/04/2013 
		fungsi pilihan kementerian semasa kemaskini
	*/
	//function pilihKementerian() {
	function pilihKementerian(modeJava) {
		//document.${formName}.mode.value="doChangeKementerianHak";
	  	//doAjaxCall${formName}("kemaskiniDetailHakmilik");
		//document.${formName}.mode.value = "kemaskini";
		//document.${formName}.idHakmilik.value = id_;
		document.${formName}.mode.value = modeJava;
		doAjaxCall${formName}("paparDetailHakmilik2");
...
function doChangeTaraf() {
	...
	//Kemaskini pada 28/05/2013 , set idHakmilik.value = id_
	//Kemaskini pada 19/07/2013 , nama fungsi Xupdate_detailRizab
	function Xupdate_detailRizab(id_){
		//doAjaxCall${formName}("updateDetailRizab","idHakmilik="+id);
		document.${formName}.idHakmilik.value = id_;
		doAjaxCall${formName}("updateDetailRizab");
		
	}

	//Kemaskini pada 19/07/2013 , nama fungsi Xupdate_detailRizab
	function update_detailRizab(id_){
		////VALIDATAION
	 ...
	 if(document.${formName}.socNegeri.value == 99999){ 
...
function simpan_detailHakmilik(id){
	...
	function semakHakmilik(){
		url = "../servlet/ekptg.view.htp.FrmHTPSemakan";
		actionName = "semakanhakmilikrizab";
		target = "div_isexist";
		doAjaxUpdater(document.${formName}, url, target, actionName);
	//	alert(document.${formName}.txtNoHakmilik.value);
	...
	function semakHakmilikStrata(){
		
		if(document.${formName}.txtNoBangunan.value != '' && document.${formName}.txtNoTingkat.value != '' 
			&& document.${formName}.txtNoPetak.value != ''){ 			
			semakHakmilik();
		}
...
function UpdateHakmilik(idPermohonan,idHakmilik){
	...
	// Skrin Senarai Hakmilik Link <No.Fail>
	function kemaskiniHakmilik(idPermohonan,idHakmilik){
		document.${formName}.idPermohonan.value = idPermohonan;
		document.${formName}.idHakmilik.value = idHakmilik;
		document.${formName}.mode.value = "view";
		document.${formName}.INS_UPD.value = "view";
		doAjaxCall${formName}("paparDetailHakmilik2");
...
function doChangeKodLuas(val) {
	...
	/** 
	* Dibuat pada 24/04/2013 
	* [PILIH LUAS], mode update/kemaskini
	*/
	function doChangeKodLuasUpdate(val) {
		//document.${formName}.mode.value = "doChangeKodLuas";
		document.${formName}.INS_UPD.value = "doChangeKodLuas";
		doAjaxCall${formName}("paparDetailHakmilik2");
...
function doChangeDaerahRizabPer() {
	...
	//Kemaskini pada 28/05/2013, [Simpan] baru bagi id_permohonan telah wujud	
	function simpanRizabPermohonan(id_){
		//VALIDATAION
		if(document.${formName}.txtIdKementerian.value == ""){ 
			alert('Sila pilih " Kementerian " terlebih dahulu.'); 
			document.${formName}.txtIdKementerian.focus();
	...
	function semakWarta(){
		...
		target = "div_isexist";
		doAjaxUpdater(document.${formName}, url, target, actionName);
	//	alert("1,"+document.${formName}.txtNoLot.value+",semakTanah="$!{session.getAttribute("semakTanah")});
	//	alert("2,"+$semakTanah1);
	//	if(document.${formName}.txtNoWarta.value != ""){
	//		return ;
	//	}
	...
	function hapusTanah(id_){
		if ( !window.confirm("Adakah Anda Pasti?")) return;
			document.${formName}.command.value = "";
		doAjaxCall${formName}("hapustanah","firstAction=deletehakmilikrizab&nextAction=kemaskiniDetailRizab&idHakmilik="+id_);	
	}
