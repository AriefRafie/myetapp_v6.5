...
<td>
	...
	<!--<legend>LAPORAN</legend>-->
		...
		<table border="0" align="center" width="100%" >
			...
			#set ( $nextno = $cnt + 1 )
			<tr>
				<td class="row2">$nextno</td>
				<td class="row2">
					<a href="javascript:cetak()" class="style2" >
						<font color="blue">
							LAPORAN STATISTIK PEMBAYARAN CUKAI TANAH PTP
 						</font>
 					</a>
				</td>
			</tr>
			#set ( $nextno = $nextno + 1 )
			<tr>
				<td class="row1">$nextno</td>
				<td class="row1">
					<a href="javascript:cetakSenaraiCukai()" class="style2" >
						<font color="blue">
							LAPORAN SENARAI CUKAI TANAH MILIK PESURUHJAYA TANAH PERSEKUTUAN
 						</font>
 					</a>
				</td>
			</tr>
...
<script>
	...
	// Digunakan untuk Laporan Permohonan Pemberimilikan/Perizapan
	...
	function cetak(){
		...
		var mula_tahun = document.${formName}.txdMula.value.substring(6,10);
  		...
  		var intMula = parseInt(mula_tahun);
  		var intAkhir = parseInt(akhir_tahun);
  		
		if(document.${formName}.txdMula.value == "" ){
  			alert("Sila pastikan \"Tarikh Dari\" dilengkapkan.");
			document.${formName}.txdMula.focus(); 
			return;
  		}
		else if(document.${formName}.txdAkhir.value == ""){
  			alert("Sila pastikan \"Tarikh Hingga\" dilengkapkan.");
			document.${formName}.txdAkhir.focus(); 
			return;
  		}
  		else if(intMula > intAkhir){
			alert("Sila pastikan \"Tarikh Hingga\" tidak kurang dari \"Tarikh Dari\".");
			document.${formName}.txdAkhir.focus(); 
			return;
		}else{
			var url = "../servlet/ekptg.report.htp.LaporanStatistikPembayaranCukai?tarikhMula="+mula_tahun+"&tarikhAkhir="+akhir_tahun;
	...
	function cetakSenaraiCukai(){
		var mula_tahun = document.${formName}.txdMula.value.substring(6,10);
  		var akhir_tahun = document.${formName}.txdAkhir.value.substring(6,10);
  		var intMula = parseInt(mula_tahun);
  		var intAkhir = parseInt(akhir_tahun);
		var negeri = document.${formName}.socNegeri.value;
		var daerah = document.${formName}.socDaerahBaru.value;
	
		if(negeri == "" || negeri == "0"){
  			alert("Sila pastikan \"Negeri\" dipilih.");
			document.${formName}.socNegeri.focus(); 
			return;
  		}
		else if(daerah == "" ){
  			alert("Sila pastikan \"Daerah\" dipilih.");
			document.${formName}.socDaerahBaru.focus(); 
			return;
  		}else if(document.${formName}.txdMula.value == "" ){
  			alert("Sila pastikan \"Tarikh Dari\" dilengkapkan.");
			document.${formName}.txdMula.focus(); 
			return;
  		}
			...
			var url = "../servlet/ekptg.report.htp.LaporanSenaraiCukai?tarikhMula="+mula_tahun+"&tarikhAkhir="+akhir_tahun+"&negeri="+negeri+"&daerah="+daerah;
		    var hWnd = window.open(url,'Cetak','full=yes, resizable=yes,scrollbars=yes');
		    if ((document.window != null) && (!hWnd.opener))
		   	hWnd.opener = document.window;
		    if (hWnd.focus != null) hWnd.focus();
