...
<fieldset>
	<table width="90%" border="0" >
		<tr>
  			<td align="center">
 				<input type="button" name="cmd " id="cmdCetak" value="Cetak" onClick="cetak('$!jenisTempoh')">
...
function doChange() {
	...
	document.${formName}.action = "?_portal_module=ekptg.view.ppt.LaporanSiasatan";
...
function cetak(jenisTempoh){
	...
	//julat
		...
		if(jenisTempoh=="julat"){
			var x = document.${formName}.socBulanSehingga.value;
			if(x=="12"){
				bulanSE = parseInt(x);
			}else{
				bulanSE = parseInt(x)+1;
			}			
	    ...
	    var item = "BULANSE='"+bulanSE2+"'&TAHUNSE='"+tahunSE+"'&BULAN='"+bulan+"'&type="+type+"&TAHUN='"+tahun+"'&bulantahun='"+bulantahun+"'&jenisTempoh='"+jenisTempoh+"'&bulantahunSE='"+bulantahunSE+"'";		
	 	var url = "../servlet/ekptg.report.ppt.LaporanJadualSiasatan?"+item 
