...
<div id="TabbedPanels1" class="TabbedPanels">
  <ul class="TabbedPanelsTabGroup">
    <li class="TabbedPanelsTab" tabindex="0">PTG/PTD</li>
    <li class="TabbedPanelsTab" tabindex="0" onclick="tabDalamMukim('$idFail','$idPermohonan')">Dalam Mukim</li>
  ...
  <tr>
    ...
    <td>
     ...
     #end
     #if($mode == 'updateNotis')
      <input name="cmdSimpan" id="cmdSimpan" type="button" value="Simpan" onclick="simpan_update('$id_notisawam')" />
      <input name="cmdBatal" id="cmdBatal" type="button" value="Batal" onclick="batalupdate()" />
      #end
    ...
    <br>
    <fieldset>
    <legend><strong>Senarai Notis PTG/PTD</strong></legend>
    <table width="100%">
  <tr>
    <td colspan="3"><label>
   	#if($mode != 'newNotis')
  ...
  <tr class = "table_header">
    ...
    <td width="36%"><strong>Tarikh Tampal</strong></td>
  </tr>
  ...
  <tr>
    <td class="$row">$fail.bil</td>
     #if ($fail.bil != '') 
    <td class="$row"><a href="javascript:viewNotis('$fail.id_notisawam')"><font color="blue">$fail.tempat</font></a></td>
     #else
...
<script>
function tabDalamMukim(id_fail,id_permohonan) {
...
function simpan(id_permohonan){
	...
	document.${formName}.id_permohonan.value = id_permohonan;
	document.${formName}.action = "?_portal_module=ekptg.view.ppt.SementaraNotisAwam&action=addPTGPTD";
	document.${formName}.submit();
}
function batal(){
...
function kemaskini(){
	...
	document.${formName}.action = "?_portal_module=ekptg.view.ppt.SementaraNotisAwam&action=updateNotisPTG";
	document.${formName}.submit();
}
function viewNotis(id_NotisAwam){
	
	document.${formName}.id_NotisAwam.value = id_NotisAwam;
	document.${formName}.action = "?_portal_module=ekptg.view.ppt.SementaraNotisAwam&action=viewNotisPTG";
	document.${formName}.submit();
}
function kembali(){
...
function simpan_update(id_NotisAwam){
	...
	if ( !window.confirm("Adakah Anda Pasti?") ) return;
	document.${formName}.id_NotisAwam.value = id_NotisAwam;
	document.${formName}.action = "?_portal_module=ekptg.view.ppt.SementaraNotisAwam&action=simpanUpdatePTG";
	document.${formName}.submit();
}
...
function hapus(){
	...
	if ( !window.confirm("Adakah Anda Pasti?") ) return;
	document.${formName}.action = "?_portal_module=ekptg.view.ppt.SementaraNotisAwam&action=hapusNotisPTG";
	document.${formName}.submit();
}
function tambah(){
