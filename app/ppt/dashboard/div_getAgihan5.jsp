...
<input type="hidden" id="div_getAgihan5_open" name="div_getAgihan5_open" value="Y">
...
<td align="right" valign="top"><a href="javascript:gotoAgihan5()" id="trigger">
...
<tr  >
<td valign="top" >
SENARAI FAIL (TELAH DIAGIHKAN)
 </td>
</tr>
</table>

<div id="scroll_boxAgihan5">
<table border="0" width="100%" > 
   ...
   <script type="text/javascript">
      ...
      $jquery(document).ready(function(e) {
		 $jquery('#boxAgihan5').show();
		    width  = 500;			
		    left = width+e.pageX;
            top  = height+e.pageY;	
			
			if(document.getElementById('boxAgihan5').clientHeight>250)
			{
		    height = 250;
			//$jquery("#boxAgihan5").css('width', width).css('height', height).css('overflow-y','scroll').css('top', top).css('left', left);
				$jquery("#boxAgihan5").css('width', width).css('height', height).css('top', top).css('left', left);	
			$jquery("#scroll_boxAgihan5").css('width', width).css('height', height-55).css('top', top).css('left', left).css('overflow-y','scroll');
			
			}
			else
			{
			height = document.getElementById('boxAgihan5').clientHeight;	
			$jquery("#boxAgihan5").css('width', width).css('height', height).css('top', top).css('left', left);	
			}	   
        });	
		
		/*
		 $jquery("div#boxAgihan5").mousemove(function(e) {
			 $jquery("div#boxAgihan5").css('top', e.pageY).css('left', e.pageX);
			});
		*/	
		
		
		var boxAgihan5 = $jquery("#boxAgihan5")
		boxAgihan5.offset({
			left: left,
			top: top
		});		
