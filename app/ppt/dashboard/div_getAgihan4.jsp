...
<input type="hidden" id="div_getAgihan4_open" name="div_getAgihan4_open" value="Y">
...
<td align="right" valign="top"><a href="javascript:gotoAgihan4()" id="trigger">
...
<tr  >
<td valign="top" >
SENARAI FAIL (TUNGGU UNTUK DIAGIKAN)
 </td>
</tr>
</table>

<div id="scroll_boxAgihan4">
<table border="0" width="100%" > 
   ...
   <script type="text/javascript">
      ...
      $jquery(document).ready(function(e) {
		   $jquery('#boxAgihan4').show();
		    width  = 500;			
		    left = width+e.pageX;
            top  = height+e.pageY;	
			
			if(document.getElementById('boxAgihan4').clientHeight>250)
			{
		    height = 250;
			//$jquery("#boxAgihan4").css('width', width).css('height', height).css('overflow-y','scroll').css('top', top).css('left', left);
			$jquery("#boxAgihan4").css('width', width).css('height', height).css('top', top).css('left', left);	
			$jquery("#scroll_boxAgihan4").css('width', width).css('height', height-55).css('top', top).css('left', left).css('overflow-y','scroll');	
			}
			else
			{
			height = document.getElementById('boxAgihan4').clientHeight;	
			$jquery("#boxAgihan4").css('width', width).css('height', height).css('top', top).css('left', left);	
			}	   	     
        });	
		
		/*
		 $jquery("div#boxAgihan4").mousemove(function(e) {
			 $jquery("div#boxAgihan4").css('top', e.pageY).css('left', e.pageX);
			});
		*/	
		
		
		var boxAgihan4 = $jquery("#boxAgihan4")
		boxAgihan4.offset({
			left: left,
			top: top
		});		
