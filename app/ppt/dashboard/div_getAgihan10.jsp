...
<input type="hidden" id="div_getAgihan10_open" name="div_getAgihan10_open" value="Y">
...
<td align="right" valign="top"><a href="javascript:gotoAgihan10()" id="trigger">
...
<tr  >
<td valign="top" >
SENARAI FAIL (TUNGGU DIAGIHKAN KEPADA PENTADBIR)
 </td>
</tr>
</table>

<div id="scroll_boxAgihan10">
<table border="0" width="100%" > 
   ...
   <script type="text/javascript">
      ...
      $jquery(document).ready(function(e) {
		   $jquery('#boxAgihan10').show();
		    width  = 500;			
		    left = width+e.pageX;
            top  = height+e.pageY;	
			
			if(document.getElementById('boxAgihan10').clientHeight>250)
			{
		    height = 250;
			//$jquery("#boxAgihan10").css('width', width).css('height', height).css('overflow-y','scroll').css('top', top).css('left', left);
			$jquery("#boxAgihan10").css('width', width).css('height', height).css('top', top).css('left', left);	
			$jquery("#scroll_boxAgihan10").css('width', width).css('height', height-55).css('top', top).css('left', left).css('overflow-y','scroll');	
			}
			else
			{
			height = document.getElementById('boxAgihan10').clientHeight;	
			$jquery("#boxAgihan10").css('width', width).css('height', height).css('top', top).css('left', left);	
			}	   	     
        });	
		
		/*
		 $jquery("div#boxAgihan10").mousemove(function(e) {
			 $jquery("div#boxAgihan10").css('top', e.pageY).css('left', e.pageX);
			});
		*/	
		
		
		var boxAgihan10 = $jquery("#boxAgihan10")
		boxAgihan10.offset({
			left: left,
			top: top
		});		
