...
#if($!div_getListEndorsanK_open == "Y")

<div id="log"  style="display:none"></div>
<div id="boxListEndorsanK" >
...
<input type="hidden" id="div_getListEndorsanK_open" name="div_getListEndorsanK_open" value="Y">
...
<td align="right" valign="top"><a href="javascript:getListEndorsanKClose()" id="trigger">
...
<tr  >
<td valign="top" >
SENARAI HAKMILIK (Tarikh hantar endorsan melebihi 14 hari)
 </td>
</tr>
</table>

<div id="scroll_boxListEndorsanK">
...
<table border="0" width="100%" > 
   ...
   <tr class="table_header">
   ...
   #if($listEndorsanK.size()>0)
 			       #set( $i = 0 )   
           #foreach($list in $listEndorsanK)
          ...
          <tr>
           ...
           #set($span3 = "span3"+$i)
            ...
            #if($list.NO_JKPTG == '')
            	<a href="javascript:papar('$list.ID_PERMOHONAN','76','$list.ID_FAIL','$list.TARIKH_PERMOHONAN','$list.FLAG_JENISPERMOHONAN','$list.ID_SUBURUSAN','$!list.FLAG_SEGERA','$!{session.getAttribute('myrole')}')" class="style1">TIADA NO Hakmilik JKPTG </a>
            #else
            	<a href="javascript:papar('$list.ID_PERMOHONAN','76','$list.ID_FAIL','$list.TARIKH_PERMOHONAN','$list.FLAG_JENISPERMOHONAN','$list.ID_SUBURUSAN','$!list.FLAG_SEGERA','$!{session.getAttribute('myrole')}')" class="style1">$list.NO_JKPTG.toUpperCase()</a>
            #end
   ...
   <script type="text/javascript">
      ...
      $jquery(document).ready(function(e) {
		    ...
		    //width  = 600;	
			width = document.getElementById("div_listEndorsanK").offsetWidth;		
		    left = width+e.pageX;
			top  = height+e.pageY;			
			height = 300;
			$jquery("#boxListEndorsanK").css('width', width).css('height', height).css('top', top).css('left', left);
			$jquery("#scroll_boxListEndorsanK").css('width', width).css('height', height-55).css('top', top).css('left', left).css('overflow-y','scroll');
			//$jquery("#Peringatan_Main").css('overflow-y','scroll');
		  	     
        });	
		
		
		
		var boxListEndorsanK = $jquery("#boxListEndorsanK")
		boxListEndorsanK.offset({
			left: left,
			top: top
		});		
		var drag = {
		...
		$jquery(document).mousemove(function(e) {
			...
			if (drag.state) {
				...
				drag.x = e.pageX;
				drag.y = e.pageY;
				$jquery("div#boxListEndorsanK").css('top', e.pageY).css('left', e.pageX);
			}
		});
...
if('$!search' != "")
{
	...
	highlightEndTag = "</font>";
	  ...
	  for (x = 0; x < parseInt('$ListEndorsanK.size()'); x++)
	  {
	  var span1 = "span1"+(x+1);
	  var span2 = "span2"+(x+1);
	  var span3 = "span3"+(x+1);	  
