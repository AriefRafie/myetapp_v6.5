...
<input type="hidden" id="div_getOnline8_open" name="div_getOnline8_open" value="Y">
<table border="0" width="100%" > 
<tr>
<td align="left" valign="top"><img id="move" name="move" src="../img/drag.png" title='Ubah Lokasi' onMouseOver="this.style.cursor='move'" width="18" height="18" align="center"/>
</td>
<td align="right" valign="top"><a href="javascript:gotoSek8()" id="trigger">
...
<tr  >
<td valign="top" >
PERMOHONAN ONLINE SEKYEN 8
 </td>
</tr>
</table>

<div id="scroll_boxOnline8" >
<table border="0" width="100%"  > 
   ...
   <tr class="table_header">
    ...
    #if($listing_online_sek8.size()>0)
           
           #foreach($list in $listing_online_sek8)
            #set( $i = $velocityCount )
         		#if ( ($i % 2) != 1 )
              		 #set( $row = "row2" )
         		#else
 ...
 function carian()
 {
	doDivAjaxCall$formname('div_getOnline8','getOnline8_carian','');	 
 }
 </script>
   ...
   <script type="text/javascript">
      ...
      $jquery(document).ready(function(e) {
		  $jquery('#boxOnline8').show();
		   width  = 500;			
		    left = width+e.pageX;
            top  = height+e.pageY;				
			if(document.getElementById('boxOnline8').clientHeight>250)
			{
		    height = 250;
			//$jquery("#boxOnline8").css('width', width).css('height', height).css('overflow-y','scroll').css('top', top).css('left', left);
			$jquery("#boxOnline8").css('width', width).css('height', height).css('top', top).css('left', left);	
			$jquery("#scroll_boxOnline8").css('width', width).css('height', height-55).css('top', top).css('left', left).css('overflow-y','scroll');	
			}
			else
			{
			height = document.getElementById('boxOnline8').clientHeight;	
			$jquery("#boxOnline8").css('width', width).css('height', height).css('top', top).css('left', left);	
			}
		...
		/*
		 $jquery("div#boxOnline8").mousemove(function(e) {
			 $jquery("div#boxOnline8").css('top', e.pageY).css('left', e.pageX);
			});
		*/	
		
		
		var boxOnline8 = $jquery("#boxOnline8")
		boxOnline8.offset({
			left: left,
			top: top
		});		
