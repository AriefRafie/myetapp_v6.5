		      ...
		      <tr> 
		       ...
		       <tr> 
					    ...
					    <td>
					      ...
					      <!-- Tambah Oleh Mohamad Rosli 14/07/2011 -->
					    	<input type="radio" name="socJenisTanahtemp" value="1" $checkedMilik  onclick="senaraiLaporan();"/>TANAH MILIK PERSEKUTUAN
					 		<input type="radio" name="socJenisTanahtemp" value="2" $checkedRizab  onclick="senaraiLaporan();"/>TANAH RIZAB PERSEKUTUAN
					    	<input type="hidden" name="socJenisTanah" value="$idJenisTanah">
					</td>
			  	</tr>
		     	<tr>
        			<td align="right">&nbsp;Perolehan</td>
 					<td>:&nbsp;</td>
			        <td>
			            <input name="status" type="checkbox" value="1,10"/>
			            Permohonan			            
			            <input name="status" type="checkbox" value="2"/>
			            Pembelian
			         	<input name="status" type="checkbox" value="5"/>
			            Perletakhakan			            
			        </td>
      </tr>
		       	
		        <td scope="row" align="right">&nbsp;Tarikh </td>
		        <td>:&nbsp;</td>
		        <td>
		       		<label>
		 	...
		 	#foreach ( $senarai in $senaraiLaporan )
		  	...
		  	<td class="$row">
		  		<a href="javascript:openLaporan('$senarai.idmodule','$para','$senarai.peringkat','$senarai.template')" class="style1" >$senarai.keterangan</a>
			         	...
			         	<input name="perolehan" type="hidden" />

</table>

<script>
	// frmSenaraiLaporanHTP.jsp
	...
	function openLaporan(urli,param,laporan,tem){
		...
		if(laporan=="negeri"){
		...
		}else if(laporan=="negeriper"){
			if(negeri=="0"){
				alert("Sila pilih \"Negeri\" terlebih dahulu.");
				document.${formName}.socNegeri.focus(); 
				return;
			}	
			field_ = document.${formName}.status;
			var intChecked = 0;
			var varperolehan = "0";
			for (i = 0; i < field_.length; i++){
				if(field_[i].checked==true){
				//perolehan
					//document.${formName}.status.value = field_[i].value;
					if(i==0){
						varperolehan = field_[i].value;
					}else{
						varperolehan += ","+field_[i].value;
					}
			...
			if(intChecked==0){
				alert("Sila pilih \"Perolehan\" terlebih dahulu.");
				return;
			}	
			//alert(varperolehan);
			pnegeri = "&PEROLEHAN="+varperolehan+"&ID_KEMENTERIAN=0&ID_AGENSI=0&ID_NEGERI="+negeri+"&bulan=1";
			...
			if(laporan=="kemselangthn"){
				...
				pnegeri = "&PEROLEHAN="+varperolehan+"&ID_NEGERI="+negeri;
	...
	// Digunakan untuk Laporan Tanah Milik/Rizab
