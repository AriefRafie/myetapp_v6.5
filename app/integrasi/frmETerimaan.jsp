...
#end
<fieldset>
  <legend><strong>eTerimaan</strong>
...
<fieldset>
  ...
  <table width="100%">
    <tr class="table_header">
      ...
      <td valign="top" width="15%" align="center"><strong>Status</strong></td>
...
#foreach ($list in $ListUploaded)
    ...
    <tr>
      ...
      <td class="$row" valign="top"><a href="javascript:displayFile('$list.IDFail')" style="color:#0000FF">$list.NamaFail</a></td>
...
#if ($list.MIME_Type == 'text/plain')
        <input type="button" id="cmdProses" name="cmdProses" value="PROSES FAIL" onclick="processFile('$list.IDFail');">
#end    
      </td>
      <td class="$row" valign="top" align="center">
        <input type="button" id="cmdHapus" name="cmdHapus" value="HAPUS FAIL" onclick="deleteFile('$list.IDFail');">
      </td>
    #else
...
<script type="text/javascript">
  ...
  if (ans) {
	  document.${formName}.action = "?_portal_module=ekptg.view.integrasi.FrmViewETerimaan&action2=processFile&IDFail=$IDFail";
...
<script type="text/javascript">
  ...
  function deleteFile(IDFail) {	
      if (!window.confirm("Adakah anda pasti?")) return;
      document.${formName}.action = "?_portal_module=ekptg.view.integrasi.FrmViewETerimaan&action2=deleteFile&IDFail=" + IDFail;
      document.${formName}.method = "POST";
      document.${formName}.submit();
  }
  function displayFile(IDFail) {	
      var url = "../servlet/ekptg.view.integrasi.DisplayBlob?ptype=ETERIMAAN&IDFail=" + IDFail;
  ...
  function uploadFile() {	
      ...
      if (!window.confirm("Adakah anda pasti?")) return;
      document.${formName}.action = "?_portal_module=ekptg.view.integrasi.FrmViewETerimaan&action2=uploadFile";
      document.${formName}.method = "POST";
  ...
  function emptyForm() {
      document.${formName}.action = "?_portal_module=ekptg.view.integrasi.FrmViewETerimaan&action2=";
      document.${formName}.method = "POST";
      document.${formName}.submit();
  }
</script>