...
<!--           <input name="cmdDaftar" type="button" value="Daftar Hakmilik" onClick="javascript:daftarHakmilik()"/> -->
        ...
        <tr class="table_header">
         ...
         <td ><input type="checkbox" name="all1" id="all1" onClick="doCheckAll1();" title="Semak untuk hapuskan semua fail" /></td>
        </tr>
        #set ($list = "")
        ...
        <tr>
          ...
          #set($id_hakmilik_final = "")          
          #if($list.idHakmilikAgensi != "")
          	#set($id_hakmilik_final = "AGENSI"+$list.idHakmilikAgensi)
          #elseif($list.idHakmilikSementara != "")
          	#set($id_hakmilik_final = "SEMENTARA"+$list.idHakmilikSementara)
          #end
          
            <td ><input type="checkbox" name="ids1" id="ids1" onClick="doUpdateCheckAll1()" value="$id_hakmilik_final" title="Semak untuk pilih fail ini" ></td>
...
function kosongkan(idNegeriPemohon) {
	...
	document.${formName}.action = "?_portal_module=ekptg.view.php2.FrmPLPPopupSenaraiTanahBerkaitanView";
...
function pilihHakmilik() {	
	document.${formName}.actionPopup.value = "pilih";
	document.${formName}.submit();
}

function closePopup() {	
try {
        window.opener.doChangeTabUpper("1");                        
    }
    catch (err) {}
    window.close();
    return false;
