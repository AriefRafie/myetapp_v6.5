<div id="lampiranupload_progress"></div>
<div id="progressBar" style="display: none;">
  <div id="progressBarBoxContent"></div>
</div>
#if ($completed)
<script>
parent.document.getElementById("lampiranupload_progress").innerHTML="<div class=\"success\">Fail berjaya dimuatnaik.</div>";
</script>
#end
<table width="100%" border="0" cellspacing="2" cellpadding="2">
  #if ($flagPopup == 'openPopupDokumen')
  <tr>
    <td> #parse("app/php2/frmCRBLampiranDetail.jsp") </td>
  </tr>
  <tr>
    <td>&nbsp;</td>
  </tr>
  #end
  <tr>
    <td><fieldset>
      <legend><strong>SENARAI LAMPIRAN</strong></legend>
      <table align="center" width="100%">
        #if ($flagPopup == '')
        <tr>
          <td colspan="2" scope="row"><input name="cmdDaftar" type="button" value="Tambah" onclick="javascript:daftarDokumen()"/></td>
        </tr>
        #end
        <tr class="table_header">
          <td scope="row" width="5%" align="center"><strong>Bil</strong></td>
          <td><strong>Nama Lampiran</strong></td>
        </tr>
        #set ($senaraiLampiran = "")
        #if ($SenaraiLampiran.size() > 0)
        #foreach ($senaraiLampiran in $SenaraiLampiran)
        #if ($senaraiLampiran.bil == '')
        #set( $row = "row1" )
        #elseif (($senaraiLampiran.bil % 2) != 0)
        #set( $row = "row1" )
        #else 
        #set( $row = "row2" )
        #end
        <tr>
          <td class="$row" align="center">$senaraiLampiran.bil</td>
          <td class="$row"><a href="javascript:paparDokumen($senaraiLampiran.idDokumen)" class="style2">$senaraiLampiran.namaDokumen</a></td>
        </tr>
        #end
        #else
        <tr>
          <td class="row1" align="center">&nbsp;</td>
          <td class="row1">Tiada Rekod</td>
        </tr>
        #end
      </table>
      </fieldset></td>
  </tr>
  #if ($flagPopup == '')
  <tr>
    <td align="center"><input type="button" name="cmKembali" id="cmKembali" value="Kembali" onclick="batalLawatanTapak()"/>
    </td>
  </tr>
  #end
</table>

