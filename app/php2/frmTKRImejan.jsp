...
<table width="100%" border="0" cellspacing="2" cellpadding="2">
  ...
  <tr>
    <td> #parse("app/php2/frmTKRImejDetail.jsp") </td>
  </tr>
  <tr>
    <td>&nbsp;</td>
  </tr>
...
function simpanDokumen(idLaporanTanah,idHakmilikPermohonan,idTanahGanti,flagJenisTanah) {
	...
	document.${formName}.action = "?_portal_module=ekptg.view.php2.FrmTKRLawatanTapakView&hitButton=simpanDokumen&namaImej="+namaImej+"&catatanImej="+catatanImej+"&idLaporanTanah="+idLaporanTanah+"&selectedTabUpper=5"+dopost+"&actionTukarguna=papar&mode=view&flagPopup=openPopupDokumen&modePopup=new&idHakmilikPermohonan="+idHakmilikPermohonan+"&idTanahGanti="+idTanahGanti+"&flagJenisTanah="+flagJenisTanah;
	document.${formName}.method="post";
...
function cetakLaporanTanahAsal(idFail,idLaporanTanah) {
	var url = "../servlet/ekptg.report.php2.TKRLaporanTanahAsal?ID_FAIL="+idFail+"&ID_LAPORANTANAH="+idLaporanTanah;
...
function cetakLaporanTanahGanti(idFail,idLaporanTanah) {
	var url = "../servlet/ekptg.report.php2.TKRLaporanTanahGanti?ID_FAIL="+idFail+"&ID_LAPORANTANAH="+idLaporanTanah;
    var hWnd = window.open(url,'printuser','width=900,height=300, resizable=yes,scrollbars=yes');
    if ((document.window != null) && (!hWnd.opener))
       hWnd.opener = document.window;
    if (hWnd.focus != null) hWnd.focus();
