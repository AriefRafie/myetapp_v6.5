...
<input type="hidden" name="idFailJilid" id="idFailJilid" value="$idFailJilid"/>
...
<input name="noFailJilid" id="noFailJilid" type="hidden" value="$!noFailJilid"/>
<input name="mode" id="mode" type="hidden" value="$!mode"/>

     <fieldset>
			<legend>MAKLUMAT FAIL BARU</legend>
             #if($jilid == 'success')
  	<div class="success">Fail Jilid baru telah dijana.</div>
 			#end
                   <table width="100%">
          <tr>
          <td scope="row">&nbsp;</td>
        ...
        <tr>
          ...
          <td><span class="style42">
            <label>$noFailJilid.toUpperCase()</label>
            </span> </td>
        </tr>
         <tr>
          <td class="style40" scope="row">&nbsp;</td>
          ...
          <td><span class="style43">
            <label>$noFailAsal.toUpperCase()  </label>
          </span></td>
         </tr>
       #else
        <tr>
        ...
        <tr>
          ...
          $tajukFailB
                ...
                <tr>
                  <td class="style40" scope="row"><div align="center" class="style1"></div></td>
                  <td scope="row"><span class="style43">Seksyen / Negeri</span></td>
...
function update1(idFail){
	...
	document.${formName}.action = "?_portal_module=ekptg.view.pfd.PendaftaranFailJilid&action1=updateFailNegeriBaru";
...
function kemaskini(idFail){
	...
	document.${formName}.action = "?_portal_module=ekptg.view.pfd.PendaftaranFailJilid&action1=kemaskiniFailNegeriBaru";
	document.${formName}.idFail.value = idFail;
	document.${formName}.submit();
...
function kembali(){
	...
	document.${formName}.action ="?_portal_module=ekptg.view.pfd.PendaftaranFailJilid&action1=";
	document.${formName}.submit();
}
function simpan(id){
	
	if ( !window.confirm("Adakah Anda Pasti?") ) return;
	document.${formName}.action = "?_portal_module=ekptg.view.pfd.PendaftaranFailJilid&action1=simpan&idFailAsal="+id;
	document.${formName}.submit();
}
function tabFailNegeriLama(){
	document.${formName}.action = "?_portal_module=ekptg.view.pfd.PendaftaranFailJilid&action1=tabFailNegeriLama&modeAktiviti=";
	document.${formName}.submit();
}
...
function aktivitiTiada(){
	document.${formName}.action = "?_portal_module=ekptg.view.pfd.PendaftaranFailJilid&action1=tabFailNegeriBaru&modeAktiviti=tiada&socNegeriB="+document.${formName}.socNegeriB.value+"&socDaerahB="+document.${formName}.socDaerahB.value;
	document.${formName}.submit();

}
function aktivitiAda(){
	document.${formName}.action = "?_portal_module=ekptg.view.pfd.PendaftaranFailJilid&action1=tabFailNegeriBaru&modeAktiviti=ada&socNegeriB="+document.${formName}.socNegeriB.value+"&socDaerahB="+document.${formName}.socDaerahB.value;
	document.${formName}.submit();
