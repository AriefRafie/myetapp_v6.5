 ...
 $command
 -->
 #set($param = "&ID_WARTATRMINDUK="+$ID_WARTATRMINDUK+"&LOCATION="+$divSenarai+"&JENISSUB=$JENISSUB")
 <!--$param-->
 <input type="hidden" name="scrolPosition$command" id="scrolPosition$command" >
           ...
           <table width="100%" border="0" cellpadding="0" cellspacing="0">
                <tr>
                    ...
                    <td  align="right">
                        ...
                        onChange="doDivAjaxCall$formname('$div','$command','action=doChangeItemPerPage&FLAG_CARIAN=$FLAG_CARIAN&scrolPosition='+getPageLocation('$command')+'$param');"
                        >
	  ...
	  <table width="100%" cellpadding="0" cellspacing="0" border="0">
		...
		<td width="70%" align="left" valign="middle">
		   ...
		   onClick="doDivAjaxCall$formname('$div','$command','action=getPage&FLAG_CARIAN=$FLAG_CARIAN&value=1&scrolPosition='+getPageLocation('$command')+'$param');"
		   src="../img/paging/page-first.gif">
		   #if ($page > 10)
		   	<img title="Previous 10" style="vertical-align=bottom;cursor:hand;cursor:pointer;" 
		   	onClick="doDivAjaxCall$formname('$div','$command','action=getPrevious&FLAG_CARIAN=$FLAG_CARIAN&page=$previous10&scrolPosition='+getPageLocation('$command')+'$param');"
		...
		&nbsp;
			...
			#foreach ( $i in [$page_mula..$totalPages] )
					...
					#else
					<a 
					href="javascript:doDivAjaxCall$formname('$div','$command','action=getPage&FLAG_CARIAN=$FLAG_CARIAN&value=$i&scrolPosition='+getPageLocation('$command')+'$param');"
		    		>[$i]</a>
					#end
				#end
			#end
		...
		&nbsp;
		    ...
		    onClick="doDivAjaxCall$formname('$div','$command','action=getNext&FLAG_CARIAN=$FLAG_CARIAN&page=$next10&scrolPosition='+getPageLocation('$command')+'$param');"
		    src="../img/paging/page-next.gif">
		    <img title="Last page:$totalPages" style="vertical-align=bottom;cursor:hand;cursor:pointer;" 
		    onClick="doDivAjaxCall$formname('$div','$command','action=getPage&FLAG_CARIAN=$FLAG_CARIAN&value=$totalPages&scrolPosition='+getPageLocation('$command')+'$param');"
