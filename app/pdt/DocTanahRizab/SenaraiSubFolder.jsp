...
#if($LAYER=="1")
<div id="div_statsUtama$ID_DOCTRMUTAMA" style="display:none" >	
#if($editable=="Y")
<a href="javascript:doDivAjaxCall$formname('divSubFolderTambahFirst$ID_DOCTRMUTAMA','addSubDir','ID_REFER=&LAYER=$LAYER&ID_DOCTRMUTAMA=$ID_DOCTRMUTAMA');">
<img title="Tambah Direktori" src="../img/plus.gif" border="0"></a>	   		
#end   
[<span id="div_totalDirUtama$ID_DOCTRMUTAMA" >0</span> Direktori Dibuka] [<span id="div_totalLampiranUtama$ID_DOCTRMUTAMA" >0</span> Dokumen Dibuka]
</div>
<div id="divSubFolderTambahFirst$ID_DOCTRMUTAMA" >
</div>
#end
...
if('$LAYER' != "1")
{
	document.getElementById('divSubFolderTambah$ID_DOCTRM').innerHTML='';
...
<table width="98%" align="center" class="classFade" border="0" cellspacing="0" cellpadding="0">
	...
	<td >
	    ...
	    <input type="hidden" id="tempFieldFolder$LFS.ID_DOCTRMUTAMA" name="tempFieldFolder$LFS.ID_DOCTRMUTAMA">
		<fieldset>
		...
		*/
		...
		#if($LFS.LAYER=="1")
			#set($span1 = "span1listFolderSub_"+$LAYER+"_"+$LFS.ID_DOCTRMUTAMA+"_"+$LFS.BIL)
			#set($spaneditsub = "spaneditsub_"+$LAYER+"_"+$LFS.ID_DOCTRMUTAMA+"_"+$LFS.BIL)
			#set($divLoadSubAfterDelete = "div_viewFolderUtama"+$ID_DOCTRMUTAMA)
		#else
			#set($span1 = "span1listFolderSub_"+$LFS.ID_DOCTRM+"_"+$LFS.BIL)
			#set($spaneditsub = "spaneditsub_"+$LFS.ID_DOCTRM+"_"+$LFS.BIL)
			#set($divLoadSubAfterDelete = "divSubFolder"+$ID_DOCTRM)
		#end	
		
		<legend id="$spaneditsub" >			
		<span class="font_tajuk_sub_wc" >
		#if($editable=="Y")
		<a href="javascript:doDivAjaxCall$formname('$span1','editSubDir','ID_DOCTRM=$LFS.ID_DOCTRM&LAYER=$LFS.LAYER&BIL=$LFS.BIL');"><img title="Kemaskini Nama Sub Direktori" src="../img/edit.gif" border="0"></a>
	    <a href="javascript:if(confirm('Data akan dipadam. Adakah Anda Pasti?')){ doDivAjaxCall$formname('$divLoadSubAfterDelete','deleteSubDir','FLAG_SUB_OPENCLOSE='+$jquery('#HID_OPENCLOSE_SUB_$ID_DOCTRM').val()+'&TAJUK=$TAJUK&FLAG_OPENCLOSE=&ID_DOCTRMUTAMA=$ID_DOCTRMUTAMA&ID_REFER=$ID_DOCTRM&ID_TO_DELETE=$LFS.ID_DOCTRM&ID_DOCTRM=$ID_DOCTRM&LAYER=$LFS.LAYER&AUTOLOAD=N&carianTerperinci='+$jquery('#carianTerperinci').val()+'&carianTerperinciLampiran='+$jquery('#carianTerperinciLampiran').val()+'&JUMLAH_SUB=$LFS.JUMLAH_SUB');}"><img title="Hapus Direktori"  src="../img/hapus.gif" border="0"></a>
	    #end
	    </span> 					
		...
		-->
		<input type="hidden" id="$ID_DOCTRMUTAMA$SEND_NUMBERING" name="$ID_DOCTRMUTAMA$SEND_NUMBERING" value="$LFS.ID_DOCTRM">	
		<input type="hidden" id="check_$ID_DOCTRMUTAMA$SEND_NUMBERING" name="check_$ID_DOCTRMUTAMA$SEND_NUMBERING" value="CLOSE">	
		<input type="hidden" id="checkID_REFER_$ID_DOCTRMUTAMA$SEND_NUMBERING" name="checkID_REFER_$ID_DOCTRMUTAMA$SEND_NUMBERING" value="$LFS.ID_REFER">	
		<input type="hidden" id="checkNextLayer_$ID_DOCTRMUTAMA$SEND_NUMBERING" name="checkNextLayer_$ID_DOCTRMUTAMA$SEND_NUMBERING" value="$LFS.NEXTLAYER">	
		<input type="hidden" id="checkNumbering_$ID_DOCTRMUTAMA$SEND_NUMBERING" name="checkNumbering_$ID_DOCTRMUTAMA$SEND_NUMBERING" value="$SEND_NUMBERING">	
		
		
		
		 
		
		<input type="hidden" id="HID_OPENCLOSE_SUB_$LFS.ID_DOCTRM" name="HID_OPENCLOSE_SUB_$LFS.ID_DOCTRM" value = "CLOSE"  >
		...
		if(('$AUTOLOAD'=="Y" || '$FlagCari'=="Y"))
		{
			$jquery(document).ready(function () {
					  doDivAjaxCall$formname('divSubLampiran$LFS.ID_DOCTRM','showLampiran','FLAG_LAMP_OPENCLOSE='+$jquery('#HID_OPENCLOSE_LAMP_$LFS.ID_DOCTRM').val()+'&TAJUK=$LFS.TAJUK&FLAG_OPENCLOSE=&ID_DOCTRMUTAMA=$LFS.ID_DOCTRMUTAMA&ID_REFER=$LFS.ID_REFER&ID_DOCTRM=$LFS.ID_DOCTRM&LAYER=$LFS.LAYER&AUTOLOAD=Y&carianTerperinci='+$jquery('#carianTerperinci').val()+'&carianTerperinciLampiran='+$jquery('#carianTerperinciLampiran').val());
			});
		}
		...
		<tr>
		<td>
		<input type="hidden" id="HID_OPENCLOSE_LAMP_$LFS.ID_DOCTRM" name="HID_OPENCLOSE_LAMP_$LFS.ID_DOCTRM" value = "CLOSE"  >
		&nbsp;
		#if($editable=="Y")
		<a href="javascript:doDivAjaxCall$formname('divSubLampiranTambah$LFS.ID_DOCTRM','addLampiran','ID_DOCTRM=$LFS.ID_DOCTRM&ID_DOCTRMUTAMA=$LFS.ID_DOCTRMUTAMA');">
		<img title="Tambah Direktori" src="../img/plus.gif" border="0"></a>
		#end
		<span id="countLampiran$LFS.ID_DOCTRM">	
		#if($LFS.JUMLAH_LAMP != "0" && $LFS.JUMLAH_LAMP != "")
		<span onClick="doDivAjaxCall$formname('divSubLampiran$LFS.ID_DOCTRM','showLampiran','FLAG_LAMP_OPENCLOSE='+$jquery('#HID_OPENCLOSE_LAMP_$LFS.ID_DOCTRM').val()+'&TAJUK=$LFS.TAJUK&FLAG_OPENCLOSE=&ID_DOCTRMUTAMA=$LFS.ID_DOCTRMUTAMA&ID_REFER=$LFS.ID_REFER&ID_DOCTRM=$LFS.ID_DOCTRM&LAYER=$LFS.LAYER&AUTOLOAD=N&carianTerperinci='+$jquery('#carianTerperinci').val()+'&carianTerperinciLampiran='+$jquery('#carianTerperinciLampiran').val()+'&JUMLAH_LAMP=$LFS.JUMLAH_LAMP');" >
		...
		<div id="divSubLampiranTambah$LFS.ID_DOCTRM" >		
		</div>		
		<div id="divSubLampiran$LFS.ID_DOCTRM" >
		</div>	
		#set($setStyleCountFolder = "")
		#if($LAYER != "1")
		&nbsp;
		#if($editable=="Y")
		<a href="javascript:doDivAjaxCall$formname('divSubFolderTambah$LFS.ID_DOCTRM','addSubDir','ID_REFER=$LFS.ID_DOCTRM&LAYER=$LFS.NEXTLAYER&ID_DOCTRMUTAMA=$LFS.ID_DOCTRMUTAMA');">
		<img title="Tambah Direktori" src="../img/plus.gif" border="0"></a>	
		#end	
		#else
		...
		<span id="countSubFolder$LFS.ID_DOCTRM" $setStyleCountFolder >			
		<!--  
		(
		:::: $SEND_NUMBERING
		...
		<span onClick="doDivAjaxCall$formname('divSubFolder$LFS.ID_DOCTRM','showAllFolder','FLAG_SUB_OPENCLOSE='+$jquery('#HID_OPENCLOSE_SUB_$LFS.ID_DOCTRM').val()+'&TAJUK=$LFS.TAJUK&FLAG_OPENCLOSE=&ID_DOCTRMUTAMA=$LFS.ID_DOCTRMUTAMA&ID_REFER=$LFS.ID_DOCTRM&ID_DOCTRM=$LFS.ID_DOCTRM&LAYER=$LFS.NEXTLAYER&AUTOLOAD=N&carianTerperinci='+$jquery('#carianTerperinci').val()+'&carianTerperinciLampiran='+$jquery('#carianTerperinciLampiran').val()+'&JUMLAH_SUB=$LFS.JUMLAH_SUB&SEND_NUMBERING=$SEND_NUMBERING');" >
			<span class="font_tajuk_sub" >
		...
		<div id="divSubFolderTambah$LFS.ID_DOCTRM" >
		
		</div>
		<div id="divSubFolder$LFS.ID_DOCTRM" >
			...
			#if($LFS.LAYER=="1")
			<script>
			highlight_item($jquery('#carianTerperinci').val(),'span1listFolderSub_'+'$LAYER'+'_'+'$LFS.ID_DOCTRMUTAMA'+'_'+'$LFS.BIL'); 
			</script>
			#else
			<script>
			highlight_item($jquery('#carianTerperinci').val(),'span1listFolderSub'+'_'+'$LFS.ID_DOCTRM'+'_'+'$LFS.BIL'); 
			</script>
			#end
		#end
		<script>
...
//alert('$AUTOLOAD'+'____'+'$FlagCari');
		var  send_ID_DOCTRM_value  = $jquery('#'+send_ID_DOCTRM).val();
		...
		if(('$AUTOLOAD'=='Y' || '$FlagCari'=='Y'))
		{
			...
			var id_refer = $jquery('#checkID_REFER_'+send_ID_DOCTRM).val();
			var next_layer = $jquery('#checkNextLayer_'+send_ID_DOCTRM).val();
			var checkNumbering = $jquery('#checkNumbering_'+send_ID_DOCTRM).val();
			...
			$jquery(document).ready(function () {
				  ...
				  doDivAjaxCall$formname('divSubFolder'+send_ID_DOCTRM_value,'showAllFolder','FLAG_SUB_OPENCLOSE='+$jquery('#HID_OPENCLOSE_SUB_'+send_ID_DOCTRM_value).val()+'&TAJUK=&FLAG_OPENCLOSE=&ID_DOCTRMUTAMA=$ID_DOCTRMUTAMA&ID_REFER='+send_ID_DOCTRM_value+'&ID_DOCTRM='+send_ID_DOCTRM_value+'&LAYER='+next_layer+'&AUTOLOAD=Y&carianTerperinci='+$jquery('#carianTerperinci').val()+'&carianTerperinciLampiran='+$jquery('#carianTerperinciLampiran').val()+'&JUMLAH_SUB=&SEND_NUMBERING='+checkNumbering);
			});
		}
...
document.getElementById("HID_MAXLAYER_$ID_DOCTRMUTAMA").value = '$MAX_LAYER';
...
{
	$jquery(document).ready(function () {
		  doDivAjaxCall$formname('countSubFolder$ID_DOCTRM','countSubFolder','FLAG_SUB_OPENCLOSE='+$jquery('#HID_OPENCLOSE_SUB_$ID_DOCTRM').val()+'&ID_DOCTRMUTAMA=$ID_DOCTRMUTAMA&ID_DOCTRM=$ID_DOCTRM&LAYER=$LAYER&AUTOLOAD=N&TOTAL_SUB=$listFolderSub.size()');
	});
}
