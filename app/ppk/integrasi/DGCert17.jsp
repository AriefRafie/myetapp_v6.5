...
function openPopupPNB(NO_FAIL,id_perbicaraan,id_fail,signedData){
	...
	if (input_box == true) {
		try {
			window.opener.cetakBorangS_X(NO_FAIL,id_perbicaraan,id_fail,signedData);
		}
		catch (err) {}
	   	window.close();	
	    return false;
...
function sendToDigitalSign() {
		...
		if (input_box == true) {
			...
			document.f1.action="ekptg.view.ppk.FrmIntegrasiDGCert17?command=sahTandatangan";
			document.f1.submit();			
		}
...
function simpan() {
	...
	//alert("idfail :"+idfail);
	//var signedText = document.f1.signedText.value;
	var signedData = $("#signedText").val();
	//alert("signedData :"+signedData);
	
	document.f1.method="post";
	document.f1.action="ekptg.view.ppk.FrmIntegrasiDGCert17?command=simpanTemp";
