...
<!-- START [ tabbed 1 ] -->  
  ...
  <fieldset>  
  <legend>SENARAI SEMAK PENERIMAAN BORANG K1</legend>	
     <table width="65%"  cellpadding="1" cellspacing="1" border="0">
     	<tr>
     		...
     		<td width="95%">K1 sempurna diisi</td>
     	</tr>
     	<tr>
	 ...
	 <table width="65%"  cellpadding="1" cellspacing="1" border="0">
     	<tr>
     		...
     		<td width="95%">K1 dikembalikan dalam tempoh 14 hari</td>
     	</tr>
     </table>	
 </fieldset>  
 ...
 <fieldset>    
     <table width="65%"  cellpadding="1" cellspacing="1" border="0">
     	...
     	<tr>
     		<td valign="top" valign="top">#if($editformSKR=="new" || $editformSKR=="yes") #if($mandatory=="yes")<font color="red">*</font>#else&nbsp;#end #else&nbsp;#end </td>
...
<!-- END [ tabbed 1 ] -->    
	...
	#foreach($memo in $maklumatRayuan)
		...
		#set($alasanMemo = $memo.alasan_rayuan)
	#end
#else
	#foreach($memo in $maklumatRayuan)
		#set($perkaraMemo = "$memo.perkara_rayu")
	#end
	#set($alasanMemo = "")
#end
...
function onclickKeputusanPegawai() {
	...
	document.${formName}.action = "?_portal_module=ekptg.view.ppk.FrmRynSemakPenerimaanSek17&cb1="+checkbox1+"&cb2="+checkbox2+"&cb3="+checkbox3;
...
*/	else if(document.${formName}.validateM.value == 0 && document.${formName}.socMahkamah.value == ""){
	...
	{ 
		...
		document.${formName}.action = "?_portal_module=ekptg.view.ppk.FrmRynSemakPenerimaanSek17";
...
function updateKeputusanMahkamah() {
	...
	else
	{
		...
		document.${formName}.action="?_portal_module=ekptg.view.ppk.FrmRynSemakPenerimaanSek17";
		document.${formName}.command.value="keputusanMahkamah";
		document.${formName}.command2.value="updateKeputusanMahkamah";
		document.${formName}.submit();
	}
}
...
function changeGetAlamatMahkamah2() {
	document.${formName}.action = "?_portal_module=ekptg.view.ppk.FrmRynSemakPenerimaanSek17";
	document.${formName}.command.value = "maklumatSerahanK1";
	document.${formName}.command3.value = "kemaskiniMaklumatSerahan";
	document.${formName}.command4.value = "changeGetAlamatMahkamah2";
	document.${formName}.submit();
}
...
function permohonanSenaraiSemak(id_permohonan,id_status) 
{
	document.${formName}.id_status.value = id_status;
	document.${formName}.id_permohonan.value = id_permohonan;
	document.${formName}.action = "?_portal_module=ekptg.view.ppk.FrmRynPermohonanRayuanSek17&command=semakRayuan";
...
function permohonanRayuan(id_permohonan,id_status) 
{
	...
	document.${formName}.action = "?_portal_module=ekptg.view.ppk.FrmRynPermohonanRayuanSek17&command=maklumatRayuan&tabId=0";
	document.${formName}.submit();
}
...
function cmdSimpan_flag(id_fail_carian,txtNoFailSub,id_permohonan_kebenaran,user_id_kebenaran_edit,flag_kebenaran_edit,catatan_batal)
{
	...
	document.${formName}.action="$EkptgUtil.getTabID("Utiliti",$portal_role)?_portal_module=ekptg.view.ppk.FrmBatalkanPermohonan";
	document.${formName}.submit();
