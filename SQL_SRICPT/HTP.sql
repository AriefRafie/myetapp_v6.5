...
-- 20201118 Audit trail emel
CREATE OR REPLACE TRIGGER TBLAUDITRAILMAIL_BI    
before insert on     TBLAUDITRAILMAIL    for each row
begin    if :new.ID_AUDITRAIL   is null    then SELECT (SELECT kod_negeri FROM tbllookupstate) || TO_CHAR (SYSDATE, 'YY') || TBLAUDITRAILMAIL_SEQ.NEXTVAL into :new.ID_AUDITRAIL from dual;    end if;    end;

CREATE SEQUENCE TBLAUDITRAILMAIL_SEQ
  START WITH 1
  MAXVALUE 999999999
  MINVALUE 1
  NOCYCLE
...
CREATE UNIQUE INDEX TBLAUDITRAIL_PK ON TBLAUDITRAILMAIL
(ID_AUDITRAIL) LOGGING NOPARALLEL;

ALTER TABLE TBLAUDITRAILMAIL ADD (
  CONSTRAINT TBLAUDITRAILMAIL_PK
 PRIMARY KEY
 (ID_AUDITRAIL));

CREATE TABLE TBLAUDITRAILMAIL(
  ID_AUDITRAIL     NUMBER,
  ID_RUJUKAN        NUMBER,
  MODULE_NAME       VARCHAR2(100 BYTE),
  JENIS_AKTIVITI    VARCHAR2(10 BYTE),
  TARIKH_AKTIVITI   TIMESTAMP(6),
...
--
ALTER TABLE TBLHTPHAKMILIK ADD CATATAN_PELEPASAN VARCHAR2(1000);
...
CREATE OR REPLACE TRIGGER TBLAPORANTANAH_BI    
before insert on     TBLAPORANTANAH    for each row
begin    if :new.ID_LAPORANTANAH    is null    then SELECT (SELECT kod_negeri FROM tbllookupstate) || TO_CHAR (SYSDATE, 'YY') || TBLAPORANTANAH_SEQ.NEXTVAL into :new.ID_LAPORANTANAH            from dual;    end if;    end;

CREATE SEQUENCE TBLAPORANTANAH_SEQ
  START WITH 1
  MAXVALUE 999999999
  MINVALUE 1
  NOCYCLE
...
ALTER TABLE TBLAPORANTANAH ADD (
