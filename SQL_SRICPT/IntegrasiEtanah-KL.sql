...
begin    if :new.ID_PERMOHONAN    is null    then SELECT (SELECT kod_negeri FROM tbllookupstate) || TO_CHAR (SYSDATE, 'YY') || INTANAHPERMOHONAN_SEQ.NEXTVAL into :new.ID_PERMOHONAN                from dual;    end if;    end;
/
...
--DROP TABLE INT_PPTPERMOHONAN CASCADE CONSTRAINTS;
	...
	CREATE TABLE TBLINTANAHPERMOHONAN(
	  	ID_PERMOHONAN         NUMBER CONSTRAINT INTANAH_PERMOHONAN_C01 NOT NULL,
	 	...
	 	KETERANGAN_TRANSAKSI  VARCHAR2(500 BYTE),
	  	...
	  	TARIKH_MASUK  DATE,
	  	TARIKH_KEMASKINI DATE,
	  	CONSTRAINT TBLINTANAHPERMOHONAN PRIMARY KEY (ID_PERMOHONAN)	
  	)
