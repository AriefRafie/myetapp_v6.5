...
public class ETanahSek8BorangC implements IntegrationInternal{
	...
	private String no_permohonan = null;
	//YATI 05112020
	MaklumatWartaborangB wartaSek4 = null;
	LampiranForm[] lampirans = null;
	LampiranForm lampiran = null;
	private static Logger myLog = Logger.getLogger(ETanahSek8BorangC.class);
	...
	@Override
	public void hantarBorangC(ETanahPPTManager pptManager, 
			String noPermohonan, 
		...
		try {
			...
			myLog.info("masuk sini manager :");
			//form = getPermohonan(permohonan_);
			//hakmiliks = getHakmilik();
			lampirans = getLampiran(vecDok); // MMK
			//lampiran = getLampiran1(vecDok); // MMK
			
			//String idPermohonan = getMaklumatInt();
			String no_permohonan = "0402ACQ2020000047";//"0402ACQ2020000045";//"0403ACQ2020000085";
			//this.context.put("no_permohonan",no_permohonan);
			//no_permohonan = getMaklumatInt(permohonan_.get("idPermohonan"));
			myLog.info("no_permohonan sek8 :"+no_permohonan);
			
			//setPermohonan(permohonan_,idPengguna,db);
			
			PopupeTanahData logic = new PopupeTanahData();
			lampiran = getLampiran1(logic.getSenaraiDokumenC(no_permohonan,JenisSkrin,db)); //LAMPIRAN LAIN
			//nopermohonan = get
			myLog.info("lampirans : "+lampirans);
			
			String response = "";
			//response = pptManager.permohonanSek8(form,hakmiliks,lampiran);
			response = pptManager.BorangC(no_permohonan, lampiran,lampirans);
			myLog.info("response sek8 borangc="+response);
...
//			ResponseForm response = RESTInvoker.hantarBorangD(idPermohonan, permohonan, cal, idPengguna);
	...
	/*private MaklumatPermohonanSek8Form getMaklumatInt(Hashtable<String, String> permohonan_) {
		// TODO Auto-generated method stub
		return null;
	}*/
	...
	private LampiranForm[] getLampiran(Vector<Tblrujdokumen> vecDok) {
		...
		for (int i = 0; i < vecDok.size(); i++) {
			...
			myLog.info("dokumen jenis Id :"+dokumen.getIdJenis());
			myLog.info("dokumen id Dokumen :"+dokumen.getIdDokumen());
			myLog.info("dokumen get Dokumen :"+dokumen.getDokumen());
	...
	private LampiranForm getLampiran1(Vector<Tblrujdokumen> vecDok) {
		...
		for (int i = 0; i < vecDok.size(); i++) {
			...
			lampiran.setKodDokumen(dokumen.getIdDokumen()); //rujukan
			//myLog.info(i+"."+cbsemaks[i]);
			myLog.info("dokumen iddokumen 1:"+dokumen.getIdDokumen());
			myLog.info("dokumen dokumen 1:"+dokumen.getDokumen());
			myLog.info("dokumen id jenis 1:"+dokumen.getIdJenis());
...
//yati 05112020
	public String getMaklumatInt(String idPermohonanIntegrasi) throws Exception {
				Db db = null;
				String sql = "";
				ResultSet rs = null;
				Statement stmt = null;
...
//					ResponseForm response = RESTInvoker.hantarBorangD(idPermohonan, permohonan, cal, idPengguna);
		...
		private MaklumatPermohonanSek8Form getPermohonan(Hashtable<String,String> permohonan) {
			...
			myLog.info("Maklumat Sek 4 Negeri="+permohonan.get("kodNegeriA"));
			return form;
			
		}

		@Override
		public void hantar(ETanahPPTManager pptManager, Hashtable<String, String> permohonan,
				Vector<Tblrujdokumen> vecDok, String idPengguna, Db db) {
			// TODO Auto-generated method stub
