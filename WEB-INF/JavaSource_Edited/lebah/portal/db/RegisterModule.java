...
public class RegisterModule {
	...
	public static boolean add(String module_id, String module_title, String module_class, String module_group, String module_description, String[] roles) throws DbException {
		...
		try { 
			...
			if ( module_group.equals("MOBILE")  )
			{
				sql = "DELETE FROM mobile_module WHERE module_id= '" + module_id + "'";
				stmt.executeUpdate(sql);
				
				for ( int x=0; x < roles.length; x++ ) {
					sql = "INSERT INTO mobile_module (role_id, module_id, module_title) VALUES ('" + roles[x] + "', '" + module_id + "', '" + module_title + "')";		
					stmt.executeUpdate(sql);
				}
			}
