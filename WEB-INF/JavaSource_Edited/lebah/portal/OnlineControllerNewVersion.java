...
public class OnlineControllerNewVersion extends VServlet
    implements Serializable
{

	static Logger myLogger = Logger.getLogger(OnlineControllerNewVersion.class);
	
    public OnlineControllerNewVersion()
    {
        loginIntercept = null;
    }
    ...
    public synchronized void doBodyBackupRazman(HttpServletRequest req, HttpServletResponse res)
        throws Exception
    {
...
//            res.sendRedirect("../logout_online.jsp");
    ...
    //razman revamp onlineController
    public void doBody(HttpServletRequest req, HttpServletResponse res)
            throws Exception
        {
...
//                res.sendRedirect("../logout_online.jsp");
		...
		if(session.getAttribute("_dc_portletInfo")!=null)
		{
			...
			content = (Object)session.getAttribute("_dc_content");
			System.out.println("DC content : "+content);			
			if(content!=null)
			{
				DisplayContentNewVersion.printContent(content, svtCfg, req, res, out, portletInfo);			
			}			 
		}
...
//                DisplayContent.showNavigationType(engine, context, getServletConfig(), req, res, module, out, session);
                ...
                if ( "left_navigation".equals(display_type) )
					DisplayContentNewVersion.showNavigationType(engine, context, getServletConfig(), req, res, module, out, session);
                else if ( "top_navigation".equals(display_type) )
                    DisplayContentNewVersion.showTopNavigationType(engine, context, getServletConfig(), req, res, module, out, session);
                else if ( "top_navigation_with_title".equals(display_type) )
                    DisplayContentNewVersion.showTopNavigationTitleType(engine, context, getServletConfig(), req, res, module, out, session);
                else if ( "two_columns_with_bottom".equals(display_type) )
                    DisplayContentNewVersion.showTwoColumnsWithSingleBottom(engine, context, getServletConfig(), req, res, module, out, session);
                else if ( "three_columns_with_top".equals(display_type) )
                    DisplayContentNewVersion.showThreeColumnsWithSingleTop(engine, context, getServletConfig(), req, res, module, out, session);
                else if ( "three_columns_with_bottom".equals(display_type) )
                    DisplayContentNewVersion.showThreeColumnsWithSingleBottom(engine, context, getServletConfig(), req, res, module, out, session);
				else if ( "narrow_wide".equals(display_type) )
					DisplayContentNewVersion.showNarrowWideType(engine, context, getServletConfig(), req, res, module, out, session);
				else if ( "wide_narrow".equals(display_type) )
					DisplayContentNewVersion.showWideNarrowType(engine, context, getServletConfig(), req, res, module, out, session);
				else if ( "two_columns".equals(display_type) )
					DisplayContentNewVersion.showTwoColumnsType(engine, context, getServletConfig(), req, res, module, out, session);
				else if ( "three_columns".equals(display_type) )
					DisplayContentNewVersion.showThreeColumnsType(engine, context, getServletConfig(), req, res, module, out, session);
				else if ( "pulldown_menu".equals(display_type) )
					DisplayContentNewVersion.showPullDownMenuType(engine, context, getServletConfig(), req, res, module, out, session);
				else if ( "windows".equals(display_type) )
					DisplayContentNewVersion.showWindowsType(engine, context, getServletConfig(), req, res, module, out, session);				
				else
					DisplayContentNewVersion.showNavigationType(engine, context, getServletConfig(), req, res, module, out, session);
                out.println("</div>");
            }
