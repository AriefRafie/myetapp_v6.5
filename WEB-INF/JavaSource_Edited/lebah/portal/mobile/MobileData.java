...
public class MobileData {
	...
	public static Vector getModules(String role) throws Exception {
		...
		try {
			db = new Db();
			SQLRenderer r = new SQLRenderer();
//			String sql = "SELECT a.module_id, a.module_title as title, m.module_title, m.module_class, m.module_description FROM mobile_module a, module m WHERE a.role_id = " + role + " AND a.module_id = m.module_id ORDER BY m.module_description ASC";	
			String sql = r
			.add("a.module_id")
			.add("a.module_title as title")
			.add("m.module_title")
			.add("m.module_class")
			.add("m.module_description")
			.add("a.role_id", role)
