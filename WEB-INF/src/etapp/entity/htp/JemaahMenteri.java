package etapp.entity.htp;

import java.util.Date;

import javax.persistence.Column;
import javax.persistence.Entity;
import javax.persistence.Id;
import javax.persistence.JoinColumn;
import javax.persistence.ManyToOne;
import javax.persistence.Table;
import javax.persistence.Temporal;
import javax.persistence.TemporalType;

import etapp.entity.Permohonan;

@Entity
@Table(name = "TBLHTPJEMAAHMENTERI")
public class JemaahMenteri {
	@Id @Column(name="ID_JEMAAHMENTERI")
	private long id;
	@ManyToOne @JoinColumn(name="ID_PERMOHONAN")
	private Permohonan permohonan;
	
	@Temporal(TemporalType.TIMESTAMP) @Column(name="TARIKH_HANTAR_KSU")
	private Date tarikhHantarKSU;
	@Temporal(TemporalType.TIMESTAMP) @Column(name="TARIKH_TERIMA_KSU")
	private Date tarikhTerimaKSU;
	@Temporal(TemporalType.TIMESTAMP) @Column(name="TARIKH_TERIMA")
	private Date tarikhTerima;
	@Temporal(TemporalType.TIMESTAMP) @Column(name="TARIKH_HANTAR_PEMOHON")
	private Date tarikhHantarPemohon;	
	@Temporal(TemporalType.TIMESTAMP) @Column(name="TARIKH_MEMORANDUM")
	private Date tarikhMemorandun;
	@Temporal(TemporalType.TIMESTAMP) @Column(name="TARIKH_KEPUTUSAN")
	private Date tarikhKeputusan;
	@Column(name="NO_RUJUKAN_KPTSN")
	private String noRujukanKeputusan;
	@Column(name="ULASAN")
	private String ulasan;
	@Column(name="STATUS_KEPUTUSAN")
	private String statusKeputusan;
	@Column(name="TINDAKAN_LANJUT")
	private String tindakanLanjut;
	@Temporal(TemporalType.TIMESTAMP) @Column(name="TARIKH_HANTAR_DASAR")
	private Date tarikhHantarDasar;	
	@Column(name="NO_MEMORANDUM")
	private String noMemorandum;
	@Temporal(TemporalType.TIMESTAMP) @Column(name="TARIKH_MSYRT_JEMAAH")
	private Date tarikhMesyuarat;	
	@Temporal(TemporalType.TIMESTAMP) @Column(name="TARIKH_KUASA_PBDN")
	private Date tarikhDiperbadan;	
		
	@Column(name="ID_MASUK")
	private Long idMasuk;
	@Temporal(TemporalType.TIMESTAMP) @Column(name="TARIKH_MASUK")
	private Date tarikhMasuk;
	@Column(name="ID_KEMASKINI")
	private Long idKemaskini;
	@Temporal(TemporalType.TIMESTAMP) @Column(name="TARIKH_KEMASKINI")
	private Date tarikhKemaskini;
	//
	public long getId() {
		return id;
	}
	public void setId(long id) {
		this.id = id;
	}
	public Permohonan getPermohonan() {
		return permohonan;
	}
	public void setPermohonan(Permohonan permohonan) {
		this.permohonan = permohonan;
	}
	public Date getTarikhHantarKSU() {
		return tarikhHantarKSU;
	}
	public void setTarikhHantarKSU(Date tarikhHantarKSU) {
		this.tarikhHantarKSU = tarikhHantarKSU;
	}
	public Date getTarikhTerimaKSU() {
		return tarikhTerimaKSU;
	}
	public void setTarikhTerimaKSU(Date tarikhTerimaKSU) {
		this.tarikhTerimaKSU = tarikhTerimaKSU;
	}
	public Date getTarikhTerima() {
		return tarikhTerima;
	}
	public void setTarikhTerima(Date tarikhTerima) {
		this.tarikhTerima = tarikhTerima;
	}
	public Date getTarikhHantarPemohon() {
		return tarikhHantarPemohon;
	}
	public void setTarikhHantarPemohon(Date tarikhHantarPemohon) {
		this.tarikhHantarPemohon = tarikhHantarPemohon;
	}
	public Date getTarikhMemorandun() {
		return tarikhMemorandun;
	}
	public void setTarikhMemorandun(Date tarikhMemorandun) {
		this.tarikhMemorandun = tarikhMemorandun;
	}
	public Date getTarikhKeputusan() {
		return tarikhKeputusan;
	}
	public void setTarikhKeputusan(Date tarikhKeputusan) {
		this.tarikhKeputusan = tarikhKeputusan;
	}
	public String getNoRujukanKeputusan() {
		return noRujukanKeputusan;
	}
	public void setNoRujukanKeputusan(String noRujukanKeputusan) {
		this.noRujukanKeputusan = noRujukanKeputusan;
	}
	public String getUlasan() {
		return ulasan;
	}
	public void setUlasan(String ulasan) {
		this.ulasan = ulasan;
	}
	public String getStatusKeputusan() {
		return statusKeputusan;
	}
	public void setStatusKeputusan(String statusKeputusan) {
		this.statusKeputusan = statusKeputusan;
	}
	public String getTindakanLanjut() {
		return tindakanLanjut;
	}
	public void setTindakanLanjut(String tindakanLanjut) {
		this.tindakanLanjut = tindakanLanjut;
	}
	public Date getTarikhHantarDasar() {
		return tarikhHantarDasar;
	}
	public void setTarikhHantarDasar(Date tarikhHantarDasar) {
		this.tarikhHantarDasar = tarikhHantarDasar;
	}
	public String getNoMemorandum() {
		return noMemorandum;
	}
	public void setNoMemorandum(String noMemorandum) {
		this.noMemorandum = noMemorandum;
	}
	public Date getTarikhMesyuarat() {
		return tarikhMesyuarat;
	}
	public void setTarikhMesyuarat(Date tarikhMesyuarat) {
		this.tarikhMesyuarat = tarikhMesyuarat;
	}
	public Date getTarikhDiperbadan() {
		return tarikhDiperbadan;
	}
	public void setTarikhDiperbadan(Date tarikhDiperbadan) {
		this.tarikhDiperbadan = tarikhDiperbadan;
	}
	public Long getIdMasuk() {
		return idMasuk;
	}
	public void setIdMasuk(Long idMasuk) {
		this.idMasuk = idMasuk;
	}
	public Date getTarikhMasuk() {
		return tarikhMasuk;
	}
	public void setTarikhMasuk(Date tarikhMasuk) {
		this.tarikhMasuk = tarikhMasuk;
	}
	public Long getIdKemaskini() {
		return idKemaskini;
	}
	public void setIdKemaskini(Long idKemaskini) {
		this.idKemaskini = idKemaskini;
	}
	public Date getTarikhKemaskini() {
		return tarikhKemaskini;
	}
	public void setTarikhKemaskini(Date tarikhKemaskini) {
		this.tarikhKemaskini = tarikhKemaskini;
	}

	
	
}
