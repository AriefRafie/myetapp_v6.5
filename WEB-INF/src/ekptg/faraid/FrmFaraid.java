...
public class FrmFaraid extends AjaxBasedModule{
	 ...
	 * 
	 */
	private static final long serialVersionUID = -1332866461778094705L;
	static Logger myLog = Logger.getLogger(FrmFaraid.class);
	...
	public String doTemplate2() throws Exception {
		...
		ekptg.helpers.Utils uti = new ekptg.helpers.Utils();
		...
		if (id_permohonan == null || "".equals(id_permohonan)) { //Tiada id_permohonan,assuming from searching.
			...
			if ("doCarian".equals(submit) || 
				...
				this.context.put("isMuslim", uti.isSimatiNonMuslim(id_simati));

				//myLogger.info(txtNoFail);
				Vector<Hashtable<String,String>> v = this.getFailList(txtNoFail, user_id);
...
//			vm = "app/ppk/frmFaraidDev.jsp"; //2016-2018
			...
			if ("".equals(submit)) {
				...
				Vector<Waris> hasilFaraid = new Vector<Waris>();
				...
				Waris objInfo;
				Waris newObj;
				myLog.info("Waris Count:" + warisList.size());
				...
				for(int x=0;x<warisList.size();x++) {
					...
					myLog.info("All Waris:" + waris.getID()+ ' ' + waris.getNama());
				}
	...
	public Waris getResultForWaris(String warisId, Waris waris, Vector<Waris> faraidList) {
		Waris objInfo;
		//myLogger.info("Waris Count:" + faraidList.size());
		
		for(int x=0;x<faraidList.size();x++) {
			objInfo = (Waris)faraidList.get(x);
			myLog.info("test:" + objInfo.getID()+ ' ' + objInfo.getNama());
			...
			if (waris.getNama().toUpperCase().equals("BAITULMAL")) {
				if (objInfo.getNama().toUpperCase().equals("BAITULMAL")) {
				 ...
				 myLog.info("Update Faraid Baitulmal:" +  waris.getID()+ ' ' + waris.getBahagian());
				 return waris;
				}
			} else {
				 if (warisId.equals(objInfo.getID())) {
					 ...
					 waris.setBahagianBawah(objInfo.getBahagianBawah());
					 waris.setHubungan(objInfo.getHubungan());
					 myLog.info("Update Faraid:" +  waris.getID()+ ' ' + waris.getBahagian());
					 return waris;
				 }
	...
	public Vector<Hashtable<String,String>> getFailList(String no_fail,String user_id) throws Exception {
		Db db = null;
