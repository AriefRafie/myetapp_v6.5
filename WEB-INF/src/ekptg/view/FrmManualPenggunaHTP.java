...
public class FrmManualPenggunaHTP extends AjaxBasedModule {
	/**
	 * 
	 */
	private static final long serialVersionUID = -9012102407918820290L;
	private final String PATH="app/";
    private static Logger myLog = Logger.getLogger(ekptg.view.FrmManualPenggunaHTP.class);
    ...
    public String doTemplate2() throws Exception {
		...
		String vm = PATH+"frmManualPenggunaHTP.jsp";
        return vm;
	...
	private String setContext(Vector<Tab> tabs,String manualPath){
		...
		try {
			...
			for (int i = 0; i < tabs.size(); i++) {
				tab = (Tab)tabs.get(i);
				String namaFail = "MyeTaPP_UM_"+tab.getTitle().replace("/", "")+"_v6.pdf";
				//myLog.info(namaFail);
				sb.append("<tr>");
				sb.append("	<td>&nbsp;</td>");
				sb.append("	<td>");
				...
				sb.append("			<b>Manual Pengguna Sub Modul "+getNama(tab.getTitle())+"</b>");
				sb.append("		</a>");				
				sb.append("	</td>");				
				sb.append("</tr>");
		...
		try {
			...
			while (rs.next()) {
				...
				if(
						...
						!rs.getString("tab_title").equals("<i>Dashboard</i>")  
						&& !rs.getString("tab_title").equals("'My Info'")
						&& !rs.getString("tab_title").equals("<i>Online</i>")
