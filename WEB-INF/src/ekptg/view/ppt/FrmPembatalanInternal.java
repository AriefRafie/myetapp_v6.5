...
public class FrmPembatalanInternal extends AjaxBasedModule{	
	...
	PPTHeader headerppt = new PPTHeader();
	...
	public String doTemplate2() throws Exception{
		...
		headerppt.setDataHeader(idpermohonan);
		Vector dataHeader = headerppt.getDataHeader();
		...
		list_sub = headerppt.listPerjalananFail(idpermohonan);
		...
		if ("senarai".equals(main_command)){
			if ("kembali_senarai_depan".equals(sub_command)){
				...
				senarai_pembatalan = logic.senarai_pembatalan(id_permohonan);
				this.context.put("senarai_pembatalan", senarai_pembatalan);		
				vm = "app/ppt/frmPenarikanBalikSenarai.jsp";	
	    	}
			
			jenis_button = "1";
			this.context.put("tajuk_header","PEMBATALAN PENGAMBILAN TANAH");
		...
		/*	maklumat_keputusan = logic.maklumat_keputusan(getParam("id_permohonan"));
			...
			if ("kembali_senarai_depan".equals(sub_command)){
				...
				senarai_hakmilik = logic.senarai_hakmilik_pembatalan(id_permohonan,getParam("id_pembatalan"));
				this.context.put("senarai_hakmilik",senarai_hakmilik);
				senarai_hakmilik_batal = logic.senarai_hakmilik_batal_pembatalan(getParam("id_pembatalan"));
				...
				maklumat_pembatalan = logic.maklumat_pembatalan(getParam("id_pembatalan"));
				this.context.put("maklumat_pembatalan", maklumat_pembatalan);
				senarai_hakmilik = logic.senarai_hakmilik_pembatalan(getParam("id_permohonan"),getParam("id_pembatalan"));
				this.context.put("senarai_hakmilik",senarai_hakmilik);
				senarai_hakmilik_batal = logic.senarai_hakmilik_batal_pembatalan(getParam("id_pembatalan"));
	    		this.context.put("senarai_hakmilik_batal",senarai_hakmilik_batal);
				this.context.put("hakmilik_belumbatal",senarai_hakmilik.size());
				...
				updatePembatalanOnline3Tolak(session);
				this.context.put("readmode", "view");
				maklumat_pembatalan = logic.maklumat_pembatalan(getParam("id_pembatalan"));
				this.context.put("maklumat_pembatalan", maklumat_pembatalan);
				senarai_hakmilik = logic.senarai_hakmilik_pembatalan(getParam("id_permohonan"),getParam("id_pembatalan"));
				...
				for (int t = 0; t < id_lot_tarik.length; t++) 
				{
						...
						if (bolehsimpan.equals("yes")) 
						{
						logic.hapus_pilih_pembatalan(id_lot_tarik[t]);
						}									
												
				}
				}
				logic.deletePembatalanInternal(getParam("id_pembatalan"));
				maklumat_pembatalan = logic.maklumat_pembatalan(getParam("id_pembatalan"));
				this.context.put("maklumat_pembatalan", maklumat_pembatalan);
				senarai_hakmilik = logic.senarai_hakmilik_pembatalan(getParam("id_permohonan"),getParam("id_pembatalan"));
				senarai_hakmilik_batal = logic.senarai_hakmilik_batal_pembatalan(getParam("id_pembatalan"));
				...
				if (ids != null && id_lot_tarik != null ) 
				{
					...
					{					
							...
							for (int t = 0; t < id_lot_tarik.length; t++) 
							{
								myLogger.info("I:"+ids[i]+";T:"+id_lot_tarik[t]);	
								if(id_lot_tarik[t].equals(ids[i]))
								{
									if (bolehsimpan.equals("yes")) 
									{
									logic.pilih_pembatalan(id_kat[t],id_lot_tarik[t],lot_tarik[t],lot_ambil[t],luas_lot[t],(String) session.getAttribute("_ekptg_user_id"),id_pembatalan);
									}									
								}								
							}
				...
				{
					maklumat_pembatalan = logic.maklumat_pembatalan(id_pembatalan);
					this.context.put("maklumat_pembatalan", maklumat_pembatalan);
					senarai_hakmilik = logic.senarai_hakmilik_pembatalan(getParam("id_permohonan"),id_pembatalan);
					this.context.put("senarai_hakmilik",senarai_hakmilik);
					this.context.put("hakmilik_belumbatal",senarai_hakmilik.size());
					senarai_hakmilik_batal = logic.senarai_hakmilik_batal_pembatalan(id_pembatalan);
		    		this.context.put("senarai_hakmilik_batal",senarai_hakmilik_batal);
					vm = "app/ppt/frmPenarikanInfoSenaraiLot.jsp";	
				}
	    	}
				...
				if (ids != null) {
					for (int i = 0; i < ids.length; i++) {						
							if (bolehsimpan.equals("yes")) 
							{
								...
								{
								logic.batalkan_pembatalan(ids[i],lot_tarik[i],(String) session.getAttribute("_ekptg_user_id"),getParam("id_pembatalan"));
								}
			...
			//String id_pembatalan = getParam("id_pembatalan");			
     		//listDokumen = logic.senarai_dokumen_pembatalan(id_pembatalan);
		...
		/*	maklumat_keputusan = logic.maklumat_keputusan(getParam("id_pembatalan"));
			...
			if ("kembali_senarai_depan".equals(sub_command)){
				...
				for (int t = 0; t < id_lot_tarik.length; t++) 
				{
						...
						if (bolehsimpan.equals("yes")) 
						{
						logic.update_pilih_pembatalan(id_kat[t],id_lot_tarik[t],id_hakmilik_luas[t],lot_tarik[t],lot_ambil[t],lot_ambil_asal[t],luas_lot[t],(String) session.getAttribute("_ekptg_user_id"),getParam("id_pembatalan"));
						}									
				...
				if (ids != null ) {
					for (int i = 0; i < ids.length; i++) {
							...
							if (bolehsimpan.equals("yes")) 
							{
								if (bolehsimpan.equals("yes")) 
								{
								logic.hapus_pilih_pembatalan(ids[i]);
								}
							}
						}
					}
	...
	/*		maklumat_keputusan = logic.maklumat_keputusan(getParam("id_permohonan"));
    		...
    		context.put("screen_name",getParam("screen_name"));
    		String id_permohonan = getParam("id_permohonan");
			maklumat_tambahan = logic.maklumat_tambahan_pembatalan(id_permohonan);
		...
		{	
				...
				listdepan = logic.senarai_pembatalan_carian((String) session.getAttribute("_ekptg_user_negeri"),txtNoFail,txtNoRujJkptgNegeri,socKementerian,socUrusan,socStatus,"2",(String) session.getAttribute("_portal_role"),(String) session.getAttribute("_ekptg_user_negeri"));						
				this.context.put("listdepan",listdepan);
	...
	private void uploadFiles() throws Exception {
	 ...
	 private void saveData(FileItem item) throws Exception {
	        ...
	        try {
	        	...
	        	PreparedStatement ps = con.prepareStatement("insert into TBLPPTDOKUMEN " +
	        			"(id_Dokumen,id_pembatalan,nama_Fail,jenis_Mime,content,tajuk,keterangan,id_masuk,id_kemaskini) " +
	        			"values(?,?,?,?,?,?,?,?,?)");
	 ...
	 private void updateStatusPenarikan(HttpSession session) throws Exception {
			...
			logic.updateStatusPembatalan(h);	 	
	 }
	 ...
	 private void updatePembatalanOnline3(HttpSession session,String id_permohonan) throws Exception {
			Hashtable h = new Hashtable();
			h.put("id_pembatalan", getParam("id_pembatalan"));
			h.put("id_permohonan", id_permohonan);
			h.put("id_Masuk", (String) session.getAttribute("_ekptg_user_id"));
			logic.updatePembatalanOnline3(h);
		}
	 
	 private void updatePembatalanOnline3Tolak(HttpSession session) throws Exception {
			Hashtable h = new Hashtable();
			h.put("id_pembatalan", getParam("id_pembatalan"));
			h.put("id_Masuk", (String) session.getAttribute("_ekptg_user_id"));
			logic.updatePembatalanOnline3Tolak(h);
