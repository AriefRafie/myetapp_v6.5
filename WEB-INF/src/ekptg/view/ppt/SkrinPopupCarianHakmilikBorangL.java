...
public class SkrinPopupCarianHakmilikBorangL extends AjaxBasedModule {
	static Logger myLogger = Logger.getLogger(SkrinPopupCarianHakmilikBorangL.class);
	private static SimpleDateFormat Format =  new SimpleDateFormat("dd/MM/yyyy");
	private final String PATH="app/ppt/PopupHakmilik/";
	private String vm = PATH +"SkrinPopupCarianHakmilikBorangL.jsp";
	...
	public String doTemplate2() throws Exception {		
		...
		db = new Db();
				...
				if("simpanSj".equals(command)){ 
	            	context.put("showSJ", "no");         	
	            		simpanSj(session,id_permohonan,db,getHakmilik(id_permohonan,flag_skrin,getParam("NO_LOT"),getParam("NAMA_PB"),getParam("NO_PB"),db,id_pegawai,id_borange));
	            		context.put("refreshHakmilik", "yes");
	        	}
				...
				displayHakmilik(id_permohonan,flag_skrin,action,getParam("NO_LOT"),getParam("NAMA_PB"),getParam("NO_PB"),db,id_pegawai,id_borange);
...
public static void simpanBorangL(String idUser,String id_hakmilik,long id_borangl, String dateBorangL, String statusBL,String tempoh,Db db) throws Exception{
	...
	public static void updateBorangL(String idUser,String id_hakmilik,String id_borangl, String dateBorangL, String statusBL,String tempoh,Db db) throws Exception{
	...
	private void simpanBorangL(HttpSession session,String idpermohonan,Db db) throws Exception{
		...
		if(cb_id_hakmilik!=null){
			for (int i = 0; i < cb_id_hakmilik.length; i++) { 
				...
				myLogger.info("id_borangl :"+id_borangl);
				myLogger.info("tempoh :"+tempoh);
		
				if(id_borangl.equals("")){
					long id_borangl_seq = DB.getNextID("TBLPPTBORANGL_SEQ");
					simpanBorangL(idUser,cb_id_hakmilik[i],id_borangl_seq,dateBorangL,socStatusBL,tempoh,db);
				}else{
					updateBorangL(idUser,cb_id_hakmilik[i],id_borangl,dateBorangL,socStatusBL,tempoh,db);
				}
	...
	private void displayHakmilik(String id_permohonan,String flag_skrin,String action,String no_lot, String nama_pb,String no_pb,Db db,String id_pegawai,String id_borange) throws Exception{
		List<Hashtable>  list = null;	
	...
	*/
	...
	public Vector getHakmilik(String id_permohonan,String flag_skrin,String no_lot, String nama_pb,String no_pb,Db db,String idpegawai,String id_borange) throws Exception {
		//temp
		...
		//	db = new Db();
			...
			sql = " " +
					...
					if (!id_permohonan.equals("") && id_permohonan != null && flag_skrin.equals("bantahan_mahkamah")) 
					{				
					...
					if (flag_skrin.equals("senarai_siasatan") || flag_skrin.equals("senarai_siasatan_sementara")) 
					{
						sql += " (SELECT MAX(SS.ID_SIASATAN) FROM TBLPPTSIASATAN SS WHERE M.ID_HAKMILIK = SS.ID_HAKMILIK) AS ID_SIASATAN, "; 
					}
					else
					{
						sql += " ''  AS ID_SIASATAN,"	;
					...
					sql += " N.NAMA_NEGERI,  (SELECT COUNT(A.ID_HAKMILIKPB) FROM TBLPPTHAKMILIKPB A, TBLPPTPIHAKBERKEPENTINGAN B  WHERE A.ID_PIHAKBERKEPENTINGAN = B.ID_PIHAKBERKEPENTINGAN AND A.ID_HAKMILIK(+) = M.ID_HAKMILIK  "+
...
//					sql += " AND (select count(*)from Tblpptborangk a, Tblppthakmilikborangk b where a.id_borangk = b.id_borangk and b.id_hakmilik(+) = m.id_hakmilik) > '0' ";
					...
					"AND M.ID_JENISHAKMILIK = JH.ID_JENISHAKMILIK(+) AND M.ID_LOT = LT.ID_LOT(+)  " +
					...
					if (flag_skrin.equals("hakmilik_pampasan") || flag_skrin.equals("senarai_pampasan_sementara")) 
					{
					sql += " AND SS.id_hakmilik = M.id_hakmilik AND SS.id_siasatan = (SELECT MAX(SS.ID_SIASATAN) FROM TBLPPTSIASATAN SS WHERE M.ID_HAKMILIK = SS.ID_HAKMILIK) "; 
					...
					/* 
					 ...
					 if (id_borange != "" && id_borange != null) {
					     if (!id_borange.equals("")) { 				 
					 	sql += " AND M.ID_HAKMILIK IN ( ";
						sql +=  "SELECT M.ID_HAKMILIK  ";               
			    		sql +=  "FROM TBLPPTBORANGEHAKMILIK A, TBLPPTBORANGE B, TBLPPTHAKMILIK M ";  
