...
public class SkrinPopupNotisSecaraPukal extends AjaxBasedModule {
	static Logger myLogger = Logger.getLogger(SkrinPopupNotisSecaraPukal.class);
	private static SimpleDateFormat Format =  new SimpleDateFormat("dd/MM/yyyy");
	private final String PATH="app/ppt/PopupHakmilik/";
	private String vm = PATH +"SkrinPopupCarianPB_HantarNotisPukal.jsp";
	...
	public String doTemplate2() throws Exception {		
		...
		this.context.put("berjayasimpan","");
		...
		if(flag_skrin.equals("skrin_kemasukan_notis_secara_pukal"))
		{
			flagJenisBukti = "1";	
			//vm = PATH +"SkrinPopupCarianPB_HantarNotisPukal.jsp";
		}
		if(flag_skrin.equals("skrin_kemasukan_notisK_secara_pukal"))
		{
		flagJenisBukti = "3";
			//vm = PATH +"SkrinPopupCarianPB_HantarNotisPukal.jsp";
		}
		
		if(flag_skrin.equals("skrin_kemasukan_notis_secara_pukal") || flag_skrin.equals("skrin_kemasukan_notisK_secara_pukal")  || flag_skrin.equals("skrin_kemasukan_notisH_secara_pukal"))
		{
			...
			listPenghantarNotis(id_hakmilik,db);
			context.put("listPenghantarNotis", listPenghantarNotis(id_hakmilik,db));
			...
			{			
	        		...
	        		if (checkN != null) {	        			
	        			for (int i = 0; i < checkN.length; i++) {	
	        				...
	        				//String id_hakmilikpbH = checkN[i] +"id_hakmilikpb";
	        				...
	        				//h.put("id_hakmilikpb", getParam(id_hakmilikpbH));
	        				...
	        				else if(flag_skrin.equals("skrin_kemasukan_notisH_secara_pukal"))
	        				{
	        					myLogger.info(" checkN :::::: "+checkN[i]);
		        				...
		        				h.put("txtCatatan", getParam(txtCatatan));
		        				h.put("id_user",id_user);
		        				h.put("id_borangg",id_borangg);		        				
		        				h.put("id_hakmilikpb", getParam(id_hakmilikpbH));	        						        					        				
		        				
		        				hapusBuktiPenyampaian(id_pihakberkepentingan, flagJenisBukti,db);
		        				hapusBorangH(id_borangg,getParam(id_hakmilikpbH),db);
		        				//long id_borangh = getNextID("TBLPPTBORANGH",db);	
		        				simpanPenerimaanBorangH(h,db);
	        				}
	        				this.context.put("berjayasimpan","yes");
	        			...
	        			else if(flag_skrin.equals("skrin_kemasukan_notisH_secara_pukal"))
        				{
	            			if(id_status.equals("62") || id_status.equals("74")){
	            				updateStatus(id_permohonan,id_status,id_user,db);
	            				updateSuburusanStatusFailPPT(id_user,id_fail,id_permohonan,id_suburusanstatusfailppt,"1483",db);
	            			} 
        				}
	        		 }	
	...
	public Vector getPB(String id_hakmilik,String flag_skrin,String nama_pb,String no_pb,Db db,String flagJenisBukti,String id_borangg) throws Exception {
		...
		try {
			...
			else if(flag_skrin.equals("skrin_kemasukan_notis_secara_pukal") || flag_skrin.equals("skrin_kemasukan_notisK_secara_pukal") || flag_skrin.equals("skrin_kemasukan_notisH_secara_pukal"))
			{			
				...
				/*	", "
					...
					sql += " FROM TBLPPTHAKMILIK HM,TBLPPTPERMOHONAN PBALIK,";
					
					if(flag_skrin.equals("skrin_kemasukan_notisH_secara_pukal"))
					{
						sql += " TBLPPTAWARD AW, ";
					}
					
					
			sql += 		"TBLPPTHAKMILIKPB HMPB, ";
		sql += "TBLPPTPIHAKBERKEPENTINGAN PB,TBLRUJJENISPB JPB,TBLRUJNEGERI N,TBLRUJBANDAR B, "
					...
					if(flag_skrin.equals("skrin_kemasukan_notisH_secara_pukal"))
					{
						sql += " AND AW.id_hakmilikpb = HMPB.id_hakmilikpb";
					}
					sql += " AND PNGAMANAH.ID_PIHAKBERKEPENTINGAN(+) = HMPB.ID_PA1 "
					...
					sql += "(CASE "+
							...
							"  END "+
							"  ) ASC,";*/
							sql +=	"(CASE WHEN ID_JENISPB = 1 THEN 1 "+
			...
			while (rs.next()) {
				...
				else if(flag_skrin.equals("skrin_kemasukan_notis_secara_pukal") || flag_skrin.equals("skrin_kemasukan_notisK_secara_pukal")  || flag_skrin.equals("skrin_kemasukan_notisH_secara_pukal"))
				{
					...
					*/
					...
					h.put("TEMPAT_TAMPAL","");
					h.put("JENIS_SERAHAN","");
					h.put("FLAG_JENIS_BUKTI","");
					h.put("FLAG_SERAH","");
					h.put("NAMA_PENERIMA", "");
					h.put("NO_KP_PENERIMA", "");
					h.put("CATATAN_BUKTI", "");
					h.put("ID_JENISNOPB_BUKTI","");
					h.put("HUBUNGAN","");
	 ...
	 Hashtable getPermohonan = null;
		...
		@SuppressWarnings("unchecked")
		public static void simpanPenerimaanBorangH(Hashtable data,Db db) throws Exception
		  {
		  ...
		  // Db db = null;
		    ...
		    try{
		    		...
		    		myLogger.info("INSERT BORANG H :"+sql);
		    		stmt.executeUpdate(sql);
		...
		public static void hapusBorangH(String id_borangg, String  id_hakmilikpb,Db db) throws Exception {
			   
		    String sql = "";		   
		    try{
		    		Statement stmt = db.getStatement();				  
		    		sql = "DELETE FROM tblpptborangh where id_borangg = '"+id_borangg+"' and id_hakmilikpb = '"+id_hakmilikpb+"' ";
		    		myLogger.info("DELETE BORANG H :"+sql);
		    		stmt.executeUpdate(sql);
		    }
		    finally {		   
		    }
		...
		public Vector listPenghantarNotis(String id_hakmilik,Db db) throws Exception {
			//Db db = null;
			try {
			//	db = new Db();
				Statement stmt = db.getStatement();
				String sql = " 	SELECT DISTINCT U.USER_ID AS ID_PEGAWAI, U.USER_NAME AS NAMA_PEGAWAI FROM USERS U, USERS_INTERNAL UI "+
						...
						" 			AND UI.ID_NEGERI  IN (SELECT ID_NEGERI FROM TBLPPTHAKMILIK WHERE ID_HAKMILIK = '"+id_hakmilik+"') "+
