...
public class SkrinPopupCarianHakmilik extends AjaxBasedModule {
	...
	private void displayHakmilik(String id_permohonan, String flag_skrin,
		...
		List<Hashtable> listKJP = null;
		list = getHakmilik(id_permohonan, flag_skrin, no_lot, nama_pb, no_pb,
				db, id_pegawai, id_borange);
		listKJP = getHakmilikKJP(id_permohonan, flag_skrin, no_lot, nama_pb, no_pb,
				db, id_pegawai, id_borange);
		context.put("SenaraiFail", list);
		context.put("SenaraiFailKJP", listKJP);
	...
	//shiqa gethakmilikKJP
	Vector list_hakmilikKJP = null;
	public Vector getHakmilikKJP(String id_permohonan, String flag_skrin,
		String no_lot, String nama_pb, String no_pb, Db db,
		String idpegawai, String id_borange) throws Exception {
		// temp
		...
		String listLOTHM = "";
		double totalSize = 0;
		list_hakmilikKJP = new Vector();
		list_hakmilikKJP.clear();
		// Db db = null;
		String sql = "";
		...
		try {
			...
			myLogger.info("LIST HAKMILIK KJP :" + sql.toUpperCase());
			...
			while (rs.next()) {
				...
				list_hakmilikKJP.addElement(h);
			}
			return list_hakmilikKJP;
		} finally {
			// if (db != null)
			// db.close();
		}
