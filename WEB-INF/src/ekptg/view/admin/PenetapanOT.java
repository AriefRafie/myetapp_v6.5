...
public class PenetapanOT extends AjaxBasedModule {
	static Logger myLogger = Logger.getLogger(PenetapanOT.class);
	String skrin_name = "app/admin/OT/OTindex.jsp";
	...
	public String doTemplate2() throws Exception {
		...
		else if(command.equals("showSenaraiPMT") || command.equals("deletePMT") || command.equals("insertPMT"))
		{			
			String ID_MINITTRANSAKSI = getParam("ID_MINITTRANSAKSI");
			if(command.equals("deletePMT"))
			{
				//function delete
				deletePMT(session,ID_MINITTRANSAKSI);
			}
			else if(command.equals("insertPMT"))
			{
				//function insert
				saveTablePMT(session,"");
			}
			...
			String action = getParam("action");
			listPMT = listPMT(session);
			setupPageList(session, action, listPMT, "listPMT",command,"div_SenaraiPMT");
			
			skrin_name = "app/admin/OT/OTSenaraiPMT.jsp";
		}
		...
		else if(command.equals("addPMT") || command.equals("editPMT") || command.equals("savePMT") || command.equals("closePMT"))
		{
			String ID_MINITTRANSAKSI = getParam("ID_MINITTRANSAKSI");
			if(command.equals("savePMT") || command.equals("closePMT"))
			{
				...
				skrin_name = "app/admin/OT/OTrowPMT.jsp";
			}
			else
			{
				listAktiviti = listAktiviti(session,"2",ID_MINITTRANSAKSI);
				this.context.put("listAktiviti", listAktiviti);
				skrin_name = "app/admin/OT/OTeditPMT.jsp";
			}
			viewPMT = viewPMT(session, ID_MINITTRANSAKSI);
			this.context.put("viewPMT", viewPMT);
			this.context.put("rowCss", getParam("rowCss"));
			this.context.put("BIL", getParam("BIL"));
			
		}
		else if(command.equals("showCARIAN_PMT"))
		{
			this.context.put("FLAG_PMT_CARIAN", getParam("FLAG_PMT_CARIAN"));
			this.context.put("PMT_OPENCLOSE_CARIAN",getParam("PMT_OPENCLOSE_CARIAN"));
			skrin_name = "app/admin/OT/OTskrinCarianPMT.jsp";
		}
		...
		/*
		else if(command.equals("checkPOT"))
		{
			String ID_MINITTRANSAKSI = getParam("ID_MINITTRANSAKSI");
			String ID_TRANSAKSI = getParam("ID_TRANSAKSI_"+ID_MINITTRANSAKSI);
			String checkTransaksiPOT_msg = "";
			myLogger.info(" ID_MINITTRANSAKSI : "+ID_MINITTRANSAKSI+" ID_TRANSAKSI : "+ID_TRANSAKSI);
				
			if(!ID_TRANSAKSI.equals(""))
			{
				//checkTransaksiPOT = checkTransaksiPOT(session, ID_TRANSAKSI, ID_MINITTRANSAKSI);
				myLogger.info(" checkTransaksiPOT : "+checkTransaksiPOT);
				if(checkTransaksiPOT==false)
				{
					checkAllPOT = false;
					viewTT = viewTT(session, ID_TRANSAKSI);
					checkTransaksiPOT_msg += "<font color='red' class='blink' >** Nama Aktiviti '<b>"+(String)viewTT.get("NAMA_AKTIVITI")+"</b>' telah wujud dalam senarai setup minit transaksi!</font><br>";				
				}
			}			
			this.context.put("checkTransaksiPOT_msg", checkTransaksiPOT_msg);
			this.context.put("ID_TRANSAKSI", ID_TRANSAKSI);
			this.context.put("ID_MINITTRANSAKSI", ID_MINITTRANSAKSI);
			this.context.put("checkTransaksiPOT", checkAllPOT);
			skrin_name = "app/admin/OT/OTcheckPOT.jsp";
		}*/
		...
		else if(command.equals("showUnit_POT"))
		{
			String POT_ID_NEGERI = getParam("POT_ID_NEGERI"); 
			listUnit = listUnit(session,POT_ID_NEGERI);
			this.context.put("listUnit",listUnit);
			skrin_name = "app/admin/OT/OTskrinUnitPOT.jsp";
		}
		...
		/*
		else if(command.equals("showListDateOT_POT"))
		{
			String ID_PERMOHONANOT = getParam("ID_PERMOHONANOT"); 
			viewPOT = viewPOT(session, ID_PERMOHONANOT);
			String DATE_MULA = (String)viewPOT.get("TARIKH_MULA");
			String DATE_AKHIR = (String)viewPOT.get("TARIKH_AKHIR");
			listDates = checkLogTransaksi(session,DATE_MULA,DATE_AKHIR,ID_PERMOHONANOT);			
			this.context.put("listDates",listDates);
			this.context.put("ID_PERMOHONANOT",ID_PERMOHONANOT);
			skrin_name = "app/admin/OT/OTlistDates_POT.jsp";
		}
		*/	
		...
		{			
			...
			if(POT_FLAG_OPEN.equals("close"))
			{
				...
				myLogger.info(" #### **** listLogOT : "+listLogOT);
...
//				h.put("TARIKH_KEMASKINI",rs.getString("TARIKH_KEMASKINI") == null ? "" : rs.getString("TARIKH_KEMASKINI"));
	...
	*/
	...
	@SuppressWarnings("unchecked")
	public List listPMT(HttpSession session)throws Exception {
		
		String FLAG_PMT_CARIAN = getParam("FLAG_PMT_CARIAN");
		myLogger.info("-------- FLAG_PMT_CARIAN : "+FLAG_PMT_CARIAN);
			
		
		String PMT_NAMA_AKTIVITI = getParam("PMT_NAMA_AKTIVITI");
		//String TT_NAMA_TABLE = getParam("TT_NAMA_TABLE");
		//String TT_NAMA_FIELD = getParam("TT_NAMA_FIELD");
		this.context.put("PMT_NAMA_AKTIVITI",PMT_NAMA_AKTIVITI.toUpperCase());
		//this.context.put("TT_NAMA_TABLE",TT_NAMA_TABLE.toUpperCase());
		//this.context.put("TT_NAMA_FIELD",TT_NAMA_FIELD.toUpperCase());
		...
		try {
			...
			sql = " SELECT UPPER(S.NAMA_SEKSYEN) AS NAMA_SEKSYEN, T.ID_MINITTRANSAKSI, T.MINIT_INSERT, T.MINIT_UPDATE, "+
					" T.ID_TRANSAKSI, B.NAMA_AKTIVITI, B.ID_SEKSYEN,T.ID_MASUK, T.TARIKH_MASUK, T.ID_KEMASKINI, T.TARIKH_KEMASKINI "+
					" FROM TBLMINITTRANSAKSI T, TBLTRANSAKSI B, TBLRUJSEKSYEN S WHERE T.ID_TRANSAKSI = B.ID_TRANSAKSI" +
					" AND B.ID_SEKSYEN = S.ID_SEKSYEN AND B.ID_SEKSYEN = '2' ";
					
					
			if(FLAG_PMT_CARIAN.equals("Y"))
			{
				if(!PMT_NAMA_AKTIVITI.equals(""))
				{
					sql += " AND UPPER(B.NAMA_AKTIVITI) LIKE '%"+PMT_NAMA_AKTIVITI.toUpperCase()+"%' ";
				}
				/*
				if(!TT_NAMA_TABLE.equals(""))
				{
			...
			sql += " ORDER BY B.NAMA_AKTIVITI ";		
			myLogger.info(" OT : SQL listPMT :"+ sql);
			
			rs = stmt.executeQuery(sql);
			listPMT = Collections.synchronizedList(new ArrayList());
			Map h = null;
			int bil = 0;
			while (rs.next()) {
				h = Collections.synchronizedMap(new HashMap());
	...
	public List listNegeri(HttpSession session)throws Exception {
		...
		try {
			...
			sql = " SELECT LAYER, ID_NEGERI, NAMA_NEGERI FROM "+
					...
					" UNION "+
					" SELECT 2 AS LAYER, TO_CHAR(N.ID_NEGERI) AS ID_NEGERI, N.NAMA_NEGERI FROM TBLRUJNEGERI N WHERE ID_NEGERI != 0 "+
					" ) "+
	...
	public List listUnit(HttpSession session,String ID_NEGERI)throws Exception {
		...
		try {
			db = new Db();
			stmt = db.getStatement(); 
			sql = " SELECT LAYER, ID_UNIT,NAMA_UNIT,NAMA_NEGERI,ID_NEGERI FROM "+
					...
					" UNION "+
					" SELECT 2 AS LAYER, TO_CHAR(ID_PEJABATJKPTG) AS ID_UNIT, UPPER(NAMA_PEJABAT) AS NAMA_UNIT, UPPER(N.NAMA_NEGERI) AS NAMA_NEGERI, " +
		...
		return seqno;
		 ...
		 public void hantarEmel(HttpSession session,String ID_PERMOHONANOT) throws Exception {
				...
				if(ID_STATUS.equals("2") || ID_STATUS.equals("3"))//permohonan lulus or tolak
				{
					...
					{
						content = " Permohonan OT anda adalah diluluskan oleh "+NAMA_PELULUS+". Sila semak permohonan ini di MyeTaPP.";
					}else
					{
						content = " Permohonan OT anda adalah ditolak oleh "+NAMA_PELULUS+". Sila semak permohonan ini di MyeTaPP.";
					}
