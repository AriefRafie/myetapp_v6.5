...
public class SenaraiFailModuleOnlineAgensi extends AjaxModule {
	...
	static Logger myLog = Logger.getLogger(ekptg.view.online.htp.pembelian.SenaraiFailModuleOnlineAgensi.class);
	String actionI = "";
	...
	try{	
		...
		if (id_kementerian == null || id_kementerian.trim().length() == 0){
			if(uk == null){
				getIHTP().getErrorHTML("[ONLINE-HTP PEMBELIAN] KEMENTERIAN/AGENSI TIDAK DIJUMPAI");
...
//				    this.context.put("mode","view");					    
			...
			vm = PATH+"frmIndexAgensi.jsp";
			
		}else{
			String noFail = getParam("noFail");
			String carian = getParam("namaFail");
			idNegeri = getParam("socNegeri") == "" ? "0" : getParam("socNegeri");
			idAgensi = getParam("socAgensi") == "" ? idAgensi : getParam("socAgensi");
			idStatus = getParam("socStatus") == "" ? "0" : getParam("socStatus");
			...
			socNegeri = HTML.SelectNegeri("socNegeri",Long.parseLong(idNegeri));
			socAgensi = HTML.SelectAgensiByKementerian("socAgensi", id_kementerian, Utils.parseLong(idAgensi)
...
private void getValuesPemilik(){
	...
	private void tambahTanahDetail2() throws  Exception{
		...
		htpPermohonan = getIPembelian().findPermohonan(idPermohonan, idHtpPermohonan);
		//urusan = getIPembelian().findByHakmilikUrusanId(idHakmilikUrusan);
		context.put("socNegeri",HTML.SelectNegeri("socNegeri",Long.parseLong(urusan.getIdNegeri())," disabled class=\"disabled\""));
