...
public class FrmPermohonanPembelian extends AjaxModule {
	private static final String PATH="app/htp/pembelian/fail/online/";
	static Logger myLog = Logger.getLogger(ekptg.view.online.htp.pembelian.FrmPermohonanPembelian.class);
	private String vm = PATH+"indexOnline.jsp";
	...
	public String doAction() throws Exception {
		...
		if(command.equals("tambahFail")){
			...
			getKementerianDetail(id_kementerian, "edit");
			...
			String socKementerian = HTML.SelectKementerian("socKementerian", (idKementerian == "") ? null : Long.parseLong(idKementerian),"", " onChange=\"doChangeKementerian()\"  style=\"width:400\"");
			...
			Vector<?> v = getIPembelian().findFailOnline(carian, noFail, idNegeri,id_kementerian);
			context.put("lists", v);
			context.put("socNegeri",HTML.SelectNegeri("socNegeri",Long.parseLong(idNegeri)));
//			setupPage(ses,action, v);
			vm = PATH+"indexOnline.jsp";
		}
			...
			myLog.info("==detailTanah=="+idHakmilikUrusan);
			...
			myLog.info("==kemaskiniMaklumatTanah==");
			getValuesMaklumatTanah();
			tambahTanahDetail2();
			...
			System.out.println("===deleteMaklumatTanah===" +idHakmilikUrusan);
			...
			myLog.info("===viewPemilik===" +idPermohonan);
			getPermohonanInfo();
			getValues();
			Vector<HakmilikUrusan> m = getIPembelian().getHakmilikList(getParam("txtidPermohonan"));
			...
			String carian = getParam("namaFail");
			idNegeri = getParam("socNegeri") == "" ? "0" : getParam("socNegeri");
//			System.out.println("userIDsenarai=="+userID);
//			uk = getIPengguna().getKementerian(userID);
//			System.out.println("UK"+uk);
			
			if (id_kementerian == null || id_kementerian.trim().length() == 0){
				uk = getIPengguna().getKementerian(userID);
//				System.out.println("uk ==" +uk.getAgensi().getIdAgensi());
				if(uk == null){
					throw new Exception("[ONLINE-HTP REKOD] KEMENTERIAN TIDAK DIJUMPAI");
				}
