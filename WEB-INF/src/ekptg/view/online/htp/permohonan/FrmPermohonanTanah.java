...
public class FrmPermohonanTanah extends AjaxBasedModule{	

	private ekptg.model.htp.IHtp iHtp = null;
	private final String IDJENISTANAH = "1,2,3"; //TANAH KERAJAAN - TK,TANAH PENGAMBILAN - PT, TANAH RIZAB - TR
	...
	private static final long serialVersionUID = 3236754680647903718L;
	private static Logger myLog = Logger.getLogger(ekptg.view.online.htp.permohonan.FrmPermohonanTanah.class);
	...
	public String doTemplate2() throws Exception {
		...
		if(session ==null)	
			throw new Exception("[ONLINE-HTP PERMOHONAN] SILA LOGIN SEMULA");
		...
		if ("SahkanPermohonan".equals(submit)) {
			...
			//System.out.println("SahkanPermohonan");
			...
			idpermohonan = String.valueOf(terimaPohonInfo.get("lblIdPermohonan"));
   			//setMaklumatPermohonan(TerimaPohonInfo);
			tanahBean.setLabelDaerah(context, idnegeri);
			tanahBean.setMaklumatPermohonan(context, terimaPohonInfo, template_name);
   		 ...
   		 */
			...
			//myLog.debug(submit+"terimapohoncarian");			
			senaraiTerimaPohon = fData.TerimaPohongetListOnline(idUser,nofail,txtTajuk,
	    	...
	    	template_name = PATH +"borangKListOnline.jsp";
	    	...
	    	Vector<BorangK> k = getKIntergration().getAvailableList(noFailPPT,id_kementerian);
	    	context.put("borangKList", k);
	    	template_name = PATH +"borangKListOnline.jsp";
	    	context.put("idpermohonan", idPermohonan);
	    	...
	    	senaraiTerimaPohon = fData.TerimaPohongetList(idUser,null,null,id_kementerian,null,null,null,null,null,null,null); //--- addby zul for carian by idStatus dan noRujukanOnline --
	    	doListing(session,action,mode,senaraiTerimaPohon);
	    }
	...
	public void doListing(HttpSession session,String action,String mode,Vector v) 
		throws Exception {
		...
		socKementerian = HTML.SelectKementerian("socKementerian", Utils.parseLong(id_kementerian), "disabled=\"disabled\"","onChange=\"doChangeKementerianX()\"");
	...
	public void viewPohonFailBaru(String mode) throws Exception {
		...
		if (mode.indexOf("doChange") != -1) {
			...
			//socStatustanah = HTML.SelectJenisTanah("socStatustanah");
	 		socStatustanah = UtilHTML.SelectJenisTanah("socStatustanah", Long.parseLong(getParam("socStatusTanah")==""?"0":getParam("socStatusTanah")), "",IDJENISTANAH);
			...
			context.put("txdTarikhSuratKJP",lebah.util.Util.getDateTime(new Date(), "dd/MM/yyyy"));
	 		...
	 		//socStatustanah = HTML.SelectJenisTanah("socStatustanah");
	 		socStatustanah = UtilHTML.SelectJenisTanah("socStatustanah", null, "",IDJENISTANAH);
	...
	private void SimpanCharting()throws Exception {
		...
		FrmTerimaPohonData.LakaranPelanOnline(data,"insert");
	}
