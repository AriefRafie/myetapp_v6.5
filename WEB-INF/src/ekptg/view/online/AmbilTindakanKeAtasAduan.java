...
import ekptg.model.online.FrmAmbilTindakanKeAtasAduan;
import ekptg.model.online.FrmListAduanBaruData;

public class AmbilTindakanKeAtasAduan extends AjaxBasedModule {
	...
	public String doTemplate2() throws Exception
    {
        ...
        if ("papar".equals(action)){
			...
			vm = "app/online/frmAmbilTindakanKeAtasAduan.jsp";
			viewAduan(session);
			list = FrmAmbilTindakanKeAtasAduan.getData();
  	    	this.context.put("ViewAduan", list);
         	this.context.put("selectPegawai",HTML.SelectPegawai("socPegawai",null,null));
         	this.context.put("tarikhAgihanKePegawai", sdf.format(now));
			
			
		}
        else if ("simpanTindakan".equals(action)){
        	simpanTindakan(session);
	...
	public void viewAduan(HttpSession session)throws Exception{
		...
		FrmAmbilTindakanKeAtasAduan.setData(id);
	}
	public void simpanTindakan(HttpSession session)throws Exception{
		
		String user_id = (String)session.getAttribute("_ekptg_user_id");
		
		Hashtable h = new Hashtable();
		h.put("id_Aduan", Integer.parseInt(getParam("idAduan")));
		h.put("tindakan_Susulan",getParam("txtTindakanSusulan"));
		h.put("id_Kemaskini",user_id);
		
		FrmAmbilTindakanKeAtasAduan.updateAduan(h);
	}
