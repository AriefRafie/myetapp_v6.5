package ekptg.view.integrasi.lhdn;
...
public class FrmLHDNProses extends AjaxBasedModule {

	private final String PATH="app/integrasi/lhdn/";
	...
	static Logger myLog = Logger.getLogger(ekptg.view.integrasi.lhdn.FrmLHDNProses.class);
	...
	public String doTemplate2() throws Exception {
		...
		if(submit.equals("")){
			vm = PATH+"senaraiProsesIndex.jsp";
			list = new Vector();
			...
			list = getIntegrasi().carianPerintahHQProses(role, getParam("txtNoFail"),getParam("txtPemohon")
					, getParam("txtSimati"),getParam("socJenisKp"),getParam("txtIcSimati")
					, session);
			setupPage(session,action,list);

		}else if(submit.equals("tambahFail")){
			vm = PATH+"senaraiSimati.jsp";		
			this.context.put("selectJenisKp",HTML.SelectPPKJenisKp("socJenisKp", Long.parseLong(jenisKp), "", "class=\"autoselect\""));		
			this.context.put("txtNoFail", getParam("txtNoFail"));
			this.context.put("txtPemohon", getParam("txtPemohon"));
			this.context.put("txtSimati", getParam("txtSimati"));
			...
			if(mode.equals("simpan")){
				...
				simpanStatus("",idMaklumatCukai,MODUL,"3");
				
				getMaklumatValues(idMaklumatCukai);
				
			}else if(mode.equals("updatestatus")){
					mode = "viewsimpan";
					simpanStatus("",getParam("idmaklumatcukai"),MODUL,"3");
