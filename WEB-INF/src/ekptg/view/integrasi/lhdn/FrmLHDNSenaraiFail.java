...
public class FrmLHDNSenaraiFail extends AjaxBasedModule {
	...
	static Logger myLog = Logger.getLogger(ekptg.view.integrasi.lhdn.FrmLHDNSenaraiFail.class);
...
//				, session,true);
			...
			this.context.put("jenisIc", getParam("socJenisKp"));
			this.context.put("txtIcSimati", getParam("txtIcSimati"));			
			list = getIntegrasi().carianFail(role, getParam("txtNoFail"),getParam("txtPemohon")
					, getParam("txtSimati"),getParam("socJenisKp"),getParam("txtIcSimati")
					, session);
			setupPage(session,action,list);
			
		}else if(submit.equals("paparsimati")){
			vm = PATH+"frmMaklumatPermohonan.jsp";
