...
public class FrmCarianFailPHP extends AjaxBasedModule {
	...
	public String doTemplate2() throws Exception {
		...
		this.context.put("selectedAPB", "");
		this.context.put("selectedREV", "");
		this.context.put("selectedPLP", "");
		this.context.put("selectedPNW", "");
		this.context.put("selectedTKR", "");
		this.context.put("selectedPYW", "");
		this.context.put("selectedCRB", "");
		String idJenisFail = getParam("socJenisFail");
		if ("APB".equals(idJenisFail)) {
			this.context.put("selectedAPB", "selected");
		} else if ("REV".equals(idJenisFail)) {
			this.context.put("selectedREV", "selected");
		} else if ("PLP".equals(idJenisFail)) {
			this.context.put("selectedPLP", "selected");
		} else if ("PNW".equals(idJenisFail)) {
			this.context.put("selectedPNW", "selected");
		} else if ("TKR".equals(idJenisFail)) {
			this.context.put("selectedTKR", "selected");
		} else if ("PYW".equals(idJenisFail)) {
			this.context.put("selectedPYW", "selected");
		} else if ("CRB".equals(idJenisFail)) {
			this.context.put("selectedCRB", "selected");
		}		
		...
		try {			
			...
			if ("carian".equals(submit) || "doChanges".equals(submit)) {
				if ("".equals(noFail))
					noFail = "-";
				if ("".equals(idJenisFail)) {
					idJenisFail = "-";
				} else if ("APB".equals(idJenisFail)) {
					idJenisFail = "PASIR";
				} else if ("REV".equals(idJenisFail)) {
					idJenisFail = "HASIL";
				} else if ("PLP".equals(idJenisFail)) {
					...
					idJenisFail = "KUATKUASA";
				}					
				
				list = SIDManager.getArkibDokumenPHP(noFail, getkodNegeriByIdNegeri(idNegeri), getKodDaerahByIdNegeri(idDaerah), idJenisFail);
			}
		} catch (Exception ex) {
			ex.printStackTrace();
		}
		...
		return "app/integrasi/sid/frmCarianFailPHP.jsp";
	}
