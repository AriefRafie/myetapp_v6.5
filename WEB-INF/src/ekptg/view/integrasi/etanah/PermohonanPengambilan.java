...
public class PermohonanPengambilan extends AjaxBasedModule {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private static Logger myLog = Logger.getLogger(PermohonanPengambilan.class);
...
//			postDB = true;
//		}
		String templateDir = "app/integrasi/etanah/melaka/ppt";
...
//		myLog.info("flagUrusan="+flagUrusan);
//		.equals("A")){	
...
//		}else if(permohonan.getJenis().equals("M2")){
		...
		try {
			...
			Hashtable<String,String> maklumatPermohonan = logic.getMaklumatPermohonan(idPermohonan, dbMain);
			myLog.info("maklumatPermohonan="+maklumatPermohonan);

			Vector<Hashtable<String,String>> listHakmilik = logic.getSenaraiHakmilik(idPermohonan, dbMain);
			myLog.info("listHakmilik="+listHakmilik);

			Vector<Tblrujdokumen> listDokumen = logic.getSenaraiDokumen(idPermohonan,jenisSkrin,dbMain);
			myLog.info("listDokumen="+listDokumen);

			ETanahPPTManager pptManager  = new ETanahPPTManager("E-TANAH");
			if (command.equals("hantarSekyen8")) {
				getISek8().setHakmiliks(listHakmilik);
				getISek8().hantar(pptManager, maklumatPermohonan,listDokumen, userID, dbMain);
			
			}else if (command.equals("hantarBorangC")) {
				getIBorangC().setHakmiliks(listHakmilik);
				getIBorangC().hantar(pptManager, maklumatPermohonan,listDokumen, userID, dbMain);
			
			} else if (command.equals("hantarBorangD")) {
				getISek8().setHakmiliks(listHakmilik);
				getISek8().hantar(pptManager, maklumatPermohonan,listDokumen, userID, dbMain);
...
//				}
	...
	private void uploadFiles(String idPermohonanIntegrasi, String jenisDokumen,
		HttpSession session, Db db) throws Exception, FileUploadException {
