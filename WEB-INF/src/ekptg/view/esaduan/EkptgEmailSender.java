...
public class EkptgEmailSender {
	protected static Logger myLog = Logger.getLogger(ekptg.view.esaduan.EkptgEmailSender.class);
	private static EkptgEmailSender singleton;
	...
	private EkptgEmailSender(){
		getMailProperties();
		getMailSession();
		
	}
	
	public static synchronized EkptgEmailSender getInstance(){
		//if(singleton  == null){
			singleton = new EkptgEmailSender();
		//}	
		return singleton;
	...
	public void sendEmailNoAttache(){
		try{
			setMessageNoAttache();
			Transport transport = mailSession.getTransport();
			transport.connect();
	...
	private MimeMessage setMessageNoAttache()throws Exception{
		msg = new MimeMessage(mailSession);
		if(MESSAGE == null)
			throw new Exception("EMAIL DONT HAVE ANY MESSAGE");
		if(SUBJECT == null)
	    ...
	    msg.setFrom(new InternetAddress(FROM,"Support JKPTG"));
