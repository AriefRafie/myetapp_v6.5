...
public class DisplayBlobLampiranUndang implements IServlet2{
	
	static Logger myLogger = Logger.getLogger(DisplayBlobLampiranUndang.class);
			 ...
			 try {
				 myLogger.info("MASUK DOWNLOAD");
		            ...
		            PreparedStatement ps = con.prepareStatement("SELECT CONTENT,JENIS_MIME,NAMA_DOC from " +
		            		" TBLPDTPANDANGANLAMPIRAN WHERE ID_PANDANGANLAMPIRAN = ?");
		            String id = request.getParameter("ID_PANDANGANLAMPIRAN");
		            ps.setString(1,id);
