...
public class HTP_LaporanRingkasanTanahCerobohRizabMengikutNegeri extends AjaxBasedModule{	
	static Logger myLogger = Logger.getLogger(HTP_LaporanRingkasanTanahCerobohMilikPersekutuanMengikutNegeri.class);
	...
	public String doTemplate2() throws Exception{
    	...
    	// SKRIN JSP
    	String skrinDepanSenaraiLaporan = "app/meps/htp/LaporanRingkasanTanahCerobohRizabMengikutNegeri.jsp";
		...
		if("BarGraph".equals(submit)){ 
    		...
    		//DROPDOWN
    		//listPageDepan = model.getListTotalRingkasanTanahCerobohMilikPersekutuanMengikutNegeriAbbrev(id_tahun,id_bulan);
    		if (getJenisLaporan.equals("negeri")||getJenisLaporan.equals("") ){
        		//DROPDOWN
        		if(idNegeri!=null && idNegeri.length()>0){
            		listPageDepan = model.getListTotalDaerahRingkasanTanahCerobohRizabMengikutNegeriAbbrev_baru(idNegeri,id_tahun,id_bulan,txdTarikhMula,txdTarikhAkhir);
        			refence = "no";
        		} else {
            		listPageDepan = model.getListTotalRingkasanTanahCerobohRizabMengikutNegeriAbbrev_baru(id_tahun,id_bulan,txdTarikhMula,txdTarikhAkhir);
        		}
        	}
		...
		//DROPDOWN
		//listPageDepan = model.getListTotalRingkasanTanahCerobohRizabMengikutNegeriAbbrev(id_tahun,id_bulan);
		if (getJenisLaporan.equals("negeri")||getJenisLaporan.equals("") ){
    		//DROPDOWN
    		if(idNegeri!=null && idNegeri.length()>0){
        		listPageDepan = model.getListTotalDaerahRingkasanTanahCerobohRizabMengikutNegeriAbbrev_baru(idNegeri,id_tahun,id_bulan,txdTarikhMula,txdTarikhAkhir);
		...
		vm = skrinDepanSenaraiLaporan;
    		...
    		//GET LIST DATA
    		//listPageDepan = model.getListTotalRingkasanTanahCerobohRizabMengikutNegeri(id_tahun,id_bulan);
    		if (getJenisLaporan.equals("negeri")||getJenisLaporan.equals("") ){
        		//DROPDOWN
        		if(idNegeri!=null && idNegeri.length()>0){
            		listPageDepan = model.getListTotalDaerahRingkasanTanahCerobohRizabMengikutNegeriAbbrev_baru(idNegeri,id_tahun,id_bulan,txdTarikhMula,txdTarikhAkhir);
    		...
    		//listPageDepan = model.getListTotalRingkasanTanahCerobohRizabMengikutNegeriAbbrev(id_tahun,id_bulan);
    		//listPageDepan = model.getListTotalRingkasanTanahCerobohMilikPersekutuanMengikutNegeri(id_tahun,id_bulan);		
    		listPageDepan = model.getListTotalRingkasanTanahCerobohRizabMengikutNegeriAbbrev_baru(id_tahun,id_bulan,txdTarikhMula,txdTarikhAkhir);
    		context.put("PermohonanList", listPageDepan);
    		context.put("list_size", listPageDepan.size()); 
		...
		//context.put("xml", model.generateXML("Laporan Ringkasan Pencerobohan Tanah Milik Persekutuan Mengikut Negeri"));
		
		String tajukLaporan="";
		if (getJenisLaporan.equals("negeri")){
		 tajukLaporan = "Laporan Ringkasan Pencerobohan Tanah Rizab Persekutuan Mengikut Negeri"+(id_tahun.equals("")?"":" ("+id_tahun);
		if(!id_bulan.equals("")){			
		...
		this.context.put("tajukLaporan",tajukLaporan);
			...
			tajukLaporan = "Laporan Ringkasan Pencerobohan Tanah Milik Persekutuan Mengikut Kementerian "+(id_tahun.equals("")?"":" ("+id_tahun);
			if(!id_bulan.equals("")){			
				tajukLaporan += " - "+ekptg.report.Utils.getBulan(Integer.parseInt(id_bulan));			
			}
			if (!txdTarikhMula.trim().equals("")&&!txdTarikhAkhir.trim().equals("") ) {
