...
import ekptg.model.php2.FrmPLPPopupSenaraiTanahBerkaitanData;

public class FrmPLPPopupSenaraiTanahBerkaitanView extends AjaxBasedModule {

	private static final long serialVersionUID = 1L;
	
	FrmPLPPopupSenaraiTanahBerkaitanData logic = new FrmPLPPopupSenaraiTanahBerkaitanData();
	@Override
	public String doTemplate2() throws Exception {
		...
		if ("daftar".equals(actionPopup)){
        	...
        	vm = "app/php2/frmPLPPopupDaftarHakmilik.jsp";
		...
		else if ("pilih".equals(actionPopup)){
			
			String listHakmilikAgensi = "''";
			String listHakmilikSementara = "''";
			...
			if (ids1 != null) {
				...
				try {
					...
					{  
						  System.out.println(" ids1[i] :"+ids1[i]);
						  int checkHakmilikAgensi = ids1[i].indexOf("AGENSI");
						  int checkHakmilikSementara = ids1[i].indexOf("SEMENTARA");
						  
					      if(checkHakmilikAgensi != - 1){
					    	  logic.simpan_pukal(idPermohonan, ids1[i].replaceAll("AGENSI",""), "", session,db,conn);
					    	  listHakmilikAgensi = listHakmilikAgensi + "," + ids1[i].replaceAll("AGENSI", "");		
					      }
					      
					      if(checkHakmilikSementara != - 1){
					    	  logic.simpan_pukal(idPermohonan, "", ids1[i].replaceAll("SEMENTARA",""), session,db,conn);
					    	  listHakmilikSementara = listHakmilikSementara + "," + ids1[i].replaceAll("SEMENTARA", "");	  
					      }
						//listHakmilikAgensi = listHakmilikAgensi + "," + ids1[i];			                                                                     
				    }
					conn.commit();
			...
			System.out.println(" listHakmilikAgensi :"+listHakmilikAgensi);
			System.out.println(" listHakmilikSementara :"+listHakmilikSementara);
			
			
			this.context.put("close_popup", "yes");
			
			vm = "app/php2/frmPLPPopupSenaraiTanahBerkaitan.jsp"; 
			
		}
		else if ("papar".equals(actionPopup)){
	    	
	    	//GO TO DETAIL MAKLUMAT TANAH        	
	    	vm = "app/php2/frmPLPPopupMaklumatTanahBerkaitan.jsp";
