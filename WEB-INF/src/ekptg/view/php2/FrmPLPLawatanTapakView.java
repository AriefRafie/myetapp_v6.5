...
import ekptg.model.php2.FrmPLPLawatanTapakData;

public class FrmPLPLawatanTapakView extends AjaxBasedModule {

	private static final long serialVersionUID = 1L;

	FrmPLPHeaderData logicHeader = new FrmPLPHeaderData();
	FrmPLPLawatanTapakData logic = new FrmPLPLawatanTapakData();
	...
	public String doTemplate2() throws Exception {
        ...
        String changeDropdown = getParam("changeDropdown");
        ...
        Vector senaraiDokumenLawatanTapak = null;
		...
		if (postDB) {
			if ("simpanKemaskiniMaklumatLaporanTanah".equals(hitButton)) {
				...
				if (getParam("txtLaporanAtasTanah").trim().length() > 0)
        			logic.updateMinitKewangan(idPermohonan, getParam("txtTujuanLaporan"), getParam("txtLaporanAtasTanah"),  session);
			}
			...
			if ("hapusKehadiran".equals(hitButton)) {
				logic.hapusKehadiran(idPegawaiLaporanTanah);
			}
			...
			if("simpanDokumenLawatanTapak".equals(hitButton)){
				logic.simpanDokumenLawatanTapak(04L, idPermohonan, getParam("socPejabat"), getParam("socNegeri"), getParam("txtTarikhHantar"),
						getParam("txtJangkaMasa"),	getParam("txtTarikhJangkaTerima"), session);
				flagPopup = "closeLawatanTapak";
			}
        	if ("simpanKemaskiniMaklumatKJP".equals(hitButton)){
        		logic.simpanKemaskiniDokumenLawatanTapak(idUlasanTeknikal, idPejabat, idNegeri, getParam("txtTarikhHantar"),
        	...
        	if ("simpanMaklumatUlanganKJP".equals(hitButton)){
        		logic.simpanDokumenLawatanTapakUlangan(idUlasanTeknikal, idPermohonan, idPejabat, idNegeri, getParam("txtTarikhHantar"),
        				getParam("txtJangkaMasa"), getParam("txtTarikhJangkaTerima"), session);
        		flagPopup = "closeLawatanTapak";
    		}
    		if ("hapusMaklumatKJP".equals(hitButton)){
    			logic.hapusMaklumatKJP(idUlasanTeknikal);
    			flagPopup = "closeLawatanTapak";
    		}
    		if ("doBatalTambahBaruKJP".equals(hitButton)){
    			flagPopup = "closeLawatanTapak";
    		}
		...
		if ("papar".equals(actionPelepasan)) {
			...
			vm = "app/php2/frmPLPMaklumatLawatanTapak.jsp";
			...
			if ("0".equals(selectedTabUpper)){
	        	...
	        	if ("openPopupDokumen".equals(flagPopup)){
	        		...
	        		if ("new".equals(modePopup)){
		    			...
		    			//MAKLUMAT DOKUMEN
						beanMaklumatImejan = new Vector();
						logic.setMaklumatImej(idDokumen,"I");
				...
				logic.setSenaraiImejan(idLaporanTanah,"I");
	        	...
	        	if ("openPopupDokumen".equals(flagPopup)){
	        		...
	        		if ("new".equals(modePopup)){
		    			...
		    			hashMaklumatImejan.put("namaImej", "");
		    			hashMaklumatImejan.put("catatanImej", "");
		    			beanMaklumatLampiran.addElement(hashMaklumatImejan);
		    			...
		    			//MAKLUMAT DOKUMEN
						beanMaklumatImejan = new Vector();
						logic.setMaklumatImej(idDokumen,"L");
						beanMaklumatLampiran = logic.getBeanMaklumatImejan();
						this.context.put("BeanMaklumatLampiran", beanMaklumatLampiran);
				...
				logic.setSenaraiImejan(idLaporanTanah,"L");
				senaraiLampiran = logic.getListImejan();
			...
			vm = "app/php2/frmPLPLawatanTapak.jsp";
			...
			//DOKUMEN
			senaraiDokumenLawatanTapak = new Vector();
			logic.setSenaraiDokumenLawatanTapak(idPermohonan);
			senaraiDokumenLawatanTapak = logic.getListDokumenLawatanTapak();
			this.context.put("SenaraiDokumenLawatanTapak", senaraiDokumenLawatanTapak);

			//OPEN POPUP DETAIL MAKLUMAT KJP
			if ("openLawatanTapak".equals(flagPopup)){
				...
				if ("new".equals(modePopup)){
					...
					this.context.put("BeanMaklumatPejabat",beanMaklumatPejabat);
		    			...
		    			beanMaklumatKJP = new Vector();
						logic.setMaklumatKJP(idUlasanTeknikal);
	...
	// UPLOAD FILE
		...
		// UPLOAD FILE LAMPIRAN
				private void uploadFilesLampiran(String idLaporanTanah, String idPermohonan, HttpSession session) throws Exception {
					DiskFileItemFactory factory = new DiskFileItemFactory();
					ServletFileUpload upload = new ServletFileUpload(factory);
					boolean isMultipart = ServletFileUpload.isMultipartContent(request);
					if (isMultipart != false) {
						...
						while (itr.hasNext()) {
							...
							if ((!(item.isFormField())) && (item.getName() != null)
									&& (!("".equals(item.getName())))) {
								saveDataLampiran(item, idLaporanTanah, idPermohonan, session);
							}
						}
					}
				}

				private void saveDataLampiran(FileItem item, String idLaporanTanah, String idPermohonan, HttpSession session) throws Exception {
