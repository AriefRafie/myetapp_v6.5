 ...
 * Sub Modul Permohonan bagi kegunaan Pengguna Unit HTP JKPTG(N) 
 *
 */
public class FrmPermohonanTanahNegeri extends AjaxBasedModule{	
	...
	private static Logger myLog = Logger.getLogger(ekptg.view.htp.negeri.permohonan.FrmPermohonanTanahNegeri.class);
	...
	Vector MaklumatAsasTanahInfo = null;
	...
	public String doTemplate2() throws Exception {		
		...
		if (idUser == null || idUser.trim().length() == 0){
			throw new Exception(getHTP().getErrorHTML("[HTP PERMOHONAN] SILA LOGIN SEMULA"));			
		}
		...
		try{
			...
			// <link> No. Fail, index.jsp 
				...
				template_name = PATHVER+"FrmTerimaPohonReadOnly.jsp";
				myLog.info("submit=viewMaklumatPermohonan");
				this.context.put("readOnly", "readonly");
				this.context.put("disabled", "disabled");	
				...
				myLog.info("submit=maklumatpermohonan");
				this.context.put("readOnly", "readonly");
				this.context.put("disabled", "disabled");	
				setPaging(false,true,true,false,false);
				Hashtable TerimaPohonInfo = fData.getTerimaPohonInfo(idfail);
				idpermohonan = (String)TerimaPohonInfo.get("lblIdPermohonan");
	   			...
	   			if (hittButton.equals("kemaskini")){
    	    		...
    	    		socDaerah = HTML.SelectDaerahByNegeri(idnegeri, "socdaerah2",Utils.parseLong(String.valueOf(TerimaPohonInfo.get("lblIdDaerah"))),disability,"");
    				...
    				socjenisFail = HTML.SelectTarafKeselamatan("lblTanahKeselamatan",Utils.parseLong((String)TerimaPohonInfo.get("lblTanahKeselamatan")),disability);
				...
				//CARIAN
		    	template_name = PATHVER+"frmTerimaPohonSenaraiFailReadOnly.jsp";
				myLog.debug("terimapohoncarian:"+submit);
		    	flagAdvSearch = "Y";
	    		senaraiTerimaPohon = getPR().TerimaPohongetList(null,nofail,txtTajukCarian,
						id_kementerian,id_agensi,idnegeri,iddaerah,idmukim,idurusan,tarikhBukaFail);
				...
				myLog.debug("maklumatanah="+submit);
				//MAKLUMAT ASAS TANAH
 		    	//this.context.put("readOnly", "readonly");
				//this.context.put("disabled", "disabled");	
...
//		    		context.put("Luas", Luas);
	    		...
	    		if (hittButton.equals("Simpan")) {
	    			...
	    			myLog.info("else hittButton****"+idfail);
	    			TerimaPohonInfo = fData.getTerimaPohonInfo(idfail);
	    		...
	    		context.put("socMukim2",socMukim );
				String id_jenistanah = TerimaPohonInfo.get("lblidKodJTanah").toString();
				myLog.info("mode="+mode+",button=  "+button);
	    		if (mode.equals("MakAsasTanah")) {
		    		//MAKLUMAT ASAS TANAH
		    		...
		    		if(button.equals("SimpanMaklumatAsasTanah")){
	    				...
	    				//SENARAI 
		    			myLog.info(" ** SENARAI:tabmode= **"+tabmode);
	    			...
	    			//DISABLE OPTION UTK PILIH NEGERI
		    		socNegeri = HTML.SelectNegeri("socnegeri2",Utils.parseLong(idnegeri),DISABILITY);
		    		socDaerah = HTML.SelectDaerahByNegeri(idnegeri, "socdaerah2",Utils.parseLong(iddaerah),DISABILITY, "onChange=\"doChanges2('"+tabmode+"')\"");
		    		socMukim = HTML.SelectMukimByDaerah(iddaerah, "socMukim2", Utils.parseLong(idmukim) ,DISABILITY);
		    		socLot = HTML.SelectLot("socLot", Utils.parseLong(socLot),DISABILITY);
...
//					}else if ("1".equals(selectedTab3)) {
					...
					//if ("1".equals(tabmode)) { //TAB KEPUTUSAN 
						...
						myLog.debug("TAB TINDAKAN NEGERI: button="+button);
						myLog.debug("TAB TINDAKAN NEGERI: pageMode="+pageMode);
	    		...
	    		this.context.put("idNegeriNotis",String.valueOf(TerimaPohonInfo.get("lblNegeri")));	
...
// Lama - lama	   			
	    		...
	    		if ("Simpan".equals(hittButton)) {
			    	...
			    	getKIntergration().simpanHTPBorangK(idHakmilikPPT, idpermohonan, idUser);
			    	idfail = idFailBaru;
...
//					}else if ("1".equals(selectedTab3)) {
		    	...
		    	if(flagAdvSearch.equals("Y")){
		    		...
		    		senaraiTerimaPohon = getPR().TerimaPohongetList(idUser,"","","","","","","","",tarikhBukaFail);
	...
	public void setMaklumatPermohonan(Hashtable TerimaPohonInfo) throws Exception {
		try {
			...
			socNegeri = HTML.SelectNegeri("lblNegeri",Utils.parseLong(idnegeri),DISABILITY);
			socDaerah = HTML.SelectDaerahByNegeri(idnegeri,"lblDaerah",Utils.parseLong((String)TerimaPohonInfo.get("lblIdDaerah")),disability);
			...
			socAgensi = HTML.SelectAgensi("lblidAgensi",Utils.parseLong((String)TerimaPohonInfo.get("lblidAgensi")),disability+" style=\"width:400\"");
			socUrusan = UtilHTML.SelectUrusan("lblidUrusan",Utils.parseLong((String)TerimaPohonInfo.get("lblidUrusan")),disability);
		
			socSubUrusan = HTML.SelectSuburusanByIdUrusan(idurusan,"lblidSubUrusan",Utils.parseLong((String)TerimaPohonInfo.get("lblidSubUrusan")),disability);
			socStatustanah = HTML.SelectJenisTanah("lblidKodJTanah", Utils.parseLong((String)TerimaPohonInfo.get("lblidKodJTanah")),disability);
			socjenisFail = HTML.SelectTarafKeselamatan("lblTanahKeselamatan",Utils.parseLong((String)TerimaPohonInfo.get("lblTanahKeselamatan")),disability);
	...
	public void viewPohonFailBaru(String mode) throws Exception {
		//utk simpan
		socNegeri = HTML.SelectNegeri("socNegeri",Utils.parseLong(idnegeri),DISABILITY,"onChange=\"doChanges()\" ");
		...
		socKementerian = UtilHTML.SelectKementerianAktif("socKementerian", Utils.parseLong(id_kementerian), null,"onChange=\"doChanges()\" ");
...
//			myLog.info("txttarikhsuratPTGPTD 2 ="+rsusf.getTarikhMasukStr());
			...
			//getHTP().kemaskiniSimpanStatusPermohonanAktif(subUrusanStatusFail, subUrusanStatusFailN,rsusf.getTarikhMasukFormat());
