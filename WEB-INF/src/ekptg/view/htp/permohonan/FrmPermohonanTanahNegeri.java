...
public class FrmPermohonanTanahNegeri extends AjaxBasedModule{	
	...
	private static Logger myLog = Logger.getLogger(ekptg.view.htp.permohonan.FrmPermohonanTanahNegeri.class);
	...
	public String doTemplate2() throws Exception {
		...
		String template_name = PATH+"frmTerimaPohonSenaraiFailReadOnly.jsp";
		//////////////////////////
		myLog.debug("SUBMIT :: " +submit);
		//log.debug("ACTION :: " +action);
		myLog.debug("MODE :: " + mode);
		myLog.debug("BUTTON :: " + button);
		//log.debug("DOCHAGE :: " +doChange);
		myLog.debug("selectedTab :: " +selectedTab);
		myLog.debug("tabmode :: " + tabmode);
		myLog.debug("PAGEMODE :: " +pagemode);
		...
		try{
			if (submit.equals("SahkanPermohonan")) {
				...
				template_name = PATH+"FrmTerimaPohonReadOnly.jsp";
				Hashtable TerimaPohonInfo = fData.getTerimaPohonInfo(idfail);
				idpermohonan = (String)TerimaPohonInfo.get("lblIdPermohonan");
	   			setMaklumatPermohonan(TerimaPohonInfo);
		    	...
		    	myLog.debug("kemaskinipermohonan:submit="+submit);
		    	this.context.put("readOnly", "readonly");
				this.context.put("disabled", "disabled");	
		    	template_name = PATH+"frmTerimaPohonMaklumatTABBReadOnly.jsp";
	    		...
	    		String pageMode = "";
	    		//SIMPAN
    			//System.out.println("kemaskinipermohonan"+idfail);
	    		...
	    		if ("MakAsasTanah".equals(mode)) {
		    		...
		    		if(button.equals("SimpanMaklumatAsasTanah")){
	    				...
	    				myLog.debug("SimpanDetailLot ** "+button+" **");
	    				SimpanLokasiTanah();
	    				initMaklumatLokasi("readonly");
	       			
	    			}else if (button.equals("doKemaskiniDetailLot")){
	    				myLog.debug("doKemaskiniDetailLot ** "+button+" **");
	    				...
	    				myLog.debug("SimpanCharting ** "+button+" **");
	    				selectedTab2 = "1";
	    				this.context.put("selectedTab2", selectedTab2);
	    				SimpanCharting();
	    				...
	    				myLog.debug("doKemaskiniCharting ** "+button+" **");
	    				selectedTab2 = "1";
	    				this.context.put("selectedTab2", selectedTab2);
	    				KemaskiniCharting();
	    				initMaklumatCharting("readonly");
...
// Lama - lama	   			
		    	...
		    	if(flagAdvSearch.equals("Y")){
		    		...
		    		senaraiTerimaPohon = getPR().TerimaPohongetList(idUser,"","","","",idnegeri,"","","",tarikhBukaFail);
	...
	private void getValues(){
		...
		htpPermohonan.setNoRujukanUPT(noFailLain);
		htpPermohonan.setNoRujukanPTG("");
		htpPermohonan.setNoRujukanPTD("");
