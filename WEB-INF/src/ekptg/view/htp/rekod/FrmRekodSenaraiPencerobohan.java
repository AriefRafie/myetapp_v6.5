...
import ekptg.model.htp.rekod.FrmHakmilikRizabCerobohBean;
...
public class FrmRekodSenaraiPencerobohan extends AjaxBasedModule {
	...
	private static Logger myLog = Logger.getLogger(ekptg.view.htp.rekod.FrmRekodSenaraiPencerobohan.class);
	String userId = "";
	...
	public String doTemplate2() throws Exception {
		...
		if(submit.equals("ceroboh")){
			
			vm = PATH+"rekod/ceroboh/frmCRBMaklumatAduan.jsp";
			...
			logicHeader.setMaklumatTanah(idHakmilikAgensi);
			beanMaklumatTanah = logicHeader.getBeanMaklumatTanah();
			...
			vm = PATH+"rekod/ceroboh/frmCRBOperasiPenguatkuasaan.jsp"; 
			
			//GET DROPDOWN PARAM
					
			if (postDB) {
//				if ("simpanKemaskiniLaporanOperasi".equals(hitButton)){
//					logic.updateLaporanOperasi(idLaporanTanah, getParam("txtTarikhOperasi"), getParam("txtLaporan"), getParam("txtNamaPelaporOperasi"), getParam("txtJawatanLO")
//							,getParam("txtCatatanLO"), getParam("txtTarikhTamatOperasi"), Utils.RemoveSymbol(getParam("txtKosOperasi")), session);
//				}
//				if ("simpanDokumen".equals(hitButton)) {
//					uploadFiles(idLaporanTanah, session);
//				}
...
//					String listAgensi[] = this.request.getParameterValues("txtAgensi");
//					String listJawatan[] = this.request.getParameterValues("txtJawatan");
//					
//					if (listNama != null) {
//						for (int i = 0; i < listNama.length; i++) {
//							if (!listNama[i].equals("")) {
//								logic.saveKehadiran(i, idLaporanTanah, listNama[i], listAgensi[i], listJawatan[i], getParam("flagPengerusi"), session);
//							}
//						}
//					}
//				}
//				if ("simpanKemaskiniKehadiran".equals(hitButton)) {
//					logic.updateKehadiran(idLaporanTanah, idPegawaiLaporanTanah, getParam("txtNama"),
//							getParam("txtAgensi"),
//							getParam("txtJawatan"), getParam("socPengerusi"), session);
//				}
...
//				}
			...
			if ("update".equals(mode)){
				...
				beanLaporanOperasi = new Vector();
				logic.setMaklumatLaporanOperasi(idLaporanTanah);
				beanLaporanOperasi = logic.getBeanMaklumatLaporanOperasi();
				this.context.put("BeanLaporanOperasi", beanLaporanOperasi);
		...
		if("".equals(firstAction))	{
		   vm = PATH+"rekod/ceroboh/frmRekodIndexSenaraiHakmilikRizab.jsp";
...
//		 }
	...
	private ITanahCarian getIHakmilikRizab(){
		if (iHakmilikRizab == null){
			iHakmilikRizab = new FrmHakmilikRizabCerobohBean();
