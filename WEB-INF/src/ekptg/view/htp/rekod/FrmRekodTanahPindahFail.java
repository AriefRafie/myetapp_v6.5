...
import ekptg.model.htp.rekod.HTPUtilitiRekodBean;
...
public class FrmRekodTanahPindahFail extends AjaxBasedModule {
	...
	private static Logger myLog = Logger.getLogger(ekptg.view.htp.rekod.FrmRekodTanahPindahFail.class);
...
//					, idLotHR, noLot, idAgensi, idKementerian, idStatus);
		...
		//VIEW CARIAN HAKMILIK DAN RIZAB BY idNegeri,idDaerah,idMukim,noHakmilik
			...
			if (nextAction.equals("paparDetailHakmilik")){
				vm = PATH+"utiliti/pindahfail/frmRekodMaklumatTanah.jsp";	
				isSambungan = false;
				...
				viewModeHakmilik(session,nextAction,lastAction
...
//			   FrmRekodPembangunanPentadbiranData.updatePembangunanById(hPembangunanUpdate);
//		      	   
//			    //SENARAI PEMBANGUNAN BY ID HAKMILIK
//			   listPembangunan = view_modeSenaraiPembangunan(session);
//			   this.context.put("SenaraiPembangunan", listPembangunan);
...
//	  			langkah = "26";
//			}
//			myLog.info(hFungsi+","+disableFungsi+","+langkah);
...
//				   	String idHakmilikPerihalBaru = FrmRekodPembangunanPentadbiranData.addDetailKeluasan(hAddDetailKeluasan);
//	
//				   	String idSusulan = getISusulanPembangunan().simpan(setSusulanValues(idHakmilik,idHakmilikPerihal
//	    					,String.valueOf(context.get("txdTarikhBina")),String.valueOf(context.get("txtCatatan"))));
//	    			String idStatusFail = kemaskiniSimpanStatusSelesai(idHakmilik,idHakmilikPerihal,IDSUBURUSAN,langkah);
...
//		    	//idHakmilik
//		    	if(getISusulan().getMaklumat(idHakmilik,sumber,idHakmilikPerihal)==null){
//		    		hInsert = new Hashtable(); 	
...
//	    		
//			   if(portal_role.contains("HQPengguna")){
//				   String idStatusFail = kemaskiniSimpanStatusSelesai(idHakmilik,idHakmilikPerihal,IDSUBURUSAN,String.valueOf((Integer.parseInt(langkah)+1)));
//				   hInsert = new Hashtable(); 			
//				   hInsert.put("idStatusFail", idStatusFail);
...
//			this.context.put("idHakmilik", getParam("idHakmilik"));
		...
		}else if(firstAction.equals("failbaru")){	
			//Penggunaan sumbit button
			if (submit.equals("carianfailbaru")){
				vm = PATH+"utiliti/pindahfail/frmRekodMaklumatTanah.jsp";	
				noFail = getParam("txtNoFail");
				mode = getParam("mode");
				boolean paparMaklumat = false;
				boolean paparSenarai = false;
	        	...
	        	// VIEW MAKLUMAT RIZAB BY ID
	        	//viewModeRizab(session,nextAction,lastAction,idNegeriHR,idDaerahHR,idMukimHR,idJenisHakmilikHR,idCaraBayar,idJenisHakmilikBaru);
	        	maklumatFailPindah(idHakmilik);					
		    	if (mode.equals("carian")) {
		
		    		senaraiFail = new Vector();
		    		senaraiFail = getIHTPFail().getSenaraiFailMengikutUrusans(null, noFail.trim(), ""
							, "", ""
							, "", "", ""
							, "1,10,2,5", "", ""
							, "", true);
			    	myLog.info("list"+senaraiFail.size());
			    	if(senaraiFail.size()>1) {
			    		paparSenarai = true;
						this.context.put("SenaraiHakmilik", senaraiFail);
			    		
			    	}else if(senaraiFail.size()==1){ 
			    		paparMaklumat = true;
			    		Hashtable hMaklumatFail = (Hashtable)senaraiFail.get(0);
			    		idPermohonan = String.valueOf(hMaklumatFail.get("idPermohonan"));
			    		HtpPermohonan htPermohonan = getIHTP().findPermohonanKutipan(idPermohonan, "");			    		
						context.put("idPermohonanb", idPermohonan);
				        viewMaklumatFailBaru(htPermohonan); 					

			    	}
			    	
			    }else if (mode.equals("carianpapar")) {
				   	paparMaklumat = true;
				   	idPermohonan = getParam("idpermohonan");
				   	HtpPermohonan htPermohonan = getIHTP().findPermohonanKutipan(idPermohonan, "");	
				   	myLog.info("htPermohonan:"+htPermohonan);
					context.put("idPermohonanb", idPermohonan);
					viewMaklumatFailBaru(htPermohonan); 					
			    
				} else if (mode.equals("simpan")) {				
			    	noFail = noFail.equals("")?getParam("txtnofailb"):noFail;
			    	String noFailLama = getParam("txtnofailama").trim();
			    	idPermohonan = getParam("idpermohonan");
			    	
			    	getIUtilRekod().kemaskiniPerolehan(idHakmilik, idPermohonan, noFail.trim());			    	
		        	maklumatFailPindah(idHakmilik);					

			    	String idSeksyen = FrmUtilData.getSeksyenMengikutPengguna(userId).equals("")?IDSEKSYEN:FrmUtilData.getSeksyenMengikutPengguna(userId);
			    	String noTanah = String.valueOf(context.get("txtNoHakmilik")).equals("")?String.valueOf(context.get("txtNoWarta")):String.valueOf(context.get("txtKodJenisHakmilik"))+" "+String.valueOf(context.get("txtNoHakmilik"));
			    	noTanah	+=	", "+String.valueOf(context.get("txtKodLot"))+String.valueOf(context.get("txtNoLot")); 
			    	myLog.info(noTanah);
			    	AuditTrail.logActivity("1", idSeksyen, this, session, "UPD"
			    			,"HAKMILIK/RIZAB  ["+noTanah+"] DIKEMASKINI" +
			    			", FAIL LAMA ["+noFailLama+"]/ FAIL BARU ["+noFail.trim()+"]");
		    		noFail = "";
		    		
			    }
				context.put("txtNoFail", noFail.trim());
				context.put("paparsenarai", paparSenarai);
				context.put("paparmaklumat", paparMaklumat);
	...
	// VIEW MAKLUMAT HAKMILIK BY ID
	private void viewModeHakmilik(HttpSession session,String nextAction,String lastAction
	...
	private void view_modeLuasTerkumpul(HttpSession session) throws Exception {
		...
		this.context.put("txtBangunan",Utils.formatLuas((Double)luasTerkumpul.get("txtBangunan")));
...
//		}
		...
		private void viewMaklumatFailBaru(HtpPermohonan htPermohonan) throws Exception {

			context.put("txtFailPTDb",htPermohonan.getNoRujukanPTD());
			context.put("txtFailPTGb",htPermohonan.getNoRujukanPTG());
			context.put("txtTajukb",htPermohonan.getPermohonan().getPfdFail().getTajukFail());			
			context.put("txtNamaKementerianb",htPermohonan.getPermohonan().getPfdFail().getNamaKementerian());
			context.put("txtNoFailSeksyenb", htPermohonan.getPermohonan().getPfdFail().getNoFail());
			//context.put("txtNamaNegerib", htPermohonan.getPermohonan().getNamaNegeri());
			//this.context.put("txtNamaDaerah", (String)hMaklumatFail.get("namaDaerah"));
			//this.context.put("txtNamaMukim", (String)hMaklumatFail.get("namaMukim"));
			context.put("txtNamaAgensib", htPermohonan.getNamaAgensi());
			//this.context.put("txtJenisHakmilik", (String)hMaklumatFail.get("jenisHakmilik"));
			context.put("txtFailKJPb", htPermohonan.getNoRujukanKJP());
			//this.context.put("txtNoWarta", (String)hMaklumatFail.get("noWarta"));
			//this.context.put("txtNoHakmilik",(String) hMaklumatFail.get("noHakmilik"));
			//this.context.put("txtNoLot", (String)hMaklumatFail.get("noLot"));	
			//this.context.put("caraPerolehan", (String)hMaklumatFail.get("caraPerolehan"));
			...
			//this.context.put("jenisTanah", (String)hMaklumatFail.get("jenisTanah"));
			//return (String)hMaklumatFail.get("hakmilikAsal");
			context.put("caraPerolehanb", htPermohonan.getPermohonan().getPfdFail().getNamaUrusan());
		
		}
		
		/*
		 * Dibuat oleh Mohamad Rosli 15/12/2012(non-Javadoc)
		 * @see ekptg.model.htp.rekod.FrmRekodTanahPindahFail#kemaskiniPerolehan(HttpSession session)
		 */
		private String maklumatFailPindah(String idHakmilik) throws Exception {
			...
			context.put("txtFailPTG",(String)hMaklumatFail.get("noFailPtg"));
			context.put("txtTajuk",(String)hMaklumatFail.get("tajukFail"));
			context.put("txtNamaKementerian", (String)hMaklumatFail.get("namaKementerian"));
			context.put("txtNoFailSeksyen", (String)hMaklumatFail.get("noFailSeksyen"));
			context.put("txtNamaAgensi", (String)hMaklumatFail.get("namaAgensi"));		
			context.put("txtKodJenisHakmilik", (String)hMaklumatFail.get("kodJenisHakmilik"));
			context.put("txtFailKJP", (String)hMaklumatFail.get("noFailKjp"));
			context.put("txtNoWarta", (String)hMaklumatFail.get("noWarta"));
			context.put("txtNoHakmilik",(String) hMaklumatFail.get("noHakmilik"));
			context.put("txtKodLot", (String)hMaklumatFail.get("kodLot"));				
			context.put("txtNoLot", (String)hMaklumatFail.get("noLot"));	
			context.put("caraPerolehan", (String)hMaklumatFail.get("caraPerolehan"));
			return (String)hMaklumatFail.get("hakmilikAsal");
