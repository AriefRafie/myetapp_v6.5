...
import ekptg.model.htp.HTPFailBayaranBean;
 ...
 * Sub Modul Pajakan bagi kegunaan Pengguna Bayaran Pajakan & Akaun Belum Terima(ABT) BHTP 
 *
 */
public class FrmPajakanSenaraiFailBayaran extends AjaxBasedModule {
	...
	private static Logger myLog = Logger.getLogger(ekptg.view.htp.pajakan.FrmPajakanSenaraiFailBayaran.class);
	...
	//Lain2
	private IBayaranBean iHTPFB = null;
	//19/08/2010
	...
	public String doTemplate2() throws Exception {
        ...
        myLog.info("subUrusan:"+subUrusan);
...
//    			myLog.debug("** save draft **");
        	...
        	//PENAMATAN
        		...
        		getStatus().kemaskiniStatusPermohonan(idPermohonan,subUrusan,"98",userID);
	...
	*/		
		...
		//if ("daftarBaru".equals(submit)){       	
  		...
  		// Pendaftaran 24/01/2017
        } else if ("papardaftar".equals(actionPajakan)){
        	//GO TO PAPAR PAJAKAN  
		    ...
		    myLog.info("idPermohonan 288:"+idPermohonan);
		    ...
		    page_ = "2";		        
        // MJM 25/01/2017
        }else if(actionPajakan.equalsIgnoreCase("paparmjm") || actionPajakan.equalsIgnoreCase("paparan")){        	
...
//	        		MemorandumJemaahMenteriView(mode);
	        		//Vector<Hashtable<String, String>> vec = logicmjm.getMaklumatMemorandumJemaahMenteri(idPermohonan);
			...
			if (selectedTab.equals("0")){        		        	
	        	...
	        	this.context.put("SenaraiDraf", senaraiDraf);	        		
	        	drafViewPerjanjian(mode, idDraf);
	        	...
	        	logicper.setMaklumatMOA(idPermohonan);
	            MOAView(mode);
...
//    	    	
			...
			if(flagAdvSearch.equals("Y")){
				...
				//Apabila pilih Negeri, Kementerian atau Agensi
				list = getIHTPFBayaran().carianFail(getParam("txtNoFail")
					, getParam("txtTajukFail")
					, getParam("txdTarikhTerima")
					, getParam("txtNamaPemohon")
...
//					myLog.info("simpanLampiranDraf:INSERT");
	...
	private void saveDraf(String idPermohonan, HttpSession session) throws Exception {
		...
		hash.put("ulasan", getParam("txtKeteranganDraf"));	
		logicper.saveDraf(idPermohonan, hash, session);
...
//	} 
	...
	public void statusView(String idPermohonan,String mode) throws Exception{			
	...
	private void kemaskiniSimpanStatusSelesai(String idFail,String idPermohonan,String idSubUrusan,String langkah)
		throws Exception {
		 try {
			...
			subUrusanStatusFailN.setTarikhMasuk(getParam("txdTarikhTerima").equals("")?"01/01/1900":getParam("txdTarikhTerima"));
	...
	private IBayaranBean getIHTPFBayaran(){
		if(iHTPFB == null)
			iHTPFB = new HTPFailBayaranBean();
		return iHTPFB;
	}	
