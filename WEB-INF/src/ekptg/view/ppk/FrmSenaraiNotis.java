...
public class FrmSenaraiNotis extends AjaxBasedModule {
	...
	FrmHeaderPpk modelNotisHeader = new FrmHeaderPpk();
	...
	public String doTemplate2() throws Exception {
		...
		String commands = getParam("commands");
		
		String screen1 = "app/ppk/frmPrmhnnSek8NotisPerbicaraan.jsp";
		String screen2 = "app/ppk/frmPrmhnnSek8Penjaga.jsp";
		String screen3 = "app/ppk/frmSenaraiNotis.jsp";
		

		// prevent something when refresh page
		...
		Vector listSerahanNotis = new Vector();
		...
		Vector tarikdatadariHeader = new Vector();
		Vector listCountOB = new Vector();
		Vector listCountPenerimaNotis = new Vector();
		Vector listPenghutang = null;
		
		// modified on 08122009
		Vector cetakAkuanSumpah = new Vector();
		cetakAkuanSumpah.clear();
		
		//aishahlatip - tab pengesahan pegawai 18052017
		...
		// flag from fail tugasan & kemaskini fail & view fail lama
		...
		// String flagFromMyJsp = getParam("flagFromMyJsp");

		// modified on 07122009
		...
		String tarikhEmail = getParam("txdTarikhHantar");
		...
		// get data keputusan permohonan
					...
					kp2a = modelNotisHeader.getHeaderData(session, id, "Y", "", "T");
					...
					Hashtable kp1a = new Hashtable();
					String idkp1 = "";

					if (keputusanPermohonan.size() != 0) {
						kp1a = (Hashtable) keputusanPermohonan.get(0);					
						idkp1 = kp1a.get("id_keputusanpermohonan").toString();					
					}

					// --data notis
					modelNotis.setListSemakWithData(idkp1);
					dataNotis = modelNotis.getListSemakWithData();
					
					String idperbicaraan1 = "";
					Hashtable idn1 = new Hashtable();
					if (dataNotis.size() != 0) {
						idn1 = (Hashtable) dataNotis.get(0);
						idperbicaraan1 = idn1.get("id_perbicaraan").toString();
					}
					
					//convert object to string
					String id_permohonansimati2= String.valueOf(id_permohonansimati1);
				
					// get senarai penerima notis
					modelNotis.setListPenerimaNotis(idperbicaraan1, id_permohonansimati2);
					listPenerimaNotis = modelNotis.getListPenerimaNotis();
					
					context.put("listPenerimaNotis_size1", listPenerimaNotis.size());

		
		// Untuk check sama ada notis telah dihantar atau belum - End

		String submit = getParam("command");
		id = getParam("id_permohonan");
		
		
		if (submit == "Cari" || submit == "cari" || submit.equals("Cari"))
		{
			commands = "selesai";
		...
		if 
		(commands.equals("FailLengkap")){
			
			commands = "selesai";
			submit = "Cari2";
			myLogger.info("commands ********* :: "+commands);
			// setupPage(session,action,listCari); mmg comment
			// data & size list carian mmg comment
		...
		//System.out.println("commands kat sini =" + commands);
		if (submit == "Cari2")
		{
			submit = "Cari2";
		}
		...
		if ("semakNoData".equals(submit)) {
			...
			/** ADD BY PEJE - SENARAI PEMIUTANG **/
			modelNotis.setListPemiutang(id_permohonansimati, idSimati, "8", null);
			...
			myLogger.info("id2 = "+id2);
			myLogger.info("usid = "+usid);
			logic_A.setDataPPSPP(id2, usid);
			list2 = logic_A.getDataPPSPP();
			this.context.put("list2", list2);
			//myLogger.info("id_jkptg1********* = "+idPejabatJKPTG);			
			...
			if (idPejabatJKPTG != "") {
				...
				alamatTempatBicara = modelNotis.getAlamatTempatBicara(idPejabatJKPTG);
			...
			//penghutang
			context.put("listPenghutang", listPenghutang);
			context.put("listPenghutang_size", listPenghutang.size());
			...
			if (idPejabatJKPTG != "") {
//				context.put("selectBicara",HTML.SelectTempatBicaraByPejabatJKPTG(
//												idPejabatJKPTG,
//												"socTempatBicara",
//												Utils.parseLong(idPejabatJKPTG),
//												null,
//												"style=width:400 onChange=\"doChangeidTempatBicara();\" "));
				context.put("selectBicara",PPKUtilHTML.SelectTempatBicaraByPejabatJKPTG(
						usid,idPejabatJKPTG,"socTempatBicara",Utils.parseLong(idPejabatJKPTG), null,"style=width:400 onChange=\"doChangeidTempatBicara();\""));

			} else {
				context.put("selectBicara",HTML.SelectTempatBicara(
						"socTempatBicara",null, null,"style=width:400 onChange=\"doChangeidTempatBicara();\" "));
...
//													"style=width:400 onChange=\"doChangeidTempatBicara();\" "));
				...
				context.put("checkP2", "");
				context.put("checkP3", "");
				//myLogger.info("idPejabatJKPTG2 = "+idPejabatJKPTG);
				
			} else if (jenispejabat.equals("2")) {
				// dropdown pejabat tanah
				if (idPejabatJKPTG != "") {
					context.put("selectBicara",HTML.SelectPejabatTanahByJKPTG(
							idPejabatJKPTG
							,"socTempatBicara"
							,null,null
							,"style=width:400 onChange=\"doChangeidTempatBicara();\" "));
				...
				//myLogger.info("idPejabatJKPTG3 = "+idPejabatJKPTG);
...
//													"style=width:400 onChange=\"doChangeidTempatBicara();\" "));
					context.put("selectBicara",PPKUtilHTML.SelectTempatBicaraByPejabatJKPTG(
							usid,idPejabatJKPTG,"socTempatBicara",null, null,"style=width:400 onChange=\"doChangeidTempatBicara();\" "));
			...
			context.put("selectNegeri", HTML.SelectNegeri("socNegeri", null,negeriReadonly));
		...
		// tukar notis
			...
			context.put("listPenerimaNotis", "");//aishahlatip
			// --get data ob
			modelNotis.setListSemuaOB(id_permohonansimati, idSimati, "8",null);
			listOB = modelNotis.getListSemuaOB();
			...
			//penghutang
			this.context.put("listPenghutang", listPenghutang);
			context.put("listPenghutang_size", listPenghutang.size());
...
//												"style=width:400 onChange=\"doChangeidTempatBicara();\" "));
		...
		//code WP 12/05/2017
			...
			if (idTempatBicara != "") {
				...
				if (jenispejabat.equals("1")) {
					alamatTempatBicara = modelNotis.getAlamatTempatBicara(idTempatBicara);
				} else if (jenispejabat.equals("2")) {
					alamatTempatBicara = modelNotis.getAlamatPejabatTanah(idTempatBicara);
				} else {
					alamatTempatBicara = modelNotis.getAlamatTempatBicara(idTempatBicara);
				}
...
//														idPejabatJKPTG,
//														"socTempatBicara",
//														Utils.parseLong(idTempatBicara),
//														null,
//														"style=width:400 onChange=\"doChangeidTempatBicara();\" "));
						context.put("selectBicara",PPKUtilHTML.SelectTempatBicaraByPejabatJKPTG(
								usid,idPejabatJKPTG,"socTempatBicara",Utils.parseLong(idTempatBicara), null,"style=width:400 onChange=\"doChangeidTempatBicara();\""));

					} else {
						context.put("selectBicara",HTML.SelectPejabatTanahByJKPTG(
								idPejabatJKPTG
								,"socTempatBicara"
								,Utils.parseLong(idTempatBicara),null
								,"style=width:400 onChange=\"doChangeidTempatBicara();\" "));
					}
				} else {
					if (jenispejabat.equals("1")) {
...
//														"style=width:400 onChange=\"doChangeidTempatBicara();\" "));
		...
		//code wp 12/05/2017
				else if ("doValidatePublicDate".equals(submit)) {
					String dateEvent = getParam("dateEvent");
					String txdTarikhBicara = getParam("txdTarikhBicara");
					String txdTarikhNotis = getParam("txdTarikhNotis");

					String editTarikhBicara = getParam("editTarikhBicara");
					String editTarikhNotis = getParam("editTarikhNotis");

					boolean flag = false;
					// validate public date
					if (dateEvent.equals("bicaraNew") && txdTarikhBicara != null && txdTarikhBicara.length() > 0) {
						flag = modelNotis.getTarikhValidation(txdTarikhBicara);
					} else if (dateEvent.equals("notisNew") && txdTarikhNotis != null && txdTarikhNotis.length() > 0) {
						flag = modelNotis.getTarikhValidation(txdTarikhNotis);
					} else if (dateEvent.equals("bicaraEdit") && editTarikhBicara != null && editTarikhBicara.length() > 0) {
						flag = modelNotis.getTarikhValidation(editTarikhBicara);
					} else if (dateEvent.equals("notisEdit") && editTarikhNotis != null && editTarikhNotis.length() > 0) {
						flag = modelNotis.getTarikhValidation(editTarikhNotis);
					}
					
System.out.println("flag===="+flag);

					if (flag) {
						context.put("flagPublicDate", "yes");
					} else {
						context.put("flagPublicDate", "no");
					}

					context.put("txdTarikhBicara", txdTarikhBicara);
					context.put("txdTarikhNotis", txdTarikhNotis);

					context.put("tarikhBicara", editTarikhBicara);
					context.put("tarikhNotis", editTarikhNotis);

					id = getParam("id_permohonan");
					String flagdata = "";

					// validate public holiday here
					// --end---
					...
					// get selected ob
					modelNotis.setSelectedOB(idperbicaraan, id_permohonansimati, idSimati);
					...
					if (negeri != "") {
						context.put("selectNegeri",
								HTML.SelectNegeri("socNegeri", Utils.parseLong(negeri), "class=disabled disabled style=width:305"));
					} else {
						context.put("selectNegeri", HTML.SelectNegeri("socNegeri", null, "class=disabled disabled style=width:305"));
					}
					...
					if (idNegeri != "") {
						if (idPegawai != "") {
							context.put("selectPegawai",
									HTML.SelectPegawaiPengendaliByNegeri(idNegeri, "socPegawai", Utils.parseLong(idPegawai), "style=width:305"));
						} else {
							context.put("selectPegawai", HTML.SelectPegawaiPengendaliByNegeri(idNegeri, "socPegawai", null, "style=width:305"));
						}
					} else {
						if (idPegawai != "") {
							context.put("selectPegawai", HTML.SelectPegawaiPengendali("socPegawai", Utils.parseLong(idPegawai), "style=width:305"));
						} else {
							context.put("selectPegawai", HTML.SelectPegawaiPengendali("socPegawai", null, "style=width:305"));
						}
					}
					...
					if (idPejabatJKPTG != "") {
						...
						if (idTempatBicara != "") {
							if (jenispejabat.equals("1")) {
								context.put("selectBicara", HTML.SelectTempatBicaraByPejabatJKPTG(idPejabatJKPTG, "socTempatBicara",
										Utils.parseLong(idTempatBicara), null, "style=width:400 onChange=\"doChangeidTempatBicara();\" "));
							} else {
								context.put("selectBicara", HTML.SelectPejabatTanahByJKPTG(idPejabatJKPTG, "socTempatBicara",
										Utils.parseLong(idTempatBicara), null, "style=width:400 onChange=\"doChangeidTempatBicara();\" "));
							}
						} else {
							if (jenispejabat.equals("1")) {
								context.put("selectBicara", HTML.SelectTempatBicaraByPejabatJKPTG(idPejabatJKPTG, "socTempatBicara", null, null,
										"style=width:400 onChange=\"doChangeidTempatBicara();\" "));
							} else {
								context.put("selectBicara", HTML.SelectPejabatTanahByJKPTG(idPejabatJKPTG, "socTempatBicara", null, null,
										"style=width:400 onChange=\"doChangeidTempatBicara();\" "));
							}
						}
						...
						if (idTempatBicara != "") {
							if (jenispejabat.equals("1")) {
								context.put("selectBicara", HTML.SelectTempatBicara("socTempatBicara", Utils.parseLong(idTempatBicara), null,
										"style=width:400 onChange=\"doChangeidTempatBicara();\" "));
							} else {
								context.put("selectBicara", HTML.SelectPejabatTanah("socTempatBicara", Utils.parseLong(idTempatBicara), null,
										"style=width:400 onChange=\"doChangeidTempatBicara();\" "));
							}
						} else {
							if (jenispejabat.equals("1")) {
								context.put("selectBicara", HTML.SelectTempatBicara("socTempatBicara", null, null,
										"style=width:400 onChange=\"doChangeidTempatBicara();\" "));
							} else {
								context.put("selectBicara", HTML.SelectPejabatTanah("socTempatBicara", null, null,
										"style=width:400 onChange=\"doChangeidTempatBicara();\" "));
							}
						}
					}
...
//			System.out.println("countPenerimaNotis==="+countPenerimaNotis);
			...
			// get data ob
			modelNotis.setListSemuaOB(id_permohonansimati, idSimati, "8", idperbicaraan);
			listOB = modelNotis.getListSemuaOB();
			
//			System.out.println("Read here setDataPenghutang");
			myLogger.info("Read here setDataPenghutang");
			//get data penghutang
			logic_internal.setDataPenghutang(id_permohonansimati);
			listPenghutang = logic_internal.getDataPenghutang();
			
			/** ADD BY PEJE - SENARAI PEMIUTANG **/
			modelNotis.setListPemiutang(id_permohonansimati, idSimati, "8", idperbicaraan);
			listPemiutang = modelNotis.getListPemiutang();
			...
			modelNotis.setDataOBNotis(idOBDTL, getParam("id_permohonansimati"));
			dataOBNotis = modelNotis.getDataOBNotis();
			
//			String idParentOB = modelNotis.getIdParentOB(getParam("id_permohonansimati_atheader")); 
			...
			context.put("listSerahanNotis_size", listSerahanNotis.size());
			...
			/** ADD BY PEJE - SENARAI PEMIUTANG **/
			context.put("listPemiutang_size", listPemiutang.size());
			context.put("listPenerimaNotis_size", listPenerimaNotis.size());//wp
			...
			if (idjenispejabat.equals("22")) {
				...
				if (idPejabatJKPTG != "") {
					if (idpejabat != "") {
//						context.put("showBicara",HTML.SelectTempatBicaraByPejabatJKPTG(
//														idPejabatJKPTG,"editTempatBicara",Utils.parseLong(idpejabat),null,"style=width:400 class=disabled disabled "));
						context.put("showBicara", HTML.SelectTempatBicaraByPejabatJKPTG(
								idPejabatJKPTG, "editTempatBicara",Utils.parseLong(idpejabat), null, "style=width:400 class=disabled disabled "));
					} else {
//						context.put("showBicara",HTML.SelectTempatBicaraByPejabatJKPTG(
...
//														"style=width:400 class=disabled disabled "));
						context.put("showBicara", HTML.SelectTempatBicaraByPejabatJKPTG(
								idPejabatJKPTG, "editTempatBicara",null, null, "style=width:400 class=disabled disabled "));
					}
				} else {
					if (idpejabat != "") {
						context.put("showBicara", HTML.SelectTempatBicara(
								"editTempatBicara", Utils.parseLong(idpejabat),null,
									"style=width:400 class=disabled disabled  "));
					} else {
						context.put("showBicara", HTML.SelectTempatBicara(
				...
				if (idPejabatJKPTG != "") {
					if (idpejabat != "") {
						context.put("showBicara",HTML.SelectPejabatTanahByJKPTG(
								idPejabatJKPTG
								,"editTempatBicara"
								,Utils.parseLong(idpejabat),null
								,"style=width:400 class=disabled disabled "));
					} else {
						context.put("showBicara",HTML.SelectPejabatTanahByJKPTG(
								idPejabatJKPTG
				...
				if (idPejabatJKPTG != "") {
					...
					if (idpejabat != "") {
						context.put("showBicara", HTML.SelectTempatBicara(
								"editTempatBicara", Utils.parseLong(idpejabat),null,"style=width:400 class=disabled disabled  "));
					} else {
						context.put("showBicara", HTML.SelectTempatBicara(
								"editTempatBicara", null, null,"style=width:400 class=disabled disabled  "));
					}
				}
...
//														"style=width:400 onChange=\"doChangeidTempatBicaraUPDATE();\" "));
						context.put("SELECTBicara",PPKUtilHTML.SelectTempatBicaraByPejabatJKPTG(
								usid,idPejabatJKPTG,"editTempatBicara",Utils.parseLong(idTempatBicara), null,"style=width:400 onChange=\"doChangeidTempatBicaraUPDATE();\" "));
					} else {
						context.put("SELECTBicara",HTML.SelectPejabatTanahByJKPTG(
														idPejabatJKPTG,
														"editTempatBicara",
					...
					if (jenispejabat.equals("1")) {
//						context.put("c",HTML.SelectTempatBicaraByPejabatJKPTG(
//														idPejabatJKPTG,
//														"editTempatBicara",
//														null, null,
//														"style=width:400 onChange=\"doChangeidTempatBicaraUPDATE();\" "));
						context.put("SELECTBicara",PPKUtilHTML.SelectTempatBicaraByPejabatJKPTG(
								usid,idPejabatJKPTG,"editTempatBicara",null, null,"style=width:400 onChange=\"doChangeidTempatBicaraUPDATE();\" "));
...
//			myLogger.info("idpejabat = "+idpejabat);
			...
			if (idjenispejabat.equals("22")) {
				// dropdown pejabat jkptg
				if (idPejabatJKPTG != "") {
					//myLogger.info("*****Read here1a****");
					if (idpejabat != "") {
						//myLogger.info("*****Read here1****");
						context.put("SELECTBicara",PPKUtilHTML.SelectTempatBicaraByPejabatJKPTG(
								usid,idPejabatJKPTG,"editTempatBicara",Utils.parseLong(idpejabat),null,"style=width:400 onChange=\"doChangeidTempatBicaraUPDATE();\" "));

					} else {
						//myLogger.info("*****Read here2****");
						//context.put("SELECTBicara",HTML.SelectTempatBicaraByPejabatJKPTG(
						//		idPejabatJKPTG,"editTempatBicara",null, null,"style=width:400 onChange=\"doChangeidTempatBicaraUPDATE();\" "));
						context.put("SELECTBicara",PPKUtilHTML.SelectTempatBicaraByPejabatJKPTG(
								usid,idPejabatJKPTG,"editTempatBicara",null,null,"style=width:400 onChange=\"doChangeidTempatBicaraUPDATE();\" "));
					...
					if (idpejabat != "") {
						...
						context.put("SELECTBicara",HTML.SelectTempatBicara(
								"editTempatBicara",null, null,"style=width:400 onChange=\"doChangeidTempatBicaraUPDATE();\" "));
					}
				}
				...
				if (idPejabatJKPTG != "") {
					if (idpejabat != "") {
//						context.put("SELECTBicara",HTML.SelectTempatBicaraByPejabatJKPTG(
//								idPejabatJKPTG,"editTempatBicara",Utils.parseLong(idpejabat),null,"style=width:400 onChange=\"doChangeidTempatBicaraUPDATE();\" "));
						context.put("SELECTBicara",PPKUtilHTML.SelectTempatBicaraByPejabatJKPTG(
								usid,idPejabatJKPTG,"editTempatBicara",Utils.parseLong(idpejabat), null,"style=width:400 onChange=\"doChangeidTempatBicaraUPDATE();\" "));

					} else {
						context.put("SELECTBicara",PPKUtilHTML.SelectTempatBicaraByPejabatJKPTG(
								usid,idPejabatJKPTG,"editTempatBicara",Utils.parseLong(idPejabatJKPTG), null,"style=width:400 onChange=\"doChangeidTempatBicaraUPDATE();\" "));
					}
				} else {
					if (idpejabat != "") {
						context.put("SELECTBicara",HTML.SelectTempatBicara(
				...
				// radio button
				context.put("checkP1", "checked=checked");
				context.put("checkP2", "");
				context.put("checkP3", "");
			}
			...
			//aishahlatip15052017 : untuk dapatkan samaada negeri cuti am pada jumaat sabtu atau sabtu ahad
			if (id_negeri != "") {
				flag_cuti = modelNotis.getFlagCuti(id_negeri);
			}
			...
			jenispejabat = getParam("editjenisPejabat");
			myLogger.info("onchangeJenisPejabatUpdate:jenispejabat="+jenispejabat);
			if (jenispejabat.equals("1")) {
				// dropdown pejabat jkptg
				if (idPejabatJKPTG != "") {
//					context.put("SELECTBicara",HTML.SelectTempatBicaraByPejabatJKPTG(
...
//							usid,idPejabatJKPTG,"editTempatBicara",null, null,"style=width:400 onChange=\"doChangeidTempatBicaraUPDATE();\" "));
				...
				// dropdown pejabat tanah
				myLogger.info("onchangeJenisPejabatUpdate:idPejabatJKPTG="+idPejabatJKPTG);
				if (idPejabatJKPTG != "") {
					context.put("SELECTBicara",HTML.SelectPejabatTanahByJKPTG(
													idPejabatJKPTG,
													"editTempatBicara", 
...
//													"style=width:400 onChange=\"doChangeidTempatBicaraUPDATE();\" "));
			...
			context.put("SELECTNegeri", HTML.SelectNegeri("editNegeri", null,negeriReadonly));
			...
			if (idNegeri != "") {
				...
				if (idpsk != "") {
					context.put("SELECTPegawai", HTML.SelectPegawaiPengendaliByNegeri(idNegeri,
									"editPegawai", 
									Utils.parseLong(idPegawai),
									"style=width:305"));
				} else {
					context.put("SELECTPegawai", HTML.SelectPegawaiPengendaliByNegeri(idNegeri,
									"editPegawai", 
				...
				if (idpsk != "") {
					context.put("SELECTPegawai", HTML.SelectPegawaiPengendali(
							"editPegawai", Utils.parseLong(idPegawai),"style=width:305"));
				} else {
					context.put("SELECTPegawai", HTML.SelectPegawaiPengendali(
							"editPegawai", null, "style=width:305"));
				}
...
//														"style=width:400 class=disabled disabled "));
				...
				if (idPejabatJKPTG != "") {
					if (idpejabat != "") {
						context.put("showBicara",HTML.SelectPejabatTanahByJKPTG(
								idPejabatJKPTG,"editTempatBicara"
								,Utils.parseLong(idpejabat),null
								,"style=width:400 class=disabled disabled "));
...
//														"style=width:400 class=disabled disabled "));
			...
			modelNotis.setSelectedDropdown(id_permohonansimati, idob, idSimati,
					"8");
			...
			if ("doChangeSorPenjagaOB".equals(submit2)) {
				...
				// if(socPenjagaOB!="")
				// {
				// POB = Integer.parseInt(socPenjagaOB);
				// }

				if (socPenjagaOB.isEmpty() || socPenjagaOB == "0") {
			...
			//
			// String kpbaru = a+b+c;
			// String kplama = getParam("txtNoKPLama");
			// String kplain = getParam("txtJenisKP");

			// if (modelNotis.checkKPPenjaga(id_obminor, kpbaru, kplama,kplain)
			// == true) {
			//
			// context.put("duplicate", "yes");
			//
			// }else{
			...
			selectedOB = modelNotis.getSelectedOB();
			System.out.println("::::::::::::::::selectedOB::::::::::::::::"+selectedOB.size());
			// list maklumat penjaga
			modelNotis.setMaklumatPenjaga(id_permohonansimati, idSimati);
			listMaklumatPenjaga = modelNotis.getMaklumatPenjaga();
			//myLogger.info("idPejabatJKPTG="+idPejabatJKPTG);
			...
			myLogger.info("4469"+penghantarNotisByJkptg);
			myLogger.info("4469"+penghantarNotis);
			...
			System.out.println("selectedOB.size()===="+selectedOB.size());
			...
			if ("onchangemyList".equals(submit2)) {
				...
				// get list ob
				modelNotis.setListOBatas18Semua(id_permohonansimati, idSimati,
						"8");
				listOBatas18 = modelNotis.getListOBatas18Semua();
				context.put("listOBatas18", listOBatas18);
...
System.out.println("idPejabatJKPTG=="+idPejabatJKPTG);
			if (idPejabatJKPTG != "") {
				if (penghantarNotisByJkptg.size() != 0) {
					context.put("dataPenghantarNotis",getSelectedPenghantarNotisByJkptg);
				} else {
			...
			if ("kemaskiniNotis".equals(submit2)) {
				...
				if ("updateLaporan".equals(submit3)) {
					...
					// get list ob
					modelNotis.setListOBatas18Semua(id_permohonansimati,
							idSimati, "8");
					listOBatas18 = modelNotis.getListOBatas18Semua();
					context.put("listOBatas18", listOBatas18);
			...
			carianNotis(session);
			listdepan = modelNotis.getListCarian();

			// setupPage(session,action,listCari);
		...
		else if ("Cari2".equals(submit)) {
			myLogger.info("*&*&*&*&cari2*&*&*&*&*");
			//carianNotis(session);
			carianNotis2(session,commands);
			listdepan = modelNotis.getListCarian();
			...
			context.put("listNotis", listdepan);
			context.put("list_size", listdepan.size());
			context.put("commands", "Testing123");
		...
		// this.context.put("flagFromMyJsp", flagFromMyJsp);
		...
		myLogger.info(" idpermohonansimati : " + id_permohonansimati);
	...
	private void carianNotis(HttpSession session) throws Exception {
		...
		modelNotis.setCarianFail(noFail, namaPemohon, namaSimati, icSimati,
				JenisIc, usid, statusFail);

	}// close carian
	
	
	public String carianNotis2(HttpSession session, String commands) throws Exception {
...
/*		
	...
	private void simpanNotisTambah(HttpSession session) throws Exception {
		...
		String idPerbicaraan = modelNotis.addNotisTambah(session, h);
	...
	private void simpanLaporan(HttpSession session) throws Exception {
		...
		System.out.println("daftar_pos1===="+daftar_pos1);
		...
		//System.out.println("tarikhHantar=="+tarikhHantar);
		//System.out.println("alamatEmel=="+alamatEmel);
		String lain2PeghantarNotis = getParam("lain2PeghantarNotis");
		...
		System.out.println("cbsemaks.length==="+cbsemaks.length);
		...
		//hantar emel
		//aishahlatip amik code wp
		//jenis serahan by email = 3
		
		//aishahlatip close 23022018 atas arahan pn syaida kerana ada kes peguam dapat email tersebut. 
		//sepatutnya send email pada skrin notis perbicaraan shj. skrin ni hanya simpan jenis laporan penghantaran notis
	...
	private void simpanTambahanLaporan(HttpSession session) throws Exception {
		...
		h.put("nama_penghantar", nama);
		h.put("id_penghantar", id_penghantarnotis);
		h.put("namaLain", namaLain);//aishah
