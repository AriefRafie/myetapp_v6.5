...
public class FrmPerintahSek17 extends AjaxBasedModule {
	...
	static Logger myLogger = Logger.getLogger(FrmPerintahSek17.class);
	...
	public String doTemplate2() throws Exception {
		...
		String flagSelesaiHTATH = getParam("flagSelesaiHTATH");
		...
		Vector listHTAPKTTerdahulu = new Vector();
		Vector listHAPKTTerdahulu = new Vector();

		listHTAPKTTerdahulu.clear();
		listHAPKTTerdahulu.clear();
		
		Vector listHTAPKTTerdahuluBB = new Vector();
		Vector listHAPKTTerdahuluBB = new Vector();

		listHTAPKTTerdahuluBB.clear();
		listHAPKTTerdahuluBB.clear();
		
		Vector listHTAPKTTerdahuluBL = new Vector();
		Vector listHAPKTTerdahuluBL = new Vector();

		listHTAPKTTerdahuluBB.clear();
		listHAPKTTerdahuluBB.clear();
		
		Vector listHTAPATerdahulu = new Vector();
		Vector listHAPATerdahulu = new Vector();

		listHTAPATerdahulu.clear();
		listHAPATerdahulu.clear();
		
		
		Vector listHTAduluPerintah = new Vector();
		listHTAduluPerintah.clear();
		Vector listOBHTAdulu = new Vector();
		listOBHTAdulu.clear();
		Vector listHadulu = new Vector();
		listHadulu.clear();
		Vector listOBHAdulu = new Vector();
		listOBHAdulu.clear();
		Vector check_pilihan_ha = new Vector();
		check_pilihan_ha.clear();
		Vector check_pilihan_ha_ob = new Vector();
		check_pilihan_ha_ob.clear();
		Vector check_pilihan = new Vector();
		check_pilihan.clear();
		Vector check_pilihan_hta_ob = new Vector();
		...
		if (hitButt.equals("SimpanSerahan")) {
			...
			String txtCatatan = getParam("txtCatatanSerahan");
	    ...
	    //SCREEN NAVIGATION
		System.out.println("-------actionPerintah1------" +actionPerintah);
		if (actionPerintah.equals("papar")){
			System.out.println("-------actionPerintah2------" +actionPerintah);
			vm = "app/ppk/frmPerintahMaklumatPerintahSek17.jsp";
			this.context.put("actionPerintah","papar");
			if (noFail1 != "")
			{
			noFail2 = noFail1;
			}
		paparLaporanSerahan(noFail2);
			...
			if (hitButt.equals("onchangemyList")) {
				...
				String catatanSerahan = getParam("txtCatatanSerahan");
				...
				String kplama = "";
				String kplain = "";
				System.out.println("-------catatanSerahan-------" + catatanSerahan);
				...
				System.out.println("-------TARIKH_SERAHAN-------" + TARIKH_SERAHAN);
				System.out.println("-------nama_penerima-------" + nama_penerima);
				System.out.println("-------catatanSerahan-------" + catatanSerahan);
				context.put("NAMA_PENYERAH", penyerah);
				context.put("TARIKH_SERAHAN", TARIKH_SERAHAN);
				context.put("catatanSerahan", catatanSerahan);
				context.put("SimpanStatus", "1");
				context.put("JENIS_PENGHANTARAN", radioJenis);
        	...
        	if (flagPopup.equals("openHTAPT")){
    			...
    			if (logic.getBeanHeaderDetail().size() != 0){
					Hashtable hash = (Hashtable) logic.getBeanHeaderDetail().get(0);					
...
//				}
			...
			if (flagAdaHTA.equals("1")){
				...
				//CHECK ALL HTATH YANG BELUM DIDAFTARKAN
				if (logic.checkHTATHYangBelumDibahagikan(idPermohonanSimati, idPerintah)){
					flagSelesaiHTATH = "";
				} else {
					flagSelesaiHTATH = "Y";
				}
...
//			}
			...
			vm = "app/ppk/frmSenaraiFailPerintahSek17.jsp";
	...
	private void clearBatalLantik(String jenis_harta,HttpSession session,String idPerintah,String idPermohonanSimati,String batalPT,String lantikPT,String batalPA,String lantikPA) throws Exception
	{
		...
		{
			Vector listHTAduluPerintah_Pilihan = logic.setDataHTAduluPerintah_Pilihan(idPerintah,idPermohonanSimati,batalPT,lantikPT,batalPA,lantikPT);
			
			for(int i=0; i < listHTAduluPerintah_Pilihan.size(); i++)
			{
					Hashtable h = (Hashtable) listHTAduluPerintah_Pilihan.get(i);
		...
		{
			Vector listDataHadulu_Pilihan = logic.setDataHaDulu_Pilihan(idPerintah,idPermohonanSimati,batalPT,lantikPT,batalPA,lantikPT);
			//clear rekod HA balik
			for(int i=0; i < listDataHadulu_Pilihan.size(); i++)
			{
					Hashtable h = (Hashtable) listDataHadulu_Pilihan.get(i);
	...
	//Salnizam starts
			...
			try {
				...
				if (rs != null || "".equals(rs))
				{
					...
					this.context.put("catatanSerahan", "");
