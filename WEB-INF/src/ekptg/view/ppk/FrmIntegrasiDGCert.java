...
public class FrmIntegrasiDGCert extends VTemplate {
	/**
	 * 
	 */
	private static final long serialVersionUID = -7706747181593017208L;
	static Logger myLogger = Logger.getLogger(FrmIntegrasiDGCert.class);
	private String usid = "";
...
//			String user = (String) session.getAttribute("_ekptg_user_id");
			...
			keputusanPermohonan = FrmPrmhnnSek8Notis.getKeputusanPermohonan(id);

			Hashtable<String,String> kp = new Hashtable<String,String> ();
			String idkp = "";

			if (keputusanPermohonan.size() != 0) {
				kp = (Hashtable<String,String> ) keputusanPermohonan.get(0);
			...
			if (list.size() != 0) {
				Hashtable<String,String>  ls = (Hashtable<String,String> ) list.get(0);
			...
			// --data notis
			FrmPrmhnnSek8Notis.setListSemakWithData(idkp);
			dataNotis = FrmPrmhnnSek8Notis.getListSemakWithData();
					
			String bil = "";
			int _bil = 0;

			if (dataNotis.size() != 0) {
				Hashtable<String,String> idn = (Hashtable<String,String>) dataNotis.get(0);
			...
			Hashtable<String,String> idn = new Hashtable<String,String>();

			if (dataNotis.size() != 0) {
				idn = (Hashtable<String,String>) dataNotis.get(0);
...
//			String idJKPTG = "";
			...
			if (idPejabatJKPTG != "") {
				...
				if (alamatTempatBicara.size() != 0) {
					Hashtable<String,String> AB = (Hashtable<String,String>) alamatTempatBicara.get(0);
			...
			if(idjenispejabat.equals("22")){
				if (idPejabatJKPTG != "") {
					context.put("selectBicara",HTML.SelectTempatBicaraByPejabatJKPTG(idPejabatJKPTG
									,"socTempatBicara",Utils.parseLong(idPejabatJKPTG),null,
									"style=width:400 onChange=\"doChangeidTempatBicara();\" class=disabled disabled "));
			} else {
				context.put("selectBicara",HTML.SelectTempatBicara("socTempatBicara"
										,null, null,"style=width:400 onChange=\"doChangeidTempatBicara();\" class=disabled disabled "));
			}
				
			context.put("previousBil", _bil);
			
				if (idpejabat != "") {
					context.put("showBicara", HTML.SelectTempatBicara("editTempatBicara"
						, Utils.parseLong(idpejabat),null,"style=width:400 onChange=\"doChangeidTempatBicara();\" class=disabled disabled "));
				} else {
					context.put("showBicara", HTML.SelectTempatBicara("editTempatBicara"
							, null, null,"style=width:400 onChange=\"doChangeidTempatBicara();\" class=disabled disabled "));
				}
...
//				myLogger.info("idpejabat= "+idpejabat);
				if (idPejabatJKPTG != "") {
					if (idpejabat != "") {
						context.put("selectBicara",HTML.SelectPejabatTanahByJKPTG(idPejabatJKPTG,
														"editTempatBicara",
					...
					if (idpejabat != "") {
						context.put("selectBicara", HTML.SelectPejabatTanah("editTempatBicara"
				...
				if (idPejabatJKPTG != "") {
					...
					if (idpejabat != "") {
						context.put("selectBicara", HTML.SelectTempatBicara("editTempatBicara"
							, Utils.parseLong(idpejabat),null,"style=width:400 class=disabled disabled  "));
					} else {
						context.put("selectBicara", HTML.SelectTempatBicara("editTempatBicara"
							, null, null,"style=width:400 class=disabled disabled  "));
					}
				}
			...
			vm = "app/ppk/integrasi/DGCert.jsp";
//			myLogger.info("vm ::"+vm);	
...
//			String user = (String) session.getAttribute("_ekptg_user_id");
			...
			if (negeri != "") {
				context.put("selectNegeri", HTML.SelectNegeri("socNegeri",
						Utils.parseLong(negeri),"class=disabled disabled style=width:305"));
			} else {
				context.put("selectNegeri", HTML.SelectNegeri("socNegeri",
						null, "class=disabled disabled style=width:305"));
			}
...
//				myLogger.info("idpejabat= "+idpejabat);
				if (idPejabatJKPTG != "") {
					...
					if (idpejabat != "") {
						context.put("selectBicara", HTML.SelectPejabatTanah("editTempatBicara", Utils.parseLong(idpejabat),null,
								"style=width:400 onChange=\"doChangeidTempatBicara();\" class=disabled disabled  "));
					} else {
						context.put("selectBicara", HTML.SelectPejabatTanah("editTempatBicara", null, null,
								"style=width:400 onChange=\"doChangeidTempatBicara();\" class=disabled disabled "));
					}
				}
				// radio button
				...
				context.put("selectBicara",
							"<input type='text' name='editTempatBicara' style='text-transform:uppercase;' onBlur='this.value=this.value.toUpperCase()' size='52'  maxlength='60' value='"
										+ tempatBicara+ "' class='disabled' readonly>");
			...
			//GET SIGNEDDATA
			String dataDahSign = modelNotis.getSignedData(idperbicaraan);
//			System.out.println("dataDahSign    "+dataDahSign);
			context.put("dataDahSign", dataDahSign);

			vm = "app/ppk/integrasi/verifyDGCert.jsp";
//			myLogger.info("vm ::"+vm);
...
//			System.out.println("signedText::::: "+signedText);
			...
			try {
				...
				con.commit();		
				//aishah add untuk pindah dr table TEMP ke table real
				FrmPrmhnnSek8Notis.insertPenghantaranNotisReal(id_perbicaraan,id_fail,session,application,request,response);				
...
//			context.put("id_fail", idfail);	
	...
	public static Vector<Hashtable<String,String>> getAlamatTempatBicaraIntegrasi(String idBicara) throws Exception {
		Db db = null;
		String sql = "";
		try {
			db = new Db();
