...
public class FrmPrmhnnSek17Senarai extends VTemplate {
	IdentityManager identity = new IdentityManager();
	MyIdentityManager myIdentity = new MyIdentityManager();
	...
	public Template doTemplate() throws Exception {
		myLogger.debug(":FrmPrmhnnSek17Senarai:");
		...
		// + getParam("flagFromSenaraiPermohonanSek8"));
		/*
		...
		*/
		...
		/*
		 * Vector list = new Vector();list.clear(); Vector listSimati = new
		 * Vector();listSimati.clear(); Vector listPemohon = new
		 * Vector();listPemohon.clear(); Vector listPemohonOB = new
		 * Vector();listPemohonOB.clear(); Vector listCheckPeguam = new
		 * Vector();listCheckPeguam.clear(); Vector listPeguam = new
		 * Vector();listPeguam.clear(); Vector listWaris = new
		 * Vector();listWaris.clear(); Vector listWarisLapisanIdMati = new
		 * Vector();listWarisLapisanIdMati.clear(); Vector listWarisOB = new
		 * Vector();listWarisOB.clear();
		 */
		...
		this.context.put("check_pilihan_hta_ob", "");
		...
		if ("papar".equals(submit)) {
			...
			// FrmPrmhnnSek8DaftarSek8InternalData.setData(idp); 
			...
			String baru_id = getParam("idpermohonan");
			if (baru_id != "") {
			...
			jenisTab = "capaianWaris";
			vm = capaianIntegrasi(session, jenisTab, vm);  

		} else if ("capaianIdentityPemohon".equals(submit)) {
			
			jenisTab = "capaianPemohon";
			vm = capaianIntegrasi(session, jenisTab, vm);   
		} else if ("capaianIdentitySaksi".equals(submit)) {
			
			jenisTab = "capaianSaksi";
			vm = capaianIntegrasi(session, jenisTab, vm);  

		} else if ("capaianIdentityOB".equals(submit)) {
			...
			this.context.put("listsemakhta", listsemakhta);
			listsemakha = logic.setTujuanSemak_ha(getParam("idtempTerdahulu"));
			this.context.put("listsemakha", listsemakha);
			...
			System.out.println("Dalam FrmPrmhnnSek17Senarai2.java");
			String eventstatus = getParam("eventStatus");
			...
			if (eventStatus == 0) {
				...
				//String[] cbsemaks20 = this.request.getParameterValues("cbsemaks");
				FrmPrmhnnSek8SenaraiSemakInternalData frmPrmhnnSek8SenaraiSemakData = new FrmPrmhnnSek8SenaraiSemakInternalData();
				if (cbsemaks != null) {
					System.out.println("READ HERE******");
					for (int i = 0; i < cbsemaks.length; i++) {
						...
						if (cbsemaks[i].equals("9")) {
							...
							System.out.println("txtLainLainTujuan17 " + txtbox);
						}
						
						
						if (cbsemaks[i].equals("8")) {
						...
						if (bolehsimpan.equals("yes")) {
							System.out.println("semakanAdd17");
							frmPrmhnnSek8SenaraiSemakData.semakanAdd17(
				...
				String[] cbsemaks = this.request.getParameterValues("cbsemaks");
				//String cbsemaks20 = getParam("cbsemaks20");
				System.out.println("cbsemaks = " + cbsemaks);
				//String txtbox2 = getParam("txtLainLainDokumen");
				FrmPrmhnnSek8SenaraiSemakInternalData frmPrmhnnSek8SenaraiSemakData = new FrmPrmhnnSek8SenaraiSemakInternalData();
				...
				this.context.put("ht", ht);
				this.context.put("bpa", bpa);
				this.context.put("lpa", lpa);
				this.context.put("bkp", bkp);
				this.context.put("lp", lp);
				this.context.put("lt", lt);
				...
				if (cbsemaks != null) {
					for (int i = 0; i < cbsemaks.length; i++) {
						...
						} else if (cbsemaks[i].equals("10")) {
							ht = "Y";
							this.context.put("ht", ht);
						}else if (cbsemaks[i].equals("12")) {
							bpa = "Y";
							this.context.put("bpa", bpa);
						}else if (cbsemaks[i].equals("13")) {
							lpa = "Y";
							this.context.put("lpa", lpa);
						}else if (cbsemaks[i].equals("15")) {
							bkp = "Y";
							this.context.put("bkp", bkp);
						}else if (cbsemaks[i].equals("16")) {
							lp = "Y";
							this.context.put("lp", lp);	
						}
						//} else if (cbsemaks[i].equals("17")) {
						...
						}else if (cbsemaks[i].equals("18")) {
							txtbox = getParam("txtLainLain");
						}
						/*
						 * else if (cbsemaks[i].equals("10")){ //harta
						 * tertinggal if(bolehsimpan.equals("yes")){
						 * logic_C.semakanUpdateTujuanHT
						 * (String.valueOf(tempid),(
						 * String)session.getAttribute("_ekptg_user_id")); } }
						 * else if (cbsemaks[i].equals("13")){
						 * if(bolehsimpan.equals("yes")){
						 * logic_C.semakanUpdateTujuanLPA
						 * (String.valueOf(tempid),
						 * (String)session.getAttribute("_ekptg_user_id")); } }
						 * else if (cbsemaks[i].equals("12")){
						 * if(bolehsimpan.equals("yes")){
						 * logic_C.semakanUpdateTujuanBPA
						 * (String.valueOf(tempid),
						 * (String)session.getAttribute("_ekptg_user_id")); } }
						 * else if (cbsemaks[i].equals("16")){
						 * if(bolehsimpan.equals("yes")){
						 * logic_C.semakanUpdateTujuanLP
						 * (String.valueOf(tempid),(
						 * String)session.getAttribute("_ekptg_user_id")); } }
						 * else if (cbsemaks[i].equals("15")){
						 * if(bolehsimpan.equals("yes")){
						 * logic_C.semakanUpdateTujuanBKP
						 * (String.valueOf(tempid),
						 * (String)session.getAttribute("_ekptg_user_id")); } }
						 */
						if (bolehsimpan.equals("yes")) {
							System.out.println("semakanUpdate17");
			...
			// System.out.println("txtNomborResitH:" + txtNomborResitH);
			this.context.put("txtNomborResitH", txtNomborResitH);
			String txdTarikhByrnH = getParam("txdTarikhByrnH");
			// System.out.println("txdTarikhByrnH:" + txdTarikhByrnH);
			this.context.put("txdTarikhByrnH", txdTarikhByrnH);
			chkId = logic_A.getId();
			/*
			...
			// System.out.println("ID_BANDAR:" + h.get("idbandar").toString());
...
/*
			...
			if (cntID == 0) {
				/*
				 * addPermohonan(session,userIdNeg,userIdPejabat,userIdKodDaerah,
				 * userIdKodNegeri); this.context.put("tarikhmohondaftar", "");
				 */
			} else {
				if (bolehsimpan.equals("yes")) {
			...
			*/
			...
			// FrmPrmhnnSek8DaftarSek8InternalData.setData(idPermohonand);
			...
			list2 = logic_A.setData(idPermohonan, (String) session
					.getAttribute("_ekptg_user_id"));
			this.context.put("View", list2);
			headerppk_baru(session, idPermohonan, "Y", "", "T");
			...
			listsemakhta = logic.setTujuanSemak_hta(idPermohonand);
			this.context.put("listsemakhta", listsemakhta);
			listsemakha = logic.setTujuanSemak_ha(idPermohonand);
			this.context.put("listsemakha", listsemakha);
			...
			if ("Pemohonview".equals(mode)) {
				...
				logic.setCheckPeguam17(id);
				listCheckPeguam = logic.getCheckPeguam17();
		...
		/*
		 * 
		 * else if("Waris".equals(submit)) { readability1 = ""; readability2 =
		 * "readonly"; disability1 = "disabled"; disability2 = ""; String
		 * check_copy = getParam("copyAlamat"); this.context.put("check_copy",
		 * check_copy); this.context.put("Tambah_lapisan_berikut","");
		 ...
		 * this.context.put("listBandarTetapbyNegeri","");
		 * this.context.put("listBandarSuratbyNegeri",""); } else if
		 * ("Waristarikh".equals(mode)) { this.context.put("listWarisUpdate",
		 * ""); this.context.put("listWarisLapisanUpdate", ""); String key="";
		 * String value=""; Enumeration allparam = request.getParameterNames();
		 * for (; allparam.hasMoreElements(); ) { key =
		 * (String)allparam.nextElement(); value = request.getParameter(key);
		 * this.context.put(key, value); }
		 * if(getParam("checkHidupWaris").equals("")) {
		 * this.context.put("checkHidupWaris","0");
		 * this.context.put("show_tarikh","");
		 * this.context.put("txdTarikhMatiWaris","");
		 * this.context.put("txtWaktuKematianWaris",""); }else {
		 ...
		 * getParam("socNegeriWarisSurat")!="0"){ Vector s3 =
		 ...
		 * getParam("socNegeriWaris")!="0"){ Vector s3 =
		 * logic_A.getListBandarByNegeri
		 * (Integer.parseInt(getParam("socNegeriWaris")));
		 * this.context.put("listBandarTetapbyNegeri",s3);}
		 ...
		 * this.context.put("show_senarai_lapis_pertama","yes");
		 * this.context.put("show_tambah_waris1","yes"); }
		 * if(getParam("up_in").equals("in_lap")){ int
		 * idparent=Integer.parseInt(getParam("idparentlapis"));
		 * logic.setDataWarisLapisan(idparent); listWarisLapisan =
		 * logic.getDataWarisLapisan();
		 * this.context.put("listWarisLapisan",listWarisLapisan);
		 * logic.setDataWarisParent(idparent); listWarisParent =
		 * logic.getDataWarisParent();
		 ...
		 * 
		 * int idparent=Integer.parseInt(getParam("idparentlapis"));
		 * logic.setDataWarisLapisan(idparent); listWarisLapisan =
		 * logic.getDataWarisLapisan();
		 * this.context.put("listWarisLapisan",listWarisLapisan);
		 * logic.setDataWarisParent(idparent); listWarisParent =
		 ...
		 * this.context.put("listWarisLapisanUpdate", ""); String key=""; String
		 * value=""; Enumeration allparam = request.getParameterNames(); for (;
		 * allparam.hasMoreElements(); ) { key = (String)allparam.nextElement();
		 * value = request.getParameter(key); this.context.put(key, value); }
		 * if(getParam("checkHidupWaris").equals("")) {
		 * this.context.put("checkHidupWaris","0");
		 * this.context.put("show_tarikh",""); }else {
		 * this.context.put("checkHidupWaris","1");
		 * this.context.put("show_tarikh","yes"); }
		 * 
		 ...
		 * logic_A.getListBandarByNegeri
		 * (Integer.parseInt(getParam("socNegeriWaris")));
		 * this.context.put("listBandarTetapbyNegeri",s3);} else{
		 * this.context.put("listBandarTetapbyNegeri","");}
		 * this.context.put("listBandarSuratbyNegeri",""); }
		 * if(getParam("up_in").equals("in")){
		 * this.context.put("show_batal_waris","yes");
		 ...
		 * this.context.put("buttonwarislapisan","Simpan"); }
		 * 
		 * } else if ("Getwaris".equals(mode)) { int
		 * idwaris=Integer.parseInt(getParam("idwaris"));
		 * logic.setDataWarisUpdate(idwaris); listWarisUpdate =
		 * logic.getDataWarisUpdate();
		 * this.context.put("listWarisUpdate",listWarisUpdate);
		 * 
		 * Hashtable h = (Hashtable)listWarisUpdate.get(0);
		 * if(h.get("idnegeriSurat").toString()!="" &&
		 * h.get("idnegeriSurat").toString()!="0"){ Vector s3 =
		 * logic_A.getListBandarByNegeri
		 * (Integer.parseInt(h.get("idnegeriSurat").toString()));
		 * this.context.put("listBandarSuratbyNegeri",s3);}
		 * else{this.context.put("listBandarSuratbyNegeri","");}
		 * 
		 * if(h.get("idnegeri").toString()!="" &&
		 ...
		 * logic_A.getListBandarByNegeri
		 * (Integer.parseInt(h.get("idnegeri").toString()));
		 * this.context.put("listBandarTetapbyNegeri",s3);}
		 * else{this.context.put("listBandarTetapbyNegeri","");}
		 * 
		 * logic.setDataWarisLapisanIdMatiDelete(idwaris);
		 * listWarisLapisanIdMatiDelete =
		 * logic.getDataWarisLapisanIdMatiDelete();
		 ...
		 * this.context.put("show_senarai_lapis_pertama","yes");
		 * this.context.put("show_tambah_waris1","yes"); } else if
		 * ("Lapisan_berikut".equals(mode)) { int
		 * idwaris=Integer.parseInt(getParam("idwarisup"));
		 * this.context.put("idparent",idwaris); this.context.put("ip",idwaris);
		 * logic.setDataWarisLapisan(idwaris); listWarisLapisan =
		 * logic.getDataWarisLapisan();
		 * this.context.put("listWarisLapisan",listWarisLapisan);
		 * logic.setDataWarisParent(idwaris); listWarisParent =
		 * logic.getDataWarisParent(); this.context.put("readmode","");
		 * this.context.put("listWa",listWarisParent);
		 * this.context.put("show_lapisan_berikut","yes");
		 * this.context.put("nk_tambah_lapisan","yes"); } else if
		 * ("Lapisan_berikut_lapisan".equals(mode)) { int
		 * idwaris=Integer.parseInt(getParam("idwarisup"));
		 * this.context.put("ip",idwaris); logic.setDataWarisLapisan(idwaris);
		 * logic.setDataWarisParent(idwaris); listWarisParent =
		 * logic.getDataWarisParent();
		 * this.context.put("listWarisParent",listWarisParent); listWarisLapisan
		 * = logic.getDataWarisLapisan();
		 * this.context.put("listWarisLapisan",listWarisLapisan);
		 * logic.setDataWarisParent(idwaris); listWarisParent =
		 * logic.getDataWarisParent();
		 ...
		 * this.context.put("nk_tambah_lapisan","yes");
		 * 
		 * } else if ("tambah_waris_lapisan".equals(mode)) { int
		 * idparent=Integer.parseInt(getParam("idparentlapis")); int idwaris=
		 * Integer.parseInt(getParam("idwarisup"));
		 * if(bolehsimpan.equals("yes")){ addWarisBerikut(session); }
		 * logic.setDataWarisLapisan(idparent); listWarisLapisan =
		 * logic.getDataWarisLapisan();
		 * this.context.put("listWarisLapisan",listWarisLapisan);
		 * 
		 * if(getParam("checkHidupWaris").equals("")) {
		 * this.context.put("show_lapisan_waris",""); }else {
		 * this.context.put("show_lapisan_waris","yes"); }
		 * logic.setDataWarisParent(idparent); listWarisParent =
		 * logic.getDataWarisParent();
		 * this.context.put("listWarisParent",listWarisParent);
		 * logic.setDataWaris(idwaris); listWaris = logic.getDataWaris();
		 ...
		 * this.context.put("readmode",disability1); int
		 * idwaris=Integer.parseInt(getParam("idwarislapis")); int
		 * idparent=Integer.parseInt(getParam("idparentlapis"));
		 * logic.setDataWarisLapisan(idparent); listWarisLapisan =
		 * logic.getDataWarisLapisan(); logic.setDataWarisUpdate(idwaris);
		 ...
		 * 
		 * 
		 * } else if ("hapus_waris_lapisan".equals(mode)) { int
		 * idwarisup=Integer.parseInt(getParam("idwarisup"));
		 * logic.deleteWarisHubungan(idwarisup); logic.deleteWaris(idwarisup);
		 * int idparent=Integer.parseInt(getParam("idparentlapis"));
		 ...
		 * {this.context.put("show_lapisan_waris",""); }else
		 * {this.context.put("show_lapisan_waris","yes"); }
		 * logic.setDataWarisOB(); listWarisOB = logic.getDataWarisOB();
		 * this.context.put("listWarisOB",listWarisOB);
		 * 
		 * this.context.put("show_lapisan_berikut","yes");
		 * this.context.put("nk_tambah_lapisan","yes");
		 * 
		 * } else if ("GetwarisLapisanX".equals(mode)) { int
		 * idwaris=Integer.parseInt(getParam("idwarislapisX")); int
		 * idparent=Integer.parseInt(getParam("idparentlapisX"));
		 * logic.setDataWarisLapisan(idparent); listWarisLapisan =
		 * logic.getDataWarisLapisan();
		 * this.context.put("listWarisLapisan",listWarisLapisan);
		 * logic.setDataWarisUpdate(idwaris); listWarisUpdate=
		 ...
		 * logic.getDataWarisLapisanIdMatiDelete();
		 * this.context.put("listWarisLapisanIdMatiDelete"
		 * ,listWarisLapisanIdMatiDelete); logic.setDataWarisParent(idparent);
		 ...
		 * this.context.put("show_lapisan_berikut","yes");
		 * this.context.put("buttonwarislapisan","Kemaskini");
		 * this.context.put("readmode","disabled"); } else if
		 * ("Tambah_lapisan_berikut".equals(mode)) { listWarisup = new Vector();
		 * int idp=Integer.parseInt(getParam("idparentlapis")); Hashtable h =
		 * new Hashtable(); h.put("statushidup","0"); listWarisup.addElement(h);
		 * this.context.put("listWarisLapisanUpdate",listWarisup);
		 * 
		 * logic.setDataWarisParent(idp); listWarisParent =
		 * logic.getDataWarisParent();
		 * this.context.put("listWarisParent",listWarisParent);
		 * logic.setDataWarisLapisan(idp); listWarisLapisan =
		 * logic.getDataWarisLapisan();
		 * this.context.put("listWarisLapisan",listWarisLapisan);
		 * this.context.put("addnew","yes");
		 * this.context.put("show_lapisan_berikut_tambah","yes");
		 ...
		 * if(bolehsimpan.equals("yes")){ addWaris(session); }
		 * if(getParam("checkHidupWaris").equals(""))
		 * {this.context.put("show_lapisan_waris","");
		 * }else{this.context.put("show_lapisan_waris","yes"); }
		 * logic.setDataWarisOB(); listWarisOB = logic.getDataWarisOB();
		 * this.context.put("listWarisOB",listWarisOB);
		 * this.context.put("show_senarai_lapis_pertama","yes");
		 * this.context.put("show_lapisan_bawah","yes");
		 * this.context.put("show_tambah_waris1","yes");
		 * this.context.put("button_Kembali1","yes"); } else if
		 * ("hapus_waris".equals(mode)) { int
		 * idwarisup=Integer.parseInt(getParam("idwarisup"));
		 * logic.deleteWaris(idwarisup);
		 * if(getParam("checkHidupWaris").equals(""))
		 * {this.context.put("show_lapisan_waris",""); }else
		 * {this.context.put("show_lapisan_waris","yes"); }
		 * logic.setDataWarisOB(); listWarisOB = logic.getDataWarisOB();
		 ...
		 * this.context.put("show_lapisan_bawah","yes");
		 * this.context.put("show_tambah_waris1","yes");
		 * this.context.put("button_Kembali1","yes"); } else
		 * if("kemaskini_waris".equals(mode)){ int idwaris =
		 * Integer.parseInt(getParam("idwarisup"));
		 * logic.setDataWarisUpdate(idwaris); listWarisUpdate=
		 * logic.getDataWarisUpdate();
		 * this.context.put("listWarisUpdate",listWarisUpdate); Hashtable h =
		 * (Hashtable)listWarisUpdate.get(0);
		 * if(h.get("idnegeriSurat").toString()!="" &&
		 * h.get("idnegeriSurat").toString()!="0"){ Vector s3 =
		 * logic_A.getListBandarByNegeri
		 * (Integer.parseInt(h.get("idnegeriSurat").toString()));
		 ...
		 * this.context.put("show_batal_waris","yes");
		 * this.context.put("show_senarai_lapis_pertama","yes");
		 * this.context.put("show_tambah_waris1","yes"); } else
		 * if("kemaskini_waris_lapisan".equals(mode)){ int
		 * idwaris=Integer.parseInt(getParam("idwarisup")); int
		 * idparent=Integer.parseInt(getParam("idparentlapis"));
		 * logic.setDataWarisLapisan(idparent); listWarisLapisan =
		 * logic.getDataWarisLapisan(); logic.setDataWarisUpdate(idwaris);
		 * logic.setDataWarisParent(idparent); listWarisParent =
		 * logic.getDataWarisParent();
		 * this.context.put("listWarisParent",listWarisParent); listWarisUpdate=
		 * logic.getDataWarisUpdate(); logic.setDataWarisParent(idparent);
		 * listWarisParent = logic.getDataWarisParent();
		 * this.context.put("listWarisParent",listWarisParent);
		 * this.context.put("listWarisLapisan",listWarisLapisan);
		 * this.context.put("listWarisLapisanUpdate",listWarisUpdate);
		 ...
		 * this.context.put("show_lapisan_berikut_update","yes");
		 * this.context.put("show_lapisan_berikut","yes");
		 * this.context.put("buttonwarislapisan","Simpan"); } else
		 * if("simpan_waris".equals(mode)){
		 * 
		 * disability1 = "disabled"; updatewaris(session);
		 * logic.setDataWarisUpdate(Integer.parseInt(getParam("idwarisup")));
		 * listWarisUpdate = logic.getDataWarisUpdate();
		 * this.context.put("listWarisUpdate",listWarisUpdate); Hashtable h =
		 * (Hashtable)listWarisUpdate.get(0);
		 * if(h.get("idnegeriSurat").toString()!="" &&
		 ...
		 * }
		 * 
		 * else if("simpan_waris_lapisan".equals(mode)){ listpenting = new
		 * Vector(); int idparent=Integer.parseInt(getParam("idparentlapis"));
		 * int idxx= Integer.parseInt(getParam("idwarisup")); disability1 =
		 * "disabled"; updatewaris(session);
		 * logic.setDataWarisUpdate(Integer.parseInt(getParam("idwarisup")));
		 * listWarisUpdate = logic.getDataWarisUpdate();
		 * this.context.put("listWarisUpdate",listWarisUpdate);
		 * logic.setDataWarisUpdate(idxx); listWarisUpdate=
		 * logic.getDataWarisUpdate();
		 * this.context.put("listWarisLapisanUpdate",listWarisUpdate);
		 * logic.setDataWaris(idxx); listWaris = logic.getDataWaris();
		 * this.context.put("listWaris",listWaris);
		 * logic.setDataWarisLapisan(idparent); listWarisLapisan =
		 * logic.getDataWarisLapisan();
		 * this.context.put("listWarisLapisan",listWarisLapisan);
		 * logic.setDataWarisParent(idparent); listWarisParent =
		 * logic.getDataWarisParent();
		 * this.context.put("listWarisParent",listWarisParent); Hashtable h =
		 * (Hashtable)listWarisUpdate.get(0);
		 * if(h.get("idnegeriSurat").toString()!="" &&
		 * h.get("idnegeriSurat").toString()!="0"){ Vector s3 =
		 * logic_A.getListBandarByNegeri
		 ...
		 * this.context.put("readmode","disabled"); }
		 * 
		 * int id = Integer.parseInt(getParam("idPermohonan")); int
		 * id_Permohonansimati =
		 * Integer.parseInt(getParam("id_Permohonansimati"));
		 * logic.setDataSimati(id); listSimati = logic.getDataSimati();
		 * this.context.put("listSimati", listSimati);
		 * logic.setDataWaris(id_Permohonansimati); listWaris =
		 * logic.getDataWaris(); this.context.put("listWaris",listWaris);
		 ...
		 * logic.setDataWarisLapisanIdMatiDulu(id_Permohonansimati);
		 * listWarisLapisanIdMatiDulu = logic.getDataWarisLapisanIdMatiDulu();
		 * this
		 * .context.put("listWarisLapisanIdMatiDulu",listWarisLapisanIdMatiDulu
		 * );
		 * 
		 * logic.setDataWarisDulu(id_Permohonansimati); Vector listWarisDulu =
		 * logic.getDataWarisDulu();
		 ...
		 * this.context.put("selectedTabbawah",0);
		 * this.context.put("selectedTabtepi",0); context.put("DATEUTIL",new
		 * DateUtil());
		 * logic_A.setData(id,(String)session.getAttribute("_ekptg_user_id"));
		 * list = logic_A.getData(); this.context.put("View", list);
		 * logic_A.setDataFail(id); listFail = logic_A.getDataFail();
		 * this.context.put("ViewFail", listFail); int id1 =
		 * 
		 * Integer.parseInt(getParam("no_subjaket"));
		 * logic_A.setlistGetPermohonanSebelum(id1,(s1)); Vector
		 * listGetPermohonanSebelum = logic_A.getlistGetPermohonanSebelum();
		 * this.context.put("listGetPermohonanSebelum",
		 * listGetPermohonanSebelum);
		 * 
		 ...
		 */
			...
			logic.setDataWarisDulu(id_Permohonansimati);
			Vector listWarisDulu = logic.getDataWarisDulu();
			this.context.put("listWarisDulu", listWarisDulu);
			logic.setDataWarisOB(id_Permohonansimati);
			listWarisOB = logic.getDataWarisOB();
			this.context.put("listWarisOB", listWarisOB);
...
*/
			...
			logic.setDataPentingDulu(id_Permohonansimati);
			listPentingdulu = logic.getDataPentingDulu();
			...
			logic.setDataPenghutangDulu(id_Permohonansimati);
			listPenghutangdulu = logic.getDataPenghutangDulu();
...
//					mode = "";
				...
				System.out.println("Baca sini HTAAMnegerichangeSek17");
				String mati = getParam("id_Permohonansimati");
				...
				h.put("namaPelanUp", getParam("txtNamaPelanUp"));
				...
				this.context.put("tambahharta", "yes");
				this.context.put("negeriup", idnegeri);
				System.out.println("socNegeriHtaamUp ********** = "+getParam("socNegeriHtaamUp"));
				if (getParam("socNegeriHtaamUp") != ""
					...
					this.context.put("listBandarSuratbyNegeri", "");
					System.out.println("listBandarSuratbyNegeri30abcde");
				}
				...
				if(idhtaam.equals("$listamid.idhta")){
					idhtaam =getParam("idhtaamid");
				}
				
				logic.setDataHTAbyIdHtaam( idhtaam, getParam("id_Permohonansimati"));
				...
				if (k.get("negeri").toString() != ""
					...
					this.context.put("listBandarSuratbyNegeri", s3);
					System.out.println("listBandarSuratbyNegeri37");
				} else {
					this.context.put("listBandarSuratbyNegeri", "");
					System.out.println("listBandarSuratbyNegeri37a");
				}
				*/
				this.context.put("listHTA",listHTA);
				 ...
				 * id_Fail); } logic.setDataHTAbyIdHtaam(idhtaam); listHTAid =
				 * logic.getDataHTAbyIdHtaam();
				 * 
				 * this.context.put("idhtaam", idhtaam);
				 * this.context.put("listHTAid", listHTAid);
				 * this.context.put("show_kemaskini_htaam", "yes");
				...
				String idhtaam = getParam("idhtaamid");
				String idDokumen = getParam("idDokumen");
				logic.deleteHtaamSenarai(idDokumen ,idhtaam, getParam("id_Permohonansimati"));
				/*
				 * int mati = Integer.parseInt(getParam("id_Permohonansimati"));
				 * logic.setDataHTA(mati); listHTA = logic.getDataHTA();
				 * this.context.put("listHTA",listHTA);
				...
				String mati = getParam("id_Permohonansimati");
				String idhtaam = getParam("idhtaam");
				String idhtaamid = getParam("idhtaamid");
				String idDokumen = getParam("idDokumen");
			...
			System.out.println("bkp luar ==="+bkp);
			String lp = getParam("lp");
			String bpa = getParam("bpa");
			String lpa = getParam("lpa");
			
			Hashtable b = (Hashtable) list.get(0);
			String bkp_val = b.get("bkp").toString();
			System.out.println("bkp_val ==="+bkp_val);
			if(bkp==null){
				bkp = bkp_val;
			}
			
			System.out.println("bkp diumpukkan ===="+bkp);

			logic.setDataHTAdulu(mati, bkp, lp, bpa, lpa);
			Vector check_pilihan = logic_C.check_pilihan(mati);
			this.context.put("check_pilihan", check_pilihan);
			listHTAdulu = logic.getDataHTAdulu();
			...
			logic.setDataOBHTAdulu(mati, bkp, lp, bpa, lpa, "Y");
			listOBHTAdulu = logic.getDataOBHTAdulu();
			...
			if ("HtaamviewX3".equals(mode)) {
				...
				// int id = Integer.parseInt(getParam("idPermohonan"));
				String idSimati = getParam("idSimati");
				logic_A.updateDataNilai17(id, idSimati, (String) session.getAttribute("_ekptg_user_id"),getParam("id_Permohonansimati"));
				...
				h.put("idDokumen", getParam("idDokumen"));
				h.put("namaPelan", getParam("txtNamaPelan"));
				h.put("daerah", "");
				h.put("mukim", "");
				 ...
				 * listHTAX = logic.getDataHTAX(); this.context.put("listHTAX",
			...
			logic.setDataHTAXdulu_Pilihan(mati, bkp, lp, bpa, lpa);
			listHTAXdulu = logic.getDataHTAXdulu_Pilihan();
			...
			logic.setDataOBHTAdulu(mati, bkp, lp, bpa, lpa, "T");
			listOBHTAdulu = logic.getDataOBHTAdulu();
			this.context.put("listOBHTAdulu", listOBHTAdulu);
			...
			logic.setDataOBHAdulu(mati, bkp, lp, bpa, lpa, "T");
			listOBHAdulu = logic.getDataOBHAdulu();
			...
			logic_D.setCarianFailSenarai17(noFail, namaPemohon, namaSimati,
					icSimati, JenisIc, usid);
			list1 = logic_D.getListSenarai17();
			...
			vm = "app/ppk/frmPrmhnnSek17SenaraiPermohonan.jsp";
		}

		else if (("next".equals(submit)) || ("previous".equals(submit))) {

			this.context.put("carix", 1);
			// list = logic_E.setList17Senarai((String)
			...
			this.context.put("CountList", list17Senarai.size());
			this.context.put("selectNegeri", HTML.SelectNegeri("socNegeri"));
			prepareItemForDisplay(session, list17Senarai, 25, "first");
			vm = "app/ppk/frmPrmhnnSek17SenaraiPermohonan.jsp";
		}
		
		this.context.put("upload", upload);
...
private String capaianIntegrasi(HttpSession session, String jenisTab, String vm) throws Exception {
		
		if ("capaianWaris".equals(jenisTab)){
			this.context.put("show_simpan_button", "yes");
			this.context.put("show_batal_button", "yes");
			this.context.put("jenisTab", jenisTab);
			...
			this.context.put("myIdBaru1", getParam("txtNoKPBaru1Waris"));
			this.context.put("myIdBaru2", getParam("txtNoKPBaru2Waris"));
			this.context.put("myIdBaru3", getParam("txtNoKPBaru3Waris"));
			this.context.put("myIdLama", data.get("myIdLama"));
			this.context.put("jenisMyIdLain", data.get("jenisMyIdLain"));
			this.context.put("myIdLain", data.get("myIdLain"));
			this.context.put("namaPemohon", data.get("namaPemohon"));
			this.context.put("namaLain", data.get("namaLain"));
			this.context.put("jantina", data.get("jantina"));
			this.context.put("agama", data.get("agama"));
			this.context.put("warganegara", data.get("warganegara"));
			this.context.put("tarikhLahir", data.get("tarikhLahir"));
			this.context.put("noSuratBeranak", data.get("noSuratBeranak"));
			this.context.put("umur", data.get("umur"));
			this.context.put("alamatTetap", data.get("alamatTetap"));
			this.context.put("poskod", data.get("poskod"));
			
			
			this.context.put("listnegeri", HTML.SelectNegeri("socNegeriWaris", data.get("negeri")));
			myLogger.debug("val="+data.get("negeri"));
			this.context.put("listBandarTetapbyNegeri", HTML.SelectBandarByNegeri(data.get("negeri"), "txtBandarWaris", Long.parseLong(data.get("bandar")), ""));
			myLogger.debug("val2="+data.get("bandar"));
			
			this.context.put("catatan", data.get("catatan"));
			
			
			vm = "app/ppk/FrmPraPrmhnnSek17Waris.jsp";
		
		} else if ("capaianPemohon".equals(jenisTab)){
			...
			this.context.put("myIdBaru1", getParam("txtnoKpBaru1Pemohon"));
			this.context.put("myIdBaru2", getParam("txtnoKpBaru2Pemohon"));
			this.context.put("myIdBaru3", getParam("txtnoKpBaru3Pemohon"));
			this.context.put("myIdLama", data.get("myIdLama"));
			this.context.put("jenisMyIdLain", data.get("jenisMyIdLain"));
			this.context.put("myIdLain", data.get("myIdLain"));
			this.context.put("namaPemohon", data.get("namaPemohon"));
			...
			this.context.put("listnegeri", HTML.SelectNegeri("socNegeriPemohon", data.get("negeri")));
			myLogger.debug("val="+data.get("negeri"));
			this.context.put("listBandarTetapbyNegeri", HTML.SelectBandarByNegeri(data.get("negeri"), "bandartetap", Long.parseLong(data.get("bandar")), ""));
			myLogger.debug("val2="+data.get("bandar"));
		...
		} else if ("capaianSaksi".equals(jenisTab)){
//			Vector listPenting = null;
//			listPenting = logic.getDataPenting();
//			this.context.put("selectedTabtengah", 4);
//			this.context.put("listPenting", listPenting);
...
//			this.context.put("readmodesy", "disabled");
			...
			String id = getParam("idPermohonan");
			this.context.put("id", id);
			String myIdBaru = getParam("txtNoKPBaru1Penting")+getParam("txtNoKPBaru2Penting")+getParam("txtNoKPBaru3Penting");
			Hashtable<String, String> data = myIdentity.getMaklumat(myIdBaru);
			
			this.context.put("myIdBaru1", getParam("txtNoKPBaru1Penting"));
			this.context.put("myIdBaru2", getParam("txtNoKPBaru2Penting"));
			this.context.put("myIdBaru3", getParam("txtNoKPBaru3Penting"));
			this.context.put("jenisMyIdLain", data.get("jenisMyIdLain"));
			this.context.put("myIdLain", data.get("myIdLain"));
			this.context.put("namaOB", data.get("namaOB"));
			this.context.put("jantina", data.get("jantina"));
			this.context.put("agama", data.get("agama"));
			this.context.put("warganegara", data.get("warganegara"));
			this.context.put("tarikhLahir", data.get("tarikhLahir"));
			...
			this.context.put("listnegeri", HTML.SelectNegeri("socNegeriPenting", data.get("negeri")));
			myLogger.debug("val="+data.get("negeri"));
			this.context.put("listBandarTetapbyNegeri", HTML.SelectBandarByNegeri(data.get("negeri"), "txtBandarPenting", Long.parseLong(data.get("bandar")), ""));
			myLogger.debug("val2="+data.get("bandar"));
		...
		} else if ("capaianOB".equals(jenisTab)){
			...
			this.context.put("listPenting", listPenting);
			logic.setDataPentingDulu(id_Permohonansimati);
			Vector listPentingdulu = logic.getDataPentingDulu();
			this.context.put("listPentingdulu", listPentingdulu);
			logic.setDataPenghutang(id_Permohonansimati);
			Vector listPenghutang = logic.getDataPenghutang();
			this.context.put("listPenting", listPenghutang);
			logic.setDataPenghutangDulu(id_Permohonansimati);
			Vector listPenghutangdulu = logic.getDataPenghutangDulu();
			this.context.put("listPentingdulu", listPenghutangdulu);
			this.context.put("selectedTabatas", 0);
			...
			String txtNoKPBaru1Penting = data.get("jantina");
			this.context.put("myIdBaru1", getParam("txtNoKPBaru1Penting"));
			String txtNoKPBaru2Penting = data.get("jantina");
			this.context.put("myIdBaru2", getParam("txtNoKPBaru2Penting"));
			String txtNoKPBaru3Penting = data.get("jantina");
			this.context.put("myIdBaru3", getParam("txtNoKPBaru3Penting"));
			String myIdLama = data.get("myIdLama");
			this.context.put("myIdLama", data.get("myIdLama"));
			this.context.put("jenisMyIdLain", data.get("jenisMyIdLain"));
			this.context.put("myIdLain", data.get("myIdLain"));
			this.context.put("namaOB", data.get("namaOB"));
	...
	private void updatewaris(HttpSession session) throws Exception {
		...
		//razman bukak comment
		h.put("bandarSurat", getParam("txtBandarWarisSurat"));
		
		//razman comment
		//programmer!! dekat model terima string, kat sini kena string juga ler		
		/*
		if (getParam("txtBandarWarisSurat").equals("")) {
			h.put("bandarSurat", "0");
		} else {
	...
	public void addHtaam(HttpSession session) throws Exception {
		String bandar = getParam("txtBandarHartaHtaamX2");
		...
		h.put("poskod", getParam("txtAlamatPoskodHtaam"));
		/*if (getParam("txtBandarHartaHtaamX2").equals("")) {
...
//		logic.addHtaam(h);
		...
		String idHta = logic.addHtaamUpload(h);
	...
	public void addHtaamX(HttpSession session) throws Exception {
		...
		String idHta = logic.addHtaamUploadX(h);
	 ...
	 * 
	 * if (getParam("txtBahagianSimati1X") != "") {
	 * h.put("basimati",getParam("txtBahagianSimati1X")); } else {
	 * h.put("basimati", "0"); }
	 * 
	...
	/*
	 * private void addHa(HttpSession session) throws Exception { String id =
	 * getParam("id"); String id1 = getParam("idSimati"); String
	 * socJenisHartaAlih = getParam("socJenisHartaAlih"); String txtBhgnSimati1
	 * = getParam("txtBhgnSimati1"); String txtBhgnSimati2 =
	 * getParam("txtBhgnSimati2"); String txtNoRujukan =
	 * getParam("txtNoRujukan"); String txtNilaiTarikhMati =
	 * getParam("txtNilaiTarikhMati"); String txtNoSijil =
	 * getParam("txtNoSijil"); String txtNilaiTarikhMohon =
	 * getParam("txtNilaiTarikhMohon"); String txtBilUnit =
	 * getParam("txtBilUnit"); String txtNilaiSeunit =
	 * getParam("txtNilaiSeunit"); String txtAgensi = getParam("txtAgensi");
	 * String txtCatatan = getParam("txtCatatan"); String socNegeriHtaam =
	 * getParam("socNegeriHtaam"); String socDaerahHtaam =
	 * getParam("socDaerahHtaam"); String bil = getParam("bil"); int mati =
	 * Integer.parseInt(getParam("id_Permohonansimati"));
	 * 
	 * String txtAlamat1 = getParam("txtAlamat1"); String txtAlamat2 =
	 * getParam("txtAlamat2"); String txtAlamat3 = getParam("txtAlamat3");
	 * String txtPoskod = getParam("txtPoskod");
	 * 
	 * Hashtable h = null; h = new Hashtable(); h.put("id", id); h.put("id1",
	 * id1); h.put("id_Permohonansimati", mati); h.put("socJenisHartaAlih",
	 * socJenisHartaAlih); h.put("txtBhgnSimati1", txtBhgnSimati1);
	 * h.put("txtBhgnSimati2", txtBhgnSimati2); h.put("txtNoRujukan",
	 * txtNoRujukan); h.put("txtNilaiTarikhMati", txtNilaiTarikhMati);
	 * h.put("txtNoSijil", txtNoSijil); h.put("txtNilaiTarikhMohon",
	 * txtNilaiTarikhMohon); h.put("txtBilUnit", txtBilUnit);
	 * h.put("txtNilaiSeunit", txtNilaiSeunit); h.put("txtAgensi", txtAgensi);
	 * h.put("txtCatatan", txtCatatan); h.put("bil", bil);
	 * 
	 * h.put("txtAlamat1", txtAlamat1); h.put("txtAlamat2", txtAlamat2);
	 * h.put("txtAlamat3", txtAlamat3); h.put("txtPoskod", txtPoskod);
	 * 
	 * h.put("socNegeriHtaam", socNegeriHtaam); h.put("socDaerahHtaam",
	 * socDaerahHtaam); h.put("id_Masuk", (String)
	 * session.getAttribute("_ekptg_user_id"));
	 * 
	 * logic_A.addHa(h); }
	 * 
	 * private void updateHa(HttpSession session) throws Exception { String id1
	 * = getParam("idSimati"); String id3 = getParam("idha"); String
	 * socJenisHartaAlih = getParam("socJenisHartaAlih"); String txtBhgnSimati1
	 * = getParam("txtBhgnSimati1"); String txtBhgnSimati2 =
	 * getParam("txtBhgnSimati2"); String txtNoRujukan =
	 * getParam("txtNoRujukan"); String txtNilaiTarikhMati =
	 ...
	 * getParam("txtNilaiTarikhMohon"); String txtBilUnit =
	 * getParam("txtBilUnit"); String txtNilaiSeunit =
	 * getParam("txtNilaiSeunit"); String Agensi = getParam("txtAgensi"); String
	 * txtCatatan = getParam("txtCatatan"); String bil = getParam("bil"); String
	 * txtAlamat1 = getParam("txtAlamat1"); String txtAlamat2 =
	 * getParam("txtAlamat2"); String txtAlamat3 = getParam("txtAlamat3");
	 * String txtPoskod = getParam("txtPoskod");
	 * 
	 * String socDaerahHtaam = getParam("socDaerahHtaam"); String socNegeriHtaam
	 * = getParam("socNegeriHtaam");
	 * 
	 * Hashtable h = null; h = new Hashtable(); h.put("id3", id3); h.put("id1",
	 * id1); h.put("socJenisHartaAlih", socJenisHartaAlih);
	 * h.put("txtBhgnSimati1", txtBhgnSimati1); h.put("txtBhgnSimati2",
	 * txtBhgnSimati2); h.put("txtNoRujukan", txtNoRujukan);
	 * h.put("txtNilaiTarikhMati", txtNilaiTarikhMati); h.put("txtNoSijil",
	 * txtNoSijil); h.put("txtNilaiTarikhMohon", txtNilaiTarikhMohon);
	 * h.put("txtBilUnit", txtBilUnit); h.put("txtNilaiSeunit", txtNilaiSeunit);
	 * h.put("Agensi", Agensi); h.put("txtCatatan", txtCatatan); h.put("bil",
	 * bil); h.put("txtAlamat1", txtAlamat1); h.put("txtAlamat2", txtAlamat2);
	 * h.put("txtAlamat3", txtAlamat3); h.put("txtPoskod", txtPoskod);
	 * h.put("id_Masuk", (String) session.getAttribute("_ekptg_user_id"));
	 * 
	 * if (socNegeriHtaam != "" && socNegeriHtaam != "0") {
	 * h.put("socDaerahHtaam", socDaerahHtaam); } else { h.put("socDaerahHtaam",
	 * "0"); }
	 * 
	 * h.put("socNegeriHtaam", socNegeriHtaam); logic_A.kemaskiniHa(h); }
	 */
	...
	private void updateHtaam(HttpSession session) throws Exception {
		...
		// ADD BY SALNIZAM - TAMBAH FIELD ALAMAT
				...
				System.out.println("txtBandarHartaHtaamX2 : "+getParam("txtBandarHartaHtaamX2"));
	...
	private void simpanpilihanHTA(HttpSession session, String bolehsimpan)
		...
		if (!flag_perlu_batal_kt.equals("")) {
			...
			listHartaTakAlihdulu = logic.setDataSemuaHartaTakAlihdulu(per_mati,
					bkp, lp, bpa, lpa);
			...
			listHartaAlihdulu = logic.setDataSemuaHartaAlihdulu(per_mati, bkp,
					lp, bpa, lpa);
...
//				long id_hta = DB.getNextID(db, "TBLPPKHTA_SEQ");
				PreparedStatement ps1 = con
						.prepareStatement("INSERT INTO TBLPPKUPLOADPELAN"
								+ "(ID_PELAN,ID_DOKUMEN,ID_HTA,ID_MASUK,TARIKH_MASUK) "
								+ "VALUES(?,?,?,?,SYSDATE)");
