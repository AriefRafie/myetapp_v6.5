...
public class FrmSuburusanStatusData {

	private static Logger myLog = Logger.getLogger(ekptg.model.utils.FrmSuburusanStatusData.class);
	public static String idSubUrusanStatus = "";
	
	public FrmSuburusanStatusData() {
	...
	public static boolean insert(Hashtable parameters, String uid) {
		  ...
		  try {
			  ...
			  int x = 1;
			  long Id_SubUrusanStatus= DB.getNextID("TBLRUJSUBURUSANSTATUS_SEQ");
			  idSubUrusanStatus = String.valueOf(Id_SubUrusanStatus);
			  ...
			  sql = r.getSQLInsert("tblrujsuburusanstatus");
			  myLog.info("FrmSuburusanStatusData::insert::tblrujsuburusanstatus = "+sql);
	  ...
	  public static boolean update(Hashtable parameters,String id) {
		  ...
		  String value="";
		  if (id != null) {
			  sql = "UPDATE tblrujsuburusanstatus SET ";
			  int x = 1;
			  for (Enumeration e = parameters.keys(); e.hasMoreElements();x++)
			   {
				  name = (String)e.nextElement();
			  ...
			  sql = sql + " WHERE ID_SUBURUSANSTATUS = '" +id+ "' ";
	  ...
	  public Hashtable getDetails(String id) {
		  ...
		  try {
			  ...
			  while (rs.next()) {
				  ...
				  details_data.put("aktif",checkIsNull(rs.getString("aktif")).equals("")?0:rs.getInt("aktif"));
	...
	public Vector getListing(String id_urusan,String id_suburusan)	throws Exception{
		Db db = null;
		...
		try {
			...
			if (id_suburusan != null && !"".equals(id_suburusan)) {
				sql += " and rsu.ID_SUBURUSAN="+id_suburusan;
			}
			sql += " order by rsu.ID_SUBURUSAN,rsus.langkah";
	        System.out.println("FrmSuburusanStatusData:getListing("+id_urusan+","+id_suburusan+")::"+sql);

			ResultSet rs = stmt.executeQuery(sql);
			Hashtable record = null;
			while(rs.next()) {
				record = new Hashtable();
				record.put("idsuburusanstatus",rs.getString("id_suburusanstatus"));
				record.put("keterangan",checkIsNull(rs.getString("keterangan")));
				record.put("langkah",getShortName(checkIsNull(rs.getString("langkah"))));
				record.put("namasuburusan",checkIsNull(rs.getString("nama_suburusan")));
