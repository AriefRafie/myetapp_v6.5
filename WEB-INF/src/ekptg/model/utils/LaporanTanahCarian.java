...
public class LaporanTanahCarian implements ITanahCarian {
	
	private static Logger myLog = Logger.getLogger(ekptg.model.utils.LaporanTanahCarian.class);
...
//								"))";	
	...
	//PAPAR CARIAN HAKMILIK DAN RIZAB (12)
	@Override
	public Vector<Hashtable<String,String>> getCarianSenaraiHakmilikRizab(String idJenisTanah
			,String idNegeri, String idDaerah,String idMukim
			,String noFail
...
//	     SELECT RL.KETERANGAN AS KOD_LOT,MT.NO_LOT
//	    ,RJH.KOD_JENIS_HAKMILIK,NVl(MT.NO_HAKMILIK,'TIADA') NO_HAKMILIK
//	    ,RN.NAMA_NEGERI,RD.NAMA_DAERAH,RM.NAMA_MUKIM
	    try {
	    	db = new Db();
	    	Statement stmt = db.getStatement();  
	    	sql = "SELECT * "+
	            "FROM V_LAPORAN_TANAH A " +
	            ""+
	            "";
			...
			if (idJenisTanah != null) {
				if (!idJenisTanah.trim().equals("")) {
					if (Integer.parseInt(idJenisTanah) == 1){
						sql = sql + " WHERE  NVL(A.NO_HAKMILIK,' ') <> ' ' ";
//						sql = sql + " AND NVL(A.NO_HAKMILIK,' ') <> ' ' ";
					} else if (Integer.parseInt(idJenisTanah) == 2){
						sql = sql + " AND NVL(A.NO_WARTA,' ') <> ' ' ";
					}
...
//	      		h.put("kegunaanTanah", rs.getString("TAJUK_FAIL")== null?"":rs.getString("TAJUK_FAIL").toUpperCase());
//	      		h.put("statusRizab", rs.getString("STATUS_RIZAB")== null? "":rs.getString("STATUS_RIZAB"));
//	      		h.put("tarikhTerima",rs.getDate("TARIKH_TERIMA")== null? "":sdf.format(rs.getDate("TARIKH_TERIMA")));
//	    	  
//	      		if(rs.getString("NO_HAKMILIK")!= null){
