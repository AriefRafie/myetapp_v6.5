...
public class FrmPNWHeaderData {

	private Vector beanMaklumatPermohonan = null;
	private Vector beanMaklumatHakmilik = null;
	private static final Log log = LogFactory.getLog(FrmPNWHeaderData.class);
	...
	public String getIdHakmilikByIdFail(String idFail) throws Exception {
		...
		try {
			...
			sql = "SELECT PHPHM.ID_HAKMILIK FROM TBLPERMOHONAN MOHON, TBLPHPHAKMILIKPERMOHONAN PHPHMP, TBLPHPHAKMILIK PHPHM"
	...
	public void setMaklumatPermohonan(String idFail, HttpSession session)
		...
		try {
			...
			sql = "SELECT FAIL.ID_FAIL, FAIL.NO_FAIL, FAIL.TAJUK_FAIL, MOHON.ID_PERMOHONAN, MOHON.TARIKH_TERIMA, MOHON.TARIKH_SURAT,"
					...
					+ " MOHONPLP.KEPUTUSAN AS ID_KEPUTUSAN, MOHONPLP.FLAG_GUNA, MOHON.CATATAN_BATAL, MOHON.TARIKH_BATAL, FAIL.TARIKH_DAFTAR_FAIL, MOHONPLP.CADANGAN_KEGUNAAN, MOHONPLP.NO_RUJUKAN_SURAT"
					...
					+ " AND FAIL.ID_SEKSYEN = 4 AND FAIL.ID_URUSAN = 6 AND FAIL.ID_SUBURUSAN = 32"
			...
			if (rs.next()) {
				...
				h.put("keputusan", rs.getString("ID_KEPUTUSAN") == null ? ""
				...
				h.put("idAgensi",
						rs.getString("ID_AGENSI") == null ? "" : rs
								.getString("ID_AGENSI"));
				h.put("cadanganKegunaan", rs.getString("CADANGAN_KEGUNAAN") == null ? "" : rs
						.getString("CADANGAN_KEGUNAAN").toUpperCase());
				h.put("noRujukanSurat", rs.getString("NO_RUJUKAN_SURAT") == null ? "" : rs
