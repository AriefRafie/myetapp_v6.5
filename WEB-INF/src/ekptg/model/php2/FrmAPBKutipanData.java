...
public class FrmAPBKutipanData {
	...
	public void carianFail(String noFail, String namaPemohon,
			String tarikhTerima, String txtNoLesen) throws Exception {
	...
	public String daftarBaru(Hashtable hash, HttpSession session)
		...
		try {
			...
			r.add("NO_FAIL", hash.get("noFail"));
			r.add("NO_FAIL_ROOT", hash.get("noFail"));
			...
			r.add("ID_PERMOHONAN", idPermohonan);
			r.add("ID_SUBURUSANSTATUS", getIdSuburusanstatus("57", "1610198")); // MAKLUMAT
																				// PERMOHONAN
			r.add("AKTIF", "1");
			r.add("ID_FAIL", idFail);
	...
	public boolean checkingExistNoFail(String noFail) throws Exception {
		...
		try {
			...
			sql = "SELECT ID_FAIL FROM TBLPFDFAIL WHERE ID_SEKSYEN = '4' AND ID_URUSAN = '9' AND ID_SUBURUSAN = '57' AND UPPER(NO_FAIL) = '" + noFail + "'";
			ResultSet rs = stmt.executeQuery(sql);
