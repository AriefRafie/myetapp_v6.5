...
public class FrmPLPKutipanData {
	...
	public String daftarBaru(String idJenisTanah, String noFail,
			String tarikhTerima, String tarikhSurat, String noRujukanSurat,
			String perkara, String idKategoriPemohon, String nama,
		...
		try {
			...
			if ("3".equals(idJenisTanah)) {
				setMaklumatBorangK(idPPTBorangK, idHakmilikUrusan,
						idPHPBorangK, null);
				...
				setMaklumatTanah(idHakmilikAgensi, idHakmilikSementara, null);
			...
			r.add("ID_PERMOHONAN", idPermohonan);
			r.add("ID_SUBURUSANSTATUS", getIdSuburusanstatus("34", "1610198")); // MAKLUMAT
																				// PERMOHONAN
			r.add("AKTIF", "1");
			r.add("ID_FAIL", idFail);
			...
			r = new SQLRenderer();
			long idKertasCadangan = DB
					.getNextID("TBLPHPKERTASKERJAPELEPASAN_SEQ");
			r.add("ID_KERTASKERJA", idKertasCadangan);
			r.add("ID_PERMOHONAN", idPermohonan);
			r.add("FLAG_KERTAS", "1");
			...
			// TBLPHPKERTASKERJAPELEPASAN
			r = new SQLRenderer();
			long idKertasKewangan = DB
					.getNextID("TBLPHPKERTASKERJAPELEPASAN_SEQ");
			r.add("ID_KERTASKERJA", idKertasKewangan);
			r.add("ID_PERMOHONAN", idPermohonan);
			r.add("FLAG_KERTAS", "2");
			...
			// TBLPHPKERTASKERJAPELEPASAN
			r = new SQLRenderer();
			long idKertasCeraian = DB
					.getNextID("TBLPHPKERTASKERJAPELEPASAN_SEQ");
			r.add("ID_KERTASKERJA", idKertasCeraian);
			r.add("ID_PERMOHONAN", idPermohonan);
			r.add("FLAG_KERTAS", "3");
		...
		try {
			...
			while (rs.next()) {
				...
				h.put("tujuanKegunaan",
						rs.getString("NAMA_PROJEK") == null ? "" : rs
								.getString("NAMA_PROJEK"));
	...
	public String getIdHakmilikAgensiByPeganganHakmilik(
			String peganganHakmilik, String idNegeri) throws Exception {
		Db db = null;
		String sql = "";
		...
		try {
			...
			if (idNegeri != null) {
				if (!idNegeri.trim().equals("")
					...
					sql = sql + " AND TBLHTPHAKMILIK.ID_NEGERI = '"
	...
	public String getIdHakmilikSementaraByPeganganHakmilik(
		...
		try {
			...
			sql = "SELECT ID_HAKMILIKSEMENTARA FROM TBLPHPHAKMILIKSEMENTARA WHERE "
					+ " UPPER(TBLHTPHAKMILIK.PEGANGAN_HAKMILIK) = '"
					+ peganganHakmilik.toUpperCase() + "'";
	...
	public void setMaklumatPemohon(String idFail) throws Exception {
		...
		try {
			...
			sql = "SELECT C.ID_PEMOHON, C.NAMA, C.NO_PENGENALAN, C.ALAMAT1_TETAP, C.ALAMAT2_TETAP, C.ALAMAT3_TETAP,"
					...
					+ " C.EMEL, C.ID_KATEGORIPEMOHON, C.ID_PEJABAT "
	...
	public void setMaklumatTanah(String idHakmilikAgensi,
			String idHakmilikSementara, String idNegeri) throws Exception {
		Db db = null;
		String sql = "";
	...
	public boolean checkingExistNoFail(String noFail) throws Exception {
		...
		try {
			...
			sql = "SELECT ID_FAIL FROM TBLPFDFAIL WHERE ID_SEKSYEN = '4' AND ID_URUSAN = '6' AND ID_SUBURUSAN = '34' AND UPPER(NO_FAIL) = UPPER('"
					+ noFail + "')";
			ResultSet rs = stmt.executeQuery(sql);
	...
	public void setMaklumatBorangK(String idPPTBorangK,
			String idHakmilikUrusan, String idPHPBorangK, String idNegeri)
			throws Exception {
		Db db = null;
		String sql = "";
