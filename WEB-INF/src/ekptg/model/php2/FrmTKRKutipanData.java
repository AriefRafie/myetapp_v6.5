...
public class FrmTKRKutipanData {
	...
	public String daftarBaru(String idJenisTanah, String noFail,
			...
			String perkara, String idKategoriPemohon, String idKementerian,
			String idAgensi, String idPejabat, String idHakmilikAgensi,
			...
			String idLuasTanah, String luasTanah, HttpSession session)
			throws Exception {
		...
		try {
			...
			if ("3".equals(idJenisTanah)) {
				...
				setMaklumatTanah(idHakmilikAgensi);
		...
		try {
			...
			while (rs.next()) {
				...
				h.put("idPejabat", rs.getString("ID_PEJABAT") == null ? "99999"
						: rs.getString("ID_PEJABAT").toUpperCase());
	...
	public boolean checkingExistNoFail(String noFail) throws Exception {
		...
		try {
			...
			sql = "SELECT ID_FAIL FROM TBLPFDFAIL WHERE ID_SEKSYEN = '4' AND ID_URUSAN = '6' AND ID_SUBURUSAN = '33' AND UPPER(NO_FAIL) = UPPER('"
					+ noFail + "')";
			ResultSet rs = stmt.executeQuery(sql);
