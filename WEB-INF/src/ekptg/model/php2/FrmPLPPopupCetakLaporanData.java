...
//	}
	...
	public void getMesyuarat(String idPermohonan) throws Exception {
		...
		try {
			...
			sql = " SELECT MAX (TO_CHAR (A.TARIKH_MESYUARAT, 'DD/MM/YYYY')) AS TARIKH_MESYUARAT,"
				+ " MAX(INITCAP(A.TAJUK)) AS TAJUK, MAX (INITCAP(D.NAMA_PEGAWAI)) AS NAMA_PEGAWAI"
				+ " FROM TBLPHPMESYUARAT A, TBLPHPMESYUARATPERMOHONAN B, TBLPERMOHONAN C, TBLPHPKEHADIRANMESY D"
				+ " WHERE A.ID_MESYUARAT = B.ID_MESYUARAT AND B.ID_PERMOHONAN = C.ID_PERMOHONAN"
				+ " AND B.ID_MESYUARAT = D.ID_MESYUARAT(+) AND D.FLAG_PENGERUSI = 'Y'"
