...
public class FrmPLPKemasukanDokumenData {
	
	static Logger myLogger = Logger.getLogger(FrmPLPKemasukanDokumenData.class);	
	protected Db db;
	...
	public Vector papar(String id) throws Exception {
		...
		try {
			...
			sql = " SELECT DISTINCT A.ID_SENARAILAPORAN, A.USER_LOGIN, C.USER_NAME," 
				+ " B.NAMA_LAPORAN AS NAMA_LAPORAN, B.ID, MAX(A.TARIKH_CETAK) AS TARIKH_CETAK"
				+ " FROM TBLINTMACGDIHISTORY A, TBLINTMACGDILAPORAN B, USERS C" 
				+ " WHERE A.USER_LOGIN = C.USER_LOGIN AND A.ID_SENARAILAPORAN = B.ID"
				+ SQL_SEARCH
