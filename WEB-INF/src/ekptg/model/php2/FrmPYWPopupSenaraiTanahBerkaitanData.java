...
public class FrmPYWPopupSenaraiTanahBerkaitanData {
	...
	public void carianTanah(String idJenisTanah, String peganganHakmilik,
			...
			String idAgensi, String idPermohonan) throws Exception {
		...
		try {
			...
			sql = "SELECT F.ID_HAKMILIKAGENSI, A.ID_HAKMILIK, NULL AS ID_HAKMILIKSEMENTARA, A.PEGANGAN_HAKMILIK, B.KETERANGAN, A.NO_LOT, A.NO_HAKMILIK, A.NO_WARTA, E.NAMA_MUKIM, D.NAMA_DAERAH, I.KOD_JENIS_HAKMILIK,"
					...
					+ " WHERE A.ID_LOT = B.ID_LOT(+) AND A.ID_NEGERI = C.ID_NEGERI(+) AND A.ID_DAERAH = D.ID_DAERAH(+) AND A.ID_MUKIM = E.ID_MUKIM(+) AND H.ID_KEMENTERIAN = G.ID_KEMENTERIAN(+) AND F.ID_AGENSI = H.ID_AGENSI(+)"
					+ " AND A.ID_JENISHAKMILIK = I.ID_JENISHAKMILIK AND (A.STATUS_SAH != 'B' OR A.STATUS_SAH IS NULL) AND F.ID_HAKMILIK = A.ID_HAKMILIK AND F.FLAG_AKTIF = 'Y'";
			...
			if (jenisHakmilik != null) {
				if (!jenisHakmilik.trim().equals("")
					...
					sql = sql + " AND A.ID_JENISHAKMILIK = '"
			...
			if (tarikhWarta != null) {
				if (!tarikhWarta.toString().trim().equals("")) {
					sql = sql + " AND TO_CHAR(A.TARIKH_WARTA,'dd-MON-YY') = '"
			...
			sql = sql
					+ " SELECT NULL AS ID_HAKMILIKAGENSI, NULL AS ID_HAKMILIK, A.ID_HAKMILIKSEMENTARA, A.PEGANGAN_HAKMILIK, B.KETERANGAN, A.NO_LOT,"
					+ " A.NO_HAKMILIK, A.NO_WARTA, E.NAMA_MUKIM, D.NAMA_DAERAH, I.KOD_JENIS_HAKMILIK, C.NAMA_NEGERI, A.ID_JENISHAKMILIK, A.ID_LOT, A.TARIKH_WARTA,"
					+ " A.ID_MUKIM, A.ID_DAERAH, A.ID_NEGERI,	A.ID_KEMENTERIAN, A.ID_AGENSI, G.NAMA_KEMENTERIAN, H.NAMA_AGENSI"
					+ " FROM TBLPHPHAKMILIKSEMENTARA A, TBLRUJJENISHAKMILIK I, TBLRUJLOT B, TBLRUJMUKIM E, TBLRUJDAERAH D, TBLRUJNEGERI C,"
					+ " TBLRUJKATEGORI RUJKATEGORI, TBLRUJSUBKATEGORI RUJSUBKATEGORI, TBLRUJAGENSI H, TBLRUJKEMENTERIAN G"
					+ " WHERE A.ID_JENISHAKMILIK = I.ID_JENISHAKMILIK(+) AND A.ID_LOT = B.ID_LOT AND A.ID_MUKIM = E.ID_MUKIM(+) AND A.ID_DAERAH = D.ID_DAERAH(+)"
					+ " AND A.ID_NEGERI = C.ID_NEGERI(+) AND A.ID_KATEGORI = RUJKATEGORI.ID_KATEGORI(+) AND A.ID_SUBKATEGORI = RUJSUBKATEGORI.ID_SUBKATEGORI(+)"
					+ " AND A.ID_AGENSI = H.ID_AGENSI(+) AND H.ID_KEMENTERIAN = G.ID_KEMENTERIAN(+)";
			...
			while (rs.next()) {
				...
				h.put("idHakmilikAgensi",
						rs.getString("ID_HAKMILIKAGENSI") == null ? "" : rs
								.getString("ID_HAKMILIKAGENSI").toUpperCase());
	...
	public void setMaklumatTanah(String idHakmilikAgensi,
		...
		try {
			...
			sql = "SELECT L.ID_HAKMILIKAGENSI, A.ID_HAKMILIK, NULL AS ID_HAKMILIKSEMENTARA, A.PEGANGAN_HAKMILIK, F.KOD_JENIS_HAKMILIK, F.KETERANGAN AS JENIS_HAKMILIK, A.NO_HAKMILIK,"
				+ " B.KOD_LOT, B.KETERANGAN AS JENIS_LOT, A.NO_LOT, A.NO_WARTA, A.KEGUNAAN_TANAH, L.LUAS_BERSAMAAN, A.TARIKH_WARTA, E.NAMA_MUKIM, D.NAMA_DAERAH, C.NAMA_NEGERI,"
				...
				+ " A.ID_NEGERI, A.ID_DAERAH, A.ID_MUKIM, A.ID_JENISHAKMILIK, A.ID_LOT, L.ID_LUAS_BERSAMAAN AS ID_LUASBERSAMAAN, A.ID_KATEGORI, A.ID_SUBKATEGORI, L.ID_KEMENTERIAN, L.ID_AGENSI"
		...
		sql = sql
				...
				+ " RUJJENISHM.KOD_JENIS_HAKMILIK, RUJJENISHM.KETERANGAN AS JENIS_HAKMILIK, HMS.NO_HAKMILIK,"
				+ " RUJLOT.KOD_LOT, RUJLOT.KETERANGAN AS JENIS_LOT, HMS.NO_LOT, HMS.NO_WARTA, HMS.KEGUNAAN_TANAH, HMS.LUAS AS LUAS_BERSAMAAN, HMS.TARIKH_WARTA, RUJMUKIM.NAMA_MUKIM, RUJDAERAH.NAMA_DAERAH, RUJNEGERI.NAMA_NEGERI,"
				+ " RUJSUBKATEGORI.KETERANGAN AS SUBKATEGORI, RUJKATEGORI.KETERANGAN AS KATEGORI, HMS.SYARAT, HMS.SEKATAN, RUJKEMENTERIAN.NAMA_KEMENTERIAN, RUJAGENSI.NAMA_AGENSI, RUJLUAS.KOD_LUAS AS KOD_LUAS, RUJLUAS.KETERANGAN AS JENIS_LUAS_BERSAMAAN,"
				+ " HMS.ID_NEGERI, HMS.ID_DAERAH, HMS.ID_MUKIM, HMS.ID_JENISHAKMILIK, HMS.ID_LOT, HMS.ID_LUAS AS ID_LUASBERSAMAAN, HMS.ID_KATEGORI, HMS.ID_SUBKATEGORI, HMS.ID_KEMENTERIAN, HMS.ID_AGENSI"
			...
			h.put("hakmilik", rs.getString("KOD_JENIS_HAKMILIK") == null
					|| rs.getString("NO_HAKMILIK") == null ? "" : rs
					.getString("KOD_JENIS_HAKMILIK").toUpperCase()
					+ " "
					+ rs.getString("NO_HAKMILIK"));
			...
			h.put("idNegeri", rs.getString("ID_NEGERI") == null ? ""
					: rs.getString("ID_NEGERI"));
			h.put("idDaerah", rs.getString("ID_DAERAH") == null ? ""
					: rs.getString("ID_DAERAH"));
			h.put("idMukim", rs.getString("ID_MUKIM") == null ? ""
					: rs.getString("ID_MUKIM"));
			h.put("idJenisHakmilik", rs.getString("ID_JENISHAKMILIK") == null ? ""
					: rs.getString("ID_JENISHAKMILIK"));			
			h.put("idLot", rs.getString("ID_LOT") == null ? ""
					: rs.getString("ID_LOT"));
			h.put("idLuasBersamaan", rs.getString("ID_LUASBERSAMAAN") == null ? ""
					: rs.getString("ID_LUASBERSAMAAN"));
			h.put("idKategori", rs.getString("ID_KATEGORI") == null ? ""
					: rs.getString("ID_KATEGORI"));
			h.put("idSubKategori", rs.getString("ID_SUBKATEGORI") == null ? ""
					: rs.getString("ID_SUBKATEGORI"));
	...
	public void simpan(String idPermohonan, String idHakmilikAgensi, String idHakmilikSementara,
		...
		try {
			...
			if (getBeanMaklumatTanah().size() != 0) {
				...
				r.add("ID_LUAS", hashTanah.get("idLuasBersamaan"));
