...
public class FrmPNWOnlineKJPSenaraiFailData {
	...
	public void carianFail(String userId, String userRole, String noFail,String tajukFail, String noPermohonan,String tarikhPermohonan, String idNegeriC,
			String idDaerahC, String idMukimC,String jenisHakmilik,String txtNoHakmilik,String txtNoWarta,String jenisLot,String txtNoLot,String txtNoPegangan, String idStatusC ) throws Exception {
...
//			sql = "SELECT A.ID_FAIL, B.ID_PERMOHONAN, A.NO_FAIL, B.TARIKH_TERIMA, C.NAMA, D.KETERANGAN, B.ID_STATUS, H.USER_LOGIN,B.NO_PERMOHONAN "
...
//				+ " WHERE A.ID_URUSAN = '6' AND A.ID_SUBURUSAN = '32' AND A.FLAG_JENIS_FAIL = '4' AND A.ID_FAIL = B.ID_FAIL AND B.ID_STATUS = D.ID_STATUS(+)"
...
//				+ " AND A.ID_MASUK = H.USER_ID(+) AND A.ID_MASUK ='" + userId + "' ";
	...
	public String daftarBaru(String idJenisTanah, String tarikhTerima,
			...
			String idKategoriPemohon, String idKementerianPemohon,
			String idAgensiPemohon, String idHakmilikAgensi,
		...
		try {
			...
			myLog.info("ros 8 idhakmilikPermohonan "+idhakmilikPermohonan);
			r.add("ID_PERMOHONAN", idPermohonan);
			myLog.info("ros 8 idPermohonan "+idPermohonan);
			r.add("ID_HAKMILIKAGENSI", idHakmilikAgensi);
			myLog.info("ros 8 idHakmilikAgensi "+idHakmilikAgensi);
			r.add("ID_PPTBORANGK", idPPTBorangK);
			myLog.info("ros 8 idPPTBorangK "+idPPTBorangK);
			r.add("ID_HAKMILIKURUSAN", idHakmilikUrusan);
			myLog.info("ros 8 idHakmilikUrusan "+idHakmilikUrusan);
			r.add("ID_PHPBORANGK", idPHPBorangK);
			myLog.info("ros 8 idPHPBorangK "+idPHPBorangK);
			r.add("ID_HAKMILIKSEMENTARA", idHakmilikSementara);
			if ("3".equals(idJenisTanah)) {
				r.add("FLAG_BORANGK", "Y");
			}

			myLog.info("ros 8 idJenisTanah "+idJenisTanah);
			r.add("ID_MASUK", userId);
			r.add("TARIKH_MASUK", r.unquote("SYSDATE"));
	...
	/*public String daftarBaru(String userRole, String idKementerianPemohon, String idAgensiPemohon, String idHakmilikAgensi, HttpSession session) throws Exception {
		Db db = null;
		Connection conn = null;
		String userId = (String) session.getAttribute("_ekptg_user_id");
		String sql = "";
		...
		try {
			...
			if (rs.next()){
				...
				r.add("ID_NEGERITETAP", rs.getString("ID_NEGERI") == null ? "" : rs.getString("ID_NEGERI").toUpperCase());
			}

			r.add("ID_KATEGORIPEMOHON", "3");
			r.add("ID_MASUK", userId);
			r.add("TARIKH_MASUK", r.unquote("SYSDATE"));
			sql = r.getSQLInsert("TBLPHPPEMOHON");
			stmt.executeUpdate(sql);
			...
			r.add("ID_FAIL", idFail);
			r.add("ID_STATUS", "");
			r.add("FLAG_LAYER_KJP", "1");
			r.add("TARIKH_TERIMA", r.unquote("SYSDATE"));
			r.add("FLAG_AKTIF", "Y");

			Calendar currentDate = new GregorianCalendar();
			String noPermohonan = "JKPTG/SPHP/04/" + getKodUrusanByIdUrusan("6") + "/" + currentDate.get(Calendar.YEAR) + "/" + File.getSeqNo(db, 4, Integer.parseInt("6"), 0, 0, 0, false, false, currentDate.get(Calendar.YEAR), 0);
	...
	public String getidNegeriByIdHakmilik(String idHakmilikAgensi) throws Exception {
		...
		try {
			...
			sql = "SELECT TBLHTPHAKMILIK.ID_NEGERI FROM TBLHTPHAKMILIK, TBLHTPHAKMILIKAGENSI WHERE TBLHTPHAKMILIK.ID_HAKMILIK = TBLHTPHAKMILIKAGENSI.ID_HAKMILIK AND TBLHTPHAKMILIKAGENSI.ID_HAKMILIKAGENSI = '" + idHakmilikAgensi + "'";
	...
	public String getidKementerianByIdHakmilik(String idHakmilikAgensi) throws Exception {
		...
		try {
			...
			sql = "SELECT TBLHTPHAKMILIKAGENSI.ID_KEMENTERIAN FROM TBLHTPHAKMILIK , TBLHTPHAKMILIKAGENSI WHERE TBLHTPHAKMILIK.ID_HAKMILIK = TBLHTPHAKMILIKAGENSI.ID_HAKMILIK AND TBLHTPHAKMILIKAGENSI.ID_HAKMILIKAGENSI = '" + idHakmilikAgensi + "'";
	...
	public String getIdNegeriByUserId(String userId) throws Exception {
		...
		try {
			...
			sql = "SELECT B.ID_NEGERI FROM USERS A, USERS_ONLINE B"
	...
	public void setMaklumatHeader(String idFail) throws Exception {
		...
		try {
			...
			sql = "SELECT A.ID_FAIL, A.NO_FAIL, A.TAJUK_FAIL, B.ID_PERMOHONAN, B.TARIKH_TERIMA, B.TARIKH_SURAT, C.ID_PEMOHON, C.NAMA, C.ID_NEGERITETAP, C.ID_KATEGORIPEMOHON, C.ID_PEJABAT, C.ID_AGENSI, I.ID_NEGERI AS ID_NEGERITANAH, H.ID_KEMENTERIAN AS ID_KEMENTERIANTANAH, H.ID_AGENSI AS ID_AGENSITANAH,"
				+ " C.ALAMAT1_TETAP, C.ALAMAT2_TETAP, C.ALAMAT3_TETAP, C.POSKOD_TETAP, D.NAMA_NEGERI, G.KETERANGAN AS NAMA_BANDAR, C.NO_TEL, C.NO_FAX, B.ID_STATUS, E.KETERANGAN, I.NO_HAKMILIK, I.NO_WARTA, H.ID_HAKMILIKAGENSI, H.ID_HAKMILIK, J.KEPUTUSAN, B.NO_PERMOHONAN, B.FLAG_LAYER_KJP"
				+ " FROM TBLPFDFAIL A, TBLPERMOHONAN B, TBLPHPPEMOHON C, TBLRUJNEGERI D, TBLRUJSTATUS E, TBLPHPHAKMILIKPERMOHONAN F, TBLRUJBANDAR G, TBLHTPHAKMILIKAGENSI H, TBLHTPHAKMILIK I, TBLPHPPERMOHONANPELEPASAN J"
				+ " WHERE A.ID_SUBURUSAN = '32' AND A.ID_FAIL = B.ID_FAIL AND B.ID_PEMOHON = C.ID_PEMOHON AND C.ID_NEGERITETAP = D.ID_NEGERI(+) AND B.ID_STATUS = E.ID_STATUS(+) AND B.ID_PERMOHONAN = F.ID_PERMOHONAN"
	...
	public void setMaklumatPenawaran(String idPermohonan) throws Exception {
		...
		try {
			...
			while (rs.next()) {
				h = new Hashtable();
				h.put("idPermohonanPelepasan", rs.getString("ID_PHPPERMOHONANPELEPASAN") == null ? "" : rs.getString("ID_PHPPERMOHONANPELEPASAN"));
				h.put("tarikhTerima", rs.getDate("TARIKH_TERIMA") == null ? "" : sdf.format(rs.getDate("TARIKH_TERIMA")));
				h.put("tarikhSurat", rs.getDate("TARIKH_SURAT") == null ? "" : sdf.format(rs.getDate("TARIKH_SURAT")));
	...
	public void updatePermohonanPenawaran(String idPermohonanPelepasan,
			String idLuasKegunaan,String idLuas,String txtLuasMohon1, String txtLuasMohon2,
	...
	public boolean checkMaklumatPenawaranLengkap(String idPermohonan) throws Exception{
		...
		try {
			...
			sql = "SELECT ID_PHPPERMOHONANPELEPASAN, FLAG_GUNA FROM TBLPHPPERMOHONANPELEPASAN WHERE ID_PERMOHONAN = '" + idPermohonan + "'";
			ResultSet rs = stmt.executeQuery(sql);
	...
	public void updatePermohonanSemakan(String idPermohonan, String idKementerianPmhn, HttpSession session) throws Exception {
	...
	public void updatePermohonanKelulusan(String idPermohonan, String idKementerianPmhn, HttpSession session) throws Exception {
...
//				email.SUBJECT = "PERMOHONAN PENAWARAN HARTA TANAH PERSEKUTUAN #" + noPermohonan;
...
//			}
	...
	public void setMaklumatKeputusan(String idPermohonan) throws Exception {
		...
		try {
			...
			while (rs.next()) {
				h = new Hashtable();
				h.put("tarikhKeputusan", rs.getDate("TARIKH_HANTARKEPUTUSAN") == null ? "" : sdf.format(rs.getDate("TARIKH_HANTARKEPUTUSAN")));
	...
	public void setSenaraiAgensi(String idPermohonan) throws Exception {
		...
		try {
			...
			sql = "SELECT A.ID_PENAWARANKJP, B.NAMA_KEMENTERIAN, C.NAMA_AGENSI, A.FLAG_LULUS, A.LUAS FROM TBLPHPPENAWARANKJP A, TBLRUJKEMENTERIAN B, TBLRUJAGENSI C"
				+ " WHERE A.ID_KEMENTERIAN = B.ID_KEMENTERIAN AND A.ID_AGENSI = C.ID_AGENSI AND A.ID_PERMOHONAN = '" + idPermohonan + "' ORDER BY A.ID_PENAWARANKJP";
			ResultSet rs = stmt.executeQuery(sql);
	...
	public void setMaklumatImej(String idDokumen) throws Exception {
		...
		try {
			...
			sql =	"SELECT ID_DOKUMEN, NAMA_DOKUMEN, NAMA_FAIL, CATATAN FROM TBLPHPDOKUMEN WHERE ID_DOKUMEN = '" + idDokumen + "'";
			ResultSet rs = stmt.executeQuery(sql);
