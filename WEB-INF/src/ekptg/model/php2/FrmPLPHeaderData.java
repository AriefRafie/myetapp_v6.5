	...
	public void setMaklumatPermohonan(String idFail, HttpSession session)
		...
		try {
			...
			sql = "SELECT FAIL.ID_FAIL, FAIL.NO_FAIL, FAIL.NO_FAIL_NEGERI, FAIL.TAJUK_FAIL, MOHON.ID_PERMOHONAN, MOHON.TARIKH_TERIMA, MOHON.TARIKH_SURAT,"
					...
					+ " PEMOHON.ID_KEMENTERIAN, PEMOHON.ID_NEGERITETAP, FAIL.TARIKH_DAFTAR_FAIL, MOHONPLP.KEPUTUSAN_PTD AS ID_KEPUTUSANPTD"
					...
					+ " AND FAIL.ID_SEKSYEN = 4 AND FAIL.ID_URUSAN = 6 AND FAIL.ID_SUBURUSAN = 34"
			...
			if (rs.next()) {
				...
				h.put("idKeputusanPTD", rs.getString("ID_KEPUTUSANPTD") == null ? ""
						: rs.getString("ID_KEPUTUSANPTD").toUpperCase());
				...
				h.put("flagPampasan",
						rs.getString("FLAG_PAMPASAN") == null ? "" : rs
								.getString("FLAG_PAMPASAN").toUpperCase());
				h.put("pampasan",
						rs.getString("PAMPASAN") == null ? "" : Utils
								.RemoveSymbol(rs.getString("PAMPASAN")));

				h.put("idNegeriPemohon",
						rs.getString("ID_NEGERITETAP") == null ? "99999" : rs
								.getString("ID_NEGERITETAP").toUpperCase());
				h.put("idKementerianPemohon",
	...
	public void setMaklumatTanahGanti(String idTanahGanti) throws Exception {
		...
		try {
			...
			sql = "SELECT TG.ID_TANAHGANTI, TG.PEGANGAN_HAKMILIK, TG.ID_JENISHAKMILIK, RUJJENISHM.KOD_JENIS_HAKMILIK, TG.NO_HAKMILIK,"
					...
					+ " FROM TLPHPTANAHGANTIPLPSN TG, TBLRUJJENISHAKMILIK RUJJENISHM, TBLRUJLOT RUJLOT, TBLRUJLUAS RUJLUAS,"
					+ " TBLRUJMUKIM RUJMUKIM, TBLRUJDAERAH RUJDAERAH, TBLRUJNEGERI RUJNEGERI, TBLRUJKATEGORI RUJKATEGORI, TBLRUJSUBKATEGORI RUJSUBKATEGORI, TBLRUJAGENSI RUJAGENSI, TBLRUJKEMENTERIAN RUJKEMENTERIAN"
