...
public class FrmPLPPopupSenaraiPermohonanData {

	private Vector senaraiFailMesyuarat = null;

	SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");

	public void carianFail(String noFail, String namaPemohon) throws Exception {
		Db db = null;
		SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
		String sql = "";
		...
		try {
			...
			//		+ " AND A.ID_FAIL IS NOT NULL AND A.ID_SEKSYEN = 4 AND A.ID_URUSAN = '8' AND A.ID_SUBURUSAN = '56' AND B.ID_STATUS = '1610201' "; 
					//+ " AND B.ID_PERMOHONAN NOT IN (SELECT ID_PERMOHONAN FROM TBLPHPMESYUARATPERMOHONAN WHERE FLAG_JENIS_PERMOHONAN = 'B') ";

			sql = " SELECT A.ID_FAIL, A.NO_FAIL, B.ID_PERMOHONAN, C.NAMA AS NAMA_PEMOHON "
					+ " FROM TBLPFDFAIL A, TBLPERMOHONAN B, TBLPHPPEMOHON C "
					+ " WHERE A.ID_FAIL = B.ID_FAIL AND B.ID_PEMOHON = C.ID_PEMOHON AND A.NO_FAIL IS NOT NULL "
					+ " AND A.ID_FAIL IS NOT NULL AND A.ID_SEKSYEN = 4 AND A.ID_URUSAN = '6' AND A.ID_SUBURUSAN = '34' AND B.ID_STATUS = '1610201' ";
					//+ " AND B.ID_PERMOHONAN NOT IN (SELECT ID_PERMOHONAN FROM TBLPHPMESYUARATPERMOHONAN WHERE FLAG_JENIS_PERMOHONAN = 'B') ";
