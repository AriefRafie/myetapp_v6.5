package ekptg.model.integrasi.macgdi;
...
public class FrmLaporanHTPData {
	
	static Logger myLogger = Logger.getLogger(FrmLaporanHTPData.class);	
	protected Db db;
...
//			}
	...
	public Vector getSenaraiLaporanAPB() {
		...
		try {
			...
			while (rs.next()) {
				...
				h.put("namaLaporan", rs.getString("NAMA_LAPORAN") == null ? "" : rs.getString("NAMA_LAPORAN"));	
	...
	public Hashtable getMaklumatLaporan(String idLaporan) {
		...
		try {
			...
			sql = "SELECT * FROM TBLINTMACGDILAPORAN WHERE ID = '" + idLaporan + "'";
			ResultSet rs = stmt.executeQuery(sql);
			
			if (rs.next()) {
				laporan = new Hashtable();
				laporan.put("idLaporan",rs.getString("ID") == null ? "" : rs.getString("ID"));
				laporan.put("namaLaporan", rs.getString("NAMA_LAPORAN") == null ? "" : rs.getString("NAMA_LAPORAN").toUpperCase());
			}
		} catch (Exception ex) {
			ex.printStackTrace();
		} finally {
	...
	public Vector getSenaraiDokumen(String idLaporan) {
		...
		try {
			...
			while (rs.next()) {
				h = new Hashtable();
				h.put("idDokumen", rs.getString("ID") == null ? "" : rs.getString("ID"));
