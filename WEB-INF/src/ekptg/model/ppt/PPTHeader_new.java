...
public class PPTHeader_new {
	static Logger myLogger = Logger.getLogger(PPTHeader_new.class);
	...
	public Vector listPerjalananFail(String id_permohonan) throws Exception {
		...
		sql = "SELECT DISTINCT "+
			  ...
			  " AND PS.ID_SIMATI = SM.ID_SIMATI  ";
				...
				myLogger.info("SQL LIST SUBURUSAN edited ppk:"+sql.toUpperCase());
			*/
			/*
	...
	public String getIdSubUrusan(String id_permohonan,Db db) throws Exception {
		//Db db = null;
		String sql = "";
		ResultSet rs = null;
		Statement stmt = null;
	...
	public static void setDataHeader(String id_permohonan)throws Exception {
	    ...
	    try{
	    		...
	    		*/
	    		...
	    		sql += " OR px.id_permohonan in (select distinct hx.id_permohonan from Tblppthakmilik hx, Tblpptborange bx, Tblpptborangehakmilik beh "; 
	    		sql += " where hx.id_permohonan = p.id_permohonan and hx.id_hakmilik = beh.id_hakmilik and beh.id_borange = bx.id_borange )))as flag_open_paging14, ";
	    		...
	    		sql += " OR " +*/
	    				"px.id_permohonan in (select distinct hx.id_permohonan from Tblppthakmilik hx, Tblpptsiasatan sx "; 
	    		sql += " where hx.id_permohonan = p.id_permohonan and hx.id_hakmilik = sx.id_hakmilik) ";
	    		sql += " OR px.id_permohonan in (select distinct hx.id_permohonan from Tblppthakmilik hx, Tblpptbuktipenyampaian bpx "; 
	    		sql += " where hx.id_permohonan = p.id_permohonan and hx.id_hakmilik = bpx.id_hakmilik ";
	    		sql += " and flag_jenis_bukti = '1')))as flag_open_paging16, "; 
