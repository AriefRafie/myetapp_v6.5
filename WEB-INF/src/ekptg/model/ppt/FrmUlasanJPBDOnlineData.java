	...
	public Vector viewMaklumatPermohonan(String ID_PERMOHONAN) throws Exception {
		...
		try {
			if (!"".equalsIgnoreCase(ID_PERMOHONAN)) {
				...
				String JAWATAN_PEGAWAI_ASAL = "", TARIKH_PERMOHONAN = "", JABATAN = "", NAMA_KEMENTERIAN = "";
				...
				sql = "SELECT FAIL.NO_FAIL, PMHN.NO_PERMOHONAN, NG.NAMA_NEGERI, DE.NAMA_DAERAH, PMHN.TUJUAN, M.NO_RUJUKAN_JPBD, M.NO_WARTA, " +
					...
					"M.TANDATANGAN_BAGI_PIHAK, M.NAMA_PEGAWAI_ASAL, M.JAWATAN_PEGAWAI_ASAL, M.TARIKH_PERMOHONAN, M.JABATAN, PMHN.ID_PERMOHONAN, K.NAMA_KEMENTERIAN " +
					"FROM TBLINTJPBD M, TBLPPTPERMOHONAN PMHN, TBLPFDFAIL FAIL, TBLRUJNEGERI NG, TBLRUJDAERAH DE, TBLRUJKEMENTERIAN K " +
					"WHERE M.ID_PERMOHONAN = PMHN.ID_PERMOHONAN AND FAIL.ID_KEMENTERIAN = K.ID_KEMENTERIAN  AND PMHN.ID_FAIL = FAIL.ID_FAIL AND PMHN.ID_NEGERI = NG.ID_NEGERI(+) AND PMHN.ID_DAERAH = DE.ID_DAERAH(+) " +
				...
				if (rs.next()) {
					...
					NAMA_KEMENTERIAN = rs.getString(30) == null ? "" : rs.getString(30);
				}
