...
public class FrmPopupCapaianHakmilikEtanahData {
	...
	public void senaraiCarian(String idPermohonanSimati) throws Exception {
		Db db = null;
		String sql = "";
		...
		try {
			...
			sql = "SELECT * FROM INT_PPKHTA WHERE ID_PERMOHONANSIMATI = '" + idPermohonanSimati + "' ORDER BY TARIKH_TERIMA, FLAG_AKTIF DESC";
			...
			while (rs.next()) {
				h = new Hashtable();
				h.put("bil", bil);
				h.put("idPPKHTA", rs.getString("ID_PPKHTA") == null ? "" : rs.getString("ID_PPKHTA"));
	...
	public void setMaklumatHakmilik(String idPPKHTA) throws Exception {
		Db db = null;
		String sql = "";
		...
		try {
			...
			sql = "SELECT HTA.ID_PPKHTA, HTA.ID_PERMOHONANSIMATI, HTA.NO_RESIT, HTA.ID_HAKMILIK, HTA.ID_JENISHM, JENISHM.KOD_JENIS_HAKMILIK, HTA.NO_HAKMILIK, HTA.NO_PT, HTA.ID_LUAS, HTA.LUAS, LUAS.KETERANGAN AS JENIS_LUAS, HTA.ID_MUKIM, MUKIM.NAMA_MUKIM, HTA.ID_DAERAH, DAERAH.NAMA_DAERAH,"
					...
					+ " FROM INT_PPKHTA HTA, TBLRUJJENISHAKMILIK JENISHM, TBLRUJLUAS LUAS, TBLRUJNEGERI NEGERI, TBLRUJDAERAH DAERAH, TBLRUJMUKIM MUKIM, TBLRUJKATEGORI RUJKATEGORI, TBLRUJJENISPB JENISPB"
					
					+ " WHERE HTA.ID_JENISHM = JENISHM.ID_JENISHAKMILIK(+) AND HTA.ID_LUAS = LUAS.ID_LUAS(+) AND HTA.ID_NEGERI = NEGERI.ID_NEGERI(+) AND  HTA.ID_DAERAH = DAERAH.ID_DAERAH(+) AND HTA.ID_NEGERI = DAERAH.ID_NEGERI(+)"
					+ " AND HTA.ID_MUKIM = MUKIM.ID_MUKIM(+) AND HTA.ID_DAERAH = MUKIM.ID_DAERAH(+) AND HTA.ID_KATEGORI = RUJKATEGORI.ID_KATEGORI(+) AND UPPER(HTA.STATUS_PEMILIKAN) = UPPER(JENISPB.KOD_JENIS_PB(+))"
					+ " AND HTA.ID_PPKHTA = '" + idPPKHTA + "'";
	...
	public void daftarHakmilik(String idPPKHTA, String noResit, String idHakmilik, String idPermohonanSimati, HttpSession session) throws Exception {
		...
		try {
			...
			sql = "SELECT * FROM INT_PPKHTA WHERE NO_RESIT = '" + noResit + "' AND ID_HAKMILIK = '" + idHakmilik + "' AND ID_PERMOHONANSIMATI = '" + idPermohonanSimati + "'";
			ResultSet rs = stmt.executeQuery(sql);
			...
			if (rs.next()){
				...
				String jenisTNH = rs.getString("JENIS_TNH") == null ? "" : rs.getString("JENIS_TNH");
	...
	private boolean hakmilikRegistered(String noResit, String idHakmilik, String idPermohonanSimati) throws Exception {
		...
		try {
			...
			sql = "SELECT * FROM TBLPPKHTA WHERE NO_RESIT_CARIAN = '" + noResit + "' AND ID_HAKMILIK_ETANAH = '" + idHakmilik + "' AND ID_PERMOHONANSIMATI = '" + idPermohonanSimati + "'";
			ResultSet rs = stmt.executeQuery(sql);
