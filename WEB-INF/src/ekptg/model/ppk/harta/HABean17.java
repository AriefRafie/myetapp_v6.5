...
public class HABean17 extends EkptgCache implements IMaklumatHarta {
	
	private static Logger myLog = Logger.getLogger(ekptg.model.ppk.harta.HABean17.class);
	...
	public void getHarta(String mode
		...
		//17
		}else if ("view_harta_alih_dulu".equals(mode)) {
			context.put("EventStatus", 1);
		...
		//17
			...
			simpanPilihanHA(session,request,fnc,logic_C,hParam);
...
//			}
			...
			//online context.put("EventStatus", 3);
			context.put("EventStatus", eventstatus);
			context.put("idha", idha);
			context.put("tutup", "yes");
			context.put("showbuttontambah", "yes");
...
//
	...
	private void simpanPilihanHA(HttpSession session,HttpServletRequest request,Fungsi fnc
		, FrmPrmhnnSek8SenaraiSemakInternalData logic_C,Hashtable<String,String> hParam) throws Exception {
		
		String bolehsimpan = String.valueOf(hParam.get("bolehSimpan"));
		String per_mati = fnc.getParam(request,"id_Permohonansimati");
		String user_id = (String) session.getAttribute("_ekptg_user_id");
		logic_C.clear_pilihanHAMainDelete(per_mati, "");
		myLog.info("NAK SIMPAN PILIHAN OB");
			...
			for (int i = 0; i < ids.length; i++) {
				if (bolehsimpan.equals("yes")) {
					...
					logic_C.pilihanAdd_HA(per_mati, ids[i], user_id,
							id_Pilihanha, fnc.getParam(request,flag_daftar));
					String temp_id_ob = "main_ob" + ids[i];
					String[] ids_ob = request.getParameterValues(temp_id_ob);
					myLog.info("temp_id_ob :" + temp_id_ob);
					myLog.info("ids_ob :" + ids_ob);
					...
					if (ids_ob != null) {
						for (int y = 0; y < ids_ob.length; y++) {
							...
							logic_C.pilihanAddOBHA("", per_mati, ids_ob[y]
												...
												,ids_ob_sub4, user_id,id_Pilihanha + ""
		...
		// buka
		//myLog.info("check_pkt :" + check_pkt);
		//myLog.info("check_pkt_true :" + check_pkt_true);
		...
		if (!flag_perlu_batal_kt.equals("")) {
			...
			for (int i = 0; i < listHartaTakAlihdulu.size(); i++) {
				...
				myLog.info("GET ID HTA :" + h.get("idhta").toString());
				...
				if (flag_perlu_batal_kt.equals("batalkan")) {
					...
					for (int ix = 0; ix < listHartaTakAlihduluOB.size(); ix++) {
						...
						myLog.info("GET ID HTA OB :"+ hx.get("ID_HTA").toString());
						if (h.get("idhta").toString().equals(hx.get("ID_HTA").toString())) {
							myLog.info("GET ID OB OB :"+ hx.get("ID_OB").toString());
							logic_C.pilihanAddOBHTA("", per_mati, hx.get(
									"ID_OB").toString(), "", "", "", "",user_id,id_Pilihanhta + "", "", "");
						}
					}
				}
			}
			...
			for (int i = 0; i < listHartaAlihdulu.size(); i++) {
				...
				myLog.info("GET ID HA :" + h.get("id_Ha").toString());
				...
				if (flag_perlu_batal_kt.equals("batalkan")) {
					...
					for (int iy = 0; iy < listHartaAlihduluOB.size(); iy++) {
						...
						myLog.info("GET ID HA OB :"+ hy.get("ID_HA").toString());
						if (h.get("id_Ha").toString().equals(hy.get("ID_HA").toString())) {
							myLog.info("GET ID OB OB :"+ hy.get("ID_OB").toString());
...
//	}
	...
	private void setSocHAReadOnly(Hashtable<String,String> hParam,org.apache.velocity.VelocityContext context) throws Exception  {
		Hashtable<String,String> b = setSocParamValuesHA(String.valueOf(hParam.get("socIdNegeri"))
				,String.valueOf(hParam.get("socIdDaerah")),"0","0"
				,String.valueOf(hParam.get("socJenisHartaAlih")));
		
		setSocValues(b
			,"socNegeriHtaam","negerichangeU"
			,"socDaerahHtaam"," disabled"
			,"socMukimHtaamX"
	...
	public void setSocValues(Hashtable<String,String> b
		,String negeriNama,String negeriFunc
		,String daerahNama,String daerahStyle
		,String mukimNama
		,String bandarNama,String bandarFunc
...
//							
				String socNegeri = HTML.SelectNegeri(negeriNama, Long.valueOf(idNegeri),daerahStyle
...
//				
				String socDaerah = HTML.SelectDaerahByNegeri(idNegeri, daerahNama, Utils.parseLong(idDaerah)
						," $!readmodedaerah id=\""+daerahNama+"\" onClick=\"CheckDaerah();\" "+socStyle + daerahStyle);
//				String socMukim = HTML.SelectMukimByDaerah(dd, mukimNama, Utils.parseLong(idMukim)
...
//		}		
	...
	private void addHtaam_(HttpSession session
		,Hashtable hParam
		,FrmPrmhnnSek8InternalData logic_internal
		) throws Exception {
