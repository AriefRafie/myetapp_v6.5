...
public class FrmPrmhnnPemindahanSek17BKEData {

	private static SimpleDateFormat Format =  new SimpleDateFormat("dd/MM/yyyy");
	public static Vector setList(int usernegeri, int userdaerah) throws Exception {
		Db db = null;
		String sql = "";
	...
	public static void  setCarianFail(String noFail, String namaPemohon, String namaSimati, String icSimati, String JenisIc, int usernegeri)throws Exception {
	    ...
	    try {
	      ...
	      sql = "Select f.id_fail,f.no_fail,f.tarikh_daftar_fail,a.id_permohonan,a.tarikh_Mohon,a.tarikh_Masuk,s.keterangan,p.id_simati,p.nama_simati,pp.nama_pemohon,a.seksyen "+
	      	...
	      	"where a.id_status = s.id_status(+) and a.id_fail = f.id_fail(+) and a.id_permohonan = pp.id_permohonan(+) and " +
	    ...
	    //IC SIMATI
	      ...
	      sql = sql + "  and a.ID_DAERAHMHN = d.ID_DAERAH and a.FLAG_JENIS_PERMOHONAN = 1  and a.id_status in (8,14,56,61,153) and d.id_negeri = "+ usernegeri +" ORDER BY F.ID_FAIL DESC";
	      ResultSet rs = stmt.executeQuery(sql);
	      Hashtable h;
	      int bil = 1;
	      int count = 0;
	...
	public static Vector getNegeriDaerahMohon(int idPermohonan) throws Exception {
	    Db db = null;
	    String sql = "";
	    try {
	      db = new Db();
	...
	public static Vector getBkeData(int idPermohonan) throws Exception {
	    Db db = null;
	    String sql = "";
	    SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
	    try {
	...
	public static int chkBkeData(int idPermohonan) throws Exception {
	    Db db = null;
	    String sql = "";
	    try {
	      db = new Db();
	 ...
	 public static Vector getMaklumatPermohonan(int id) throws Exception{
			Db db = null;
			String sql = "";
			SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
			...
			try{
				...
				sql = "SELECT f.id_Fail, f.no_Fail, d.id_Daerah, p.id_Permohonan, p.tarikh_Mohon, s.no_Kp_Baru, s.no_Kp_Lama, s.jenis_Kp, s.no_Kp_Lain," +
						...
						"pm.id_Permohonan = p.id_Permohonan AND s.id_Simati = mosi.id_Simati  AND p.id_Permohonan = mosi.id_Permohonan  " +
						"AND p.id_Permohonan = sub.id_Permohonan AND st.id_Status = p.id_Status  AND d.id_daerah = u.id_daerah(+)  AND p.id_Permohonan = "+ id +"";
				ResultSet rs = stmt.executeQuery(sql);
			    Hashtable h;
	 ...
	 public static void insertNewBke(Hashtable data) throws Exception
	  {
	    ...
	    try
	    {
	      ...
	      sql = "Insert into TBLPPKBKE (ID_BKE, ID_PERMOHONAN, ID_NEGERI, ID_DAERAH, TARIKH_MOHON, ALASAN_1, ALASAN_2, ALASAN_3," +
	      		"ALASAN_4, ALASAN_5, ALASAN_6, ALASAN_7, ALASAN_LAIN, ID_MASUK, TARIKH_MASUK) values ("+idBke+","+idPermohonan+","+idnegeri+"," +
	      		""+iddaerah+","+tarikh_surat+",'"+alasan1+"','"+alasan2+"','"+alasan3+"','"+alasan4+"','"+alasan5+"'," +
	      		"'"+alasan6+"','"+alasan7+"','"+alasanlain+"',"+userId+",sysdate)";
	      stmt.executeUpdate(sql);
			...
			sql2 = "insert into tblrujsuburusanstatusfail (ID_SUBURUSANSTATUSFAIL,ID_PERMOHONAN,ID_SUBURUSANSTATUS,AKTIF,ID_MASUK,TARIKH_MASUK," +
				   "ID_FAIL) values ("+DB.getNextID("TBLRUJSUBURUSANSTATUSFAIL_SEQ")+","+idPermohonan+",303,1,"+userId+"," +
				   "sysdate,(Select id_fail from tblppkpermohonan where id_permohonan="+idPermohonan+"))";
	 ...
	 public static Vector getDaerahbyNegeri(int idPermohonan) throws Exception {
		    Db db = null;
		    String sql = "";
		    try {
		      db = new Db();
		      Statement stmt = db.getStatement();
		      sql = "select n.id_negeri,n.nama_negeri, d.id_daerah,d.kod_daerah, d.nama_daerah from tblppkbke b, tblrujnegeri n, tblrujdaerah d, tblrujpejabaturusan s where " +
		      		"b.id_negeri = n.id_negeri and b.id_daerah = d.id_daerah and b.id_permohonan = "+ idPermohonan +" group by n.id_negeri,n.nama_negeri, d.id_daerah,d.kod_daerah, d.nama_daerah";
		      ResultSet rs = stmt.executeQuery(sql);
		      Vector v = new Vector();
		      while (rs.next()) {
		        Hashtable h = new Hashtable();
	 ...
	 public static int cntKeputusan(int idPermohonan) throws Exception {
		    Db db = null;
		    String sql = "";
		    try {
		      db = new Db();
	 ...
	 public static Vector cntDataPersamaan(int idPermohonan, String keputusan) throws Exception {
		    Db db = null;
		    String sql = "";
		    try {
		      db = new Db();
	 ...
	 public static Vector cntKeputusanKptg(int idPermohonan) throws Exception {
		    Db db = null;
		    String sql = "";
		    try {
		      db = new Db();
	 ...
	 public static Vector getUserDetail(int userid) throws Exception {
		    Db db = null;
		    String sql = "";
		    try {
		      db = new Db();
	 ...
	 public static Vector getListDaerahbyNegeri(int idnegeri, int idpermohonan) throws Exception {
		    Db db = null;
//		    String sql = "select a.id_agensi, a.kod_agensi, a.nama_agensi, a.alamat1, a.alamat2, a.alamat3, a.poskod, a.jawatan, a.id_negeri from tblrujagensi a order by a.id_negeri";
		    ...
		    try {
		      ...
		      //sql = sql + " group by d.id_daerah,d.nama_daerah,d.kod_daerah,d.id_negeri order by d.id_daerah";
		      	sql = "select a4.id_Daerah, a4.kod_Daerah, a4.nama_Daerah, a3.id_negeri " +
		      		  ...
		      		  "and a6.id_permohonan = a7.id_permohonan and a7.id_permohonansimati = a8.id_permohonansimati and " +
		      		  ...
		      		  "AND A6.ID_PERMOHONAN = "+ idpermohonan +") group by a4.id_Daerah, a4.kod_Daerah, a4.nama_Daerah, a3.id_negeri";
	      ResultSet rs = stmt.executeQuery(sql);
		      Vector v = new Vector();
		      while (rs.next()) {
		        Hashtable h = new Hashtable();
...
//		    String sql = "select a.id_agensi, a.kod_agensi, a.nama_agensi, a.alamat1, a.alamat2, a.alamat3, a.poskod, a.jawatan, a.id_negeri from tblrujagensi a order by a.id_negeri";
	 ...
	 public static Vector getListnegeribyhta(int idpermohonan) throws Exception {
		    Db db = null;
//		    String sql = "select a.id_agensi, a.kod_agensi, a.nama_agensi, a.alamat1, a.alamat2, a.alamat3, a.poskod, a.jawatan, a.id_negeri from tblrujagensi a order by a.id_negeri";
		    String sql = "";
		    try {
		      ...
		      System.out.println("getListnegeribyhta  "+sql);
		      ResultSet rs = stmt.executeQuery(sql);
		      Vector v = new Vector();
		      while (rs.next()) {
		        Hashtable h = new Hashtable();
		...
		public static int getListnegeribyhtacount(int idpermohonan) throws Exception {
		    Db db = null;
		    try {
		      db = new Db();
		      Statement stmt = db.getStatement();
		      SQLRenderer r = new SQLRenderer();    
		      String  sql = "select count(a3.id_negeri) as CntId " +
	      		" from tblrujpejabaturusan a1,tblrujnegeri a3, tblrujdaerah a4,TBLRUJPEJABATJKPTG a5,tblppkpermohonan a6," +
	      		"tblppkpermohonansimati a7,tblppkhta a8 where a1.ID_PEJABATJKPTG = a5.ID_PEJABATJKPTG and " +
	      		"a1.id_negeriurus = a3.id_negeri and a1.id_daerahurus = a4.id_daerah and a6.ID_daerahMHN NOT IN (a1.id_daerahurus) " +
	      		"and a6.id_permohonan = a7.id_permohonan and a7.id_permohonansimati = a8.id_permohonansimati " +
		...
		public static Vector getDaerahbyNegerihta (int idPermohonan) throws Exception {
		    Db db = null;
		    String sql = "";
		    try {
		      db = new Db();
