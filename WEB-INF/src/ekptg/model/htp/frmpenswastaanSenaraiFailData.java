...
public class frmpenswastaanSenaraiFailData {

	static Logger mylog = Logger
			.getLogger(ekptg.model.htp.frmpenswastaanSenaraiFailData.class);

	public static void addSimpanPemaju(Hashtable data, HttpSession session) throws Exception {
	...
	public static void addSimpanPengarah(Hashtable data, HttpSession session) throws Exception {
		// Masukan data dalam table - Bukti Penyampaian
		Db db = null;
		String sql = "";
		String id = "";
		...
		try {
			long id_pengarah = DB.getNextID("TBLHTPPENGARAH_SEQ");
			//declare apa2 nama string..
			String id_pemaju = (String) data.get("idPemaju");
			String nama = (String) data.get("namaPgrh");
			String No_opb = (String) data.get("icPgrh");
			
			
			db = new Db();
			Statement stmt = db.getStatement();
			SQLRenderer r = new SQLRenderer();
			r.add("id_pengarah", id_pengarah);
			r.add("id_pemaju", id_pemaju);
			r.add("nama", nama);
			r.add("No_opb",data.get("icPgrh"));
			r.add("ID_RUJJENISOPB","1");
			r.add("ID_WARGANEGARA","14");
			r.add("ID_URUSAN","4");
	...
	public static Vector getSenaraiPengarah(String id_pemaju)throws Exception {
		Db db = null;
		String sql = "";
		...
		try {
			...
			while (rs.next()) {
				h = new Hashtable();
				h.put("bil", bil);
				h.put("id_pengarah", rs.getString("id_pengarah") == null ? "" : rs.getString("id_pengarah".toUpperCase()));
				h.put("namaPgrh",rs.getString("nama") == null ? "" : rs.getString("nama").toUpperCase());
				h.put("icPgrh", rs.getString("no_opb") == null ? "" : rs.getString("no_opb"));
	...
	public static Vector getMaklumatPengarah (String id_pengarah)throws Exception {
		Db db = null;
		String sql = "";
		...
		try {
			...
			sql = "SELECT id_pengarah, nama, no_opb FROM tblhtppengarah"+
			" where id_pengarah ='"+ id_pengarah + "'";
			System.out.println (sql);
			ResultSet rs = stmt.executeQuery(sql);
			...
			while (rs.next()) {
				...
				h.put("namaPgrh",rs.getString("nama") == null ? "" : rs.getString("nama").toUpperCase());
				h.put("icPgrh", rs.getString("no_opb") == null ? "" : rs.getString("no_opb"));
				maklumatPengarah.addElement(h);
				bil++;
			}
			return maklumatPengarah;
	...
	public static void updateSimpanPengarah(Hashtable data) throws Exception {
		// Masukan
		Db db = null;
		String sql = "";
		String id = "";
		...
		try {
			...
			String idPengarah = (String) data.get("id_pengarah");
			String idPemaju = (String) data.get("id_pemaju");
			String namaPgrh = (String) data.get("namaPgrh");
			String icPgrh = (String) data.get("icPgrh");
			...
			// set field
			//r.add("ID_PENGARAH",data.get(idPengarah) );
			r.add("NAMA", namaPgrh);
			r.add("NO_OPB", icPgrh);
