...
public class HtpBean implements IHtp {
	...
	//private FrmSenaraiFailTerimaPohonData senaraiFailData = null;
	...
	public Vector<Hashtable<String, String>> carianFail(String noFail,String tajukFail,String tarikhDaftar,String tarikhTerima
		...
		try {
			...
			Statement stmt = db.getStatement();
			// By Rosli, 05/05/2010
				sql = "SELECT A.TAJUK_FAIL,A.ID_FAIL,TO_CHAR(A.TARIKH_DAFTAR_FAIL,'dd/MM/yyyy') TARIKH_DAFTAR " +
				" ,B.ID_PERMOHONAN, B.ID_STATUS, A.NO_FAIL" +
				" ,B.TARIKH_TERIMA, C.KETERANGAN,RN.NAMA_NEGERI"+
				" FROM TBLPFDFAIL A, TBLPERMOHONAN B,TBLHTPPERMOHONAN PP, TBLRUJSTATUS C" +
				" ,TBLHTPPEMOHON D,TBLRUJNEGERI RN "+
				" WHERE B.ID_FAIL = A.ID_FAIL AND B.ID_STATUS = C.ID_STATUS " +
			...
			if (tarikhDaftar != null) {
				if (!tarikhDaftar.toString().trim().equals("")) {
					sql = sql + " AND TO_CHAR(A.TARIKH_DAFTAR_FAIL,'dd-MON-YY') = '" + sdfyy.format(sdf.parse(tarikhDaftar)).toUpperCase() +"'";
				}
			}
			//tarikhTerima
			if (tarikhTerima != null) {
			...
			if (!jenisHakmilik.toString().trim().equals("")) {
				sql = sql + " AND B.ID_PERMOHONAN IN ( " +
					"SELECT ID_PERMOHONAN FROM TBLHTPHAKMILIKURUSAN WHERE ID_JENISHAKMILIK ='" + jenisHakmilik +"')";
			}
			if (noHakmilik != null) {
			...
			if (!jenisLot.toString().trim().equals("")) {
				sql = sql + " AND B.ID_PERMOHONAN IN ( " +
					"SELECT ID_PERMOHONAN FROM TBLHTPHAKMILIKURUSAN WHERE ID_LOT ='" + jenisLot +"')";
			}
			if (noLot != null) {
...
//											     String TP = "to_date('" + TarikhPermohonan + "','dd/MM/yyyy')";
	...
	public void kemaskiniSimpanStatusPermohonanAktif(
		...
		try{
			...
			r.update("id_fail",String.valueOf(s.getIdFail()));
			r.update("aktif", "1");
			r.add("aktif",s.getAktif());
			r.add("id_kemaskini", sBaru.getIdKemaskini());
			r.add("tarikh_kemaskini", r.unquote("sysdate")); 
