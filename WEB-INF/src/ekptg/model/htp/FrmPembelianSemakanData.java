...
public class FrmPembelianSemakanData {
	private static Vector list = new Vector();
	private static SimpleDateFormat Format =  new SimpleDateFormat("dd/MM/yyyy");
	private static DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
	private static Logger log = Logger.getLogger(FrmPembelianSemakanData.class);
	...
	public static void setSemak(String idPermohonan)throws Exception {
	    ...
	    try {
	      ...
	      while (rs.next()) {
	    	  ...
	    	  //fir
	    	  h.put("TarafKeselamatanFail", rs.getString("id_tarafKeselamatan") == null ? "" : rs.getString("id_tarafKeselamatan"));
	    	  h.put("StatusTanah", rs.getString("id_jenisTanah") == null ? "" : rs.getString("id_jenisTanah"));
...
//	      return list;
	...
	public static Vector getSemak(){
		  log.info("FrmPembelianSemakanData : getSemak : " + list.size());
		  return list;
	  }
	
	//*** update data from database
	  public static long update(Hashtable data) throws Exception {
...
//	  public static int simpan(Hashtable data) throws Exception {
	  ...
	  public static long simpan(Hashtable data) throws Exception {
		  ...
		  try
		  {
			  log.info("hashtable simpan : " +  data);
			  ...
			  //fir
			 // int idTarafKeselamatan = Integer.parseInt((String)data.get("TarafKeselamatanFail"));
...
//			  String noFail = kodJabatan+"/101/"+kodUrusan+"/"+idKementerian+"/"+kodMampu+"-"+String.format("%06d",File.getSeqNo(idSeksyen, idUrusan, idKementerian, idNegeri));
			  ...
			  StatusChange_Action(idPermohonan, idSuburusan, idFail);
	  ...
	  public static void StatusChange_Action(long idPermohonan, int idSuburusan, long idFail) throws Exception{
		  ...
		  try{
			   ...
			   * 
			   * */
			  setIdSuburusanstatus = FrmUtilData.getIdSuburusanStatusByLangkah("1",String.valueOf(idSuburusan),"=");
	  ...
	  public static void StatusChange_ActionOnline(long idPermohonan, int idSuburusan, long idFail) throws Exception{		  
		  int PPP = Integer.parseInt("31"); //PINJAMAN PERUMAHAN PERBENDAHARAAN
		  int PKRJPN = Integer.parseInt("57"); //PINJAMAN KOS RENDAH JABATAN PERUMAHAN NEGARA
		  int TPM = Integer.parseInt("60"); //PINJAMAN TMP & JPK
		  int PHG = Integer.parseInt("63"); //PERLETAKHAKAN HAK GADAIAN
		  ...
		  try{			  
			  ...
			  Long setIdSuburusanstatus = 0L;
			    ...
			    * 1  untuk status - Penerimaan Permohonan
			    **/
			  setIdSuburusanstatus = FrmUtilData.getIdSuburusanStatusByLangkah("-4",String.valueOf(idSuburusan),"=");			  
			  db = new Db();
			  conn = db.getConnection();
			  conn.setAutoCommit(false);	  
			  Statement stmt = db.getStatement();
			  ...
			  r.add("id_fail", idFail);			  
			  sql = r.getSQLInsert("Tblrujsuburusanstatusfail");
		      log.info("FrmGadaianSemakanData::StatusChange_ActionOnline::TBLRUJSUBURUSANSTATUSFAIL = "+sql);
		      stmt.executeUpdate(sql);		      
		      conn.commit();
