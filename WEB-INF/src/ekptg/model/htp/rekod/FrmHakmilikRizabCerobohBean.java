...
public class FrmHakmilikRizabCerobohBean implements IHakmilikRizab {
	
	private HakmilikInterface iHakmilik = null;
	private static Logger myLog = Logger.getLogger(ekptg.model.htp.rekod.FrmHakmilikRizabCerobohBean.class);
...
//			}   
			...
			myLog.info("getCarianSenaraiHakmilikRizabById:CARIAN::sql="+sql);
	    ...
	    try {
	    	...
	    	sql = "SELECT DISTINCT A.ID_HAKMILIK,C.NO_FAIL, "+
	            ...
	            ",F.KETERANGAN AS JENIS_LOT, A.NO_LOT, D.NAMA_URUSAN "+
	             ...
	             " AND FC.ID_HAKMILIK = A.ID_HAKMILIK " +
...
//			//no warta
			...
			sql = sql +" ORDER BY C.NO_FAIL DESC ";
			myLog.info("getCarianSenaraiTanahDiceroboh:CARIAN::sql="+sql);
			ResultSet rs = stmt.executeQuery(sql);	    
			Hashtable h;
	      	int bil = 1;
	      	int count = 0;
	      	...
	      	while (rs.next()) {
	      		...
	      		h.put("idFailCeroboh",rs.getString("ID_FAILCEROBOH"));
	    ...
	    try {
	    	...
	    	sql = "SELECT DISTINCT A.ID_HAKMILIK,C.NO_FAIL, "+
	            ...
	            "WHEN A.STATUS_SAH = 'H' "+
	             ...
	             "A.STATUS_RIZAB, A.TARIKH_TERIMA,A.KEGUNAAN_TANAH "+
	             ", "+
	             "	(SELECT F.ID_FAIL FROM" +
	             "	TBLPFDFAIL F,TBLPERMOHONAN P,TBLPHPHAKMILIKPERMOHONAN HPHP,TBLHTPHAKMILIKAGENSI TPHA " +
	             "	WHERE P.ID_FAIL=F.ID_FAIL " +
	             "	AND P.ID_PERMOHONAN=HPHP.ID_PERMOHONAN " +
	             "	AND HPHP.ID_HAKMILIKAGENSI = TPHA.ID_HAKMILIKAGENSI "+
	             "	AND F.ID_URUSAN = 8 AND F.ID_SUBURUSAN = 56 " +
	             "  AND TPHA.ID_HAKMILIK = A.ID_HAKMILIK " +
	             "	) ID_FAILCEROBOH "+
	             "FROM TBLHTPHAKMILIK A, "+
	             "TBLPERMOHONAN B, "+
	             "TBLPFDFAIL C, "+
