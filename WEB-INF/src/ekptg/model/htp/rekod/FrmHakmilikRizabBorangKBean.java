...
public class FrmHakmilikRizabBorangKBean implements IHakmilikRizab {
	
	private HakmilikInterface iHakmilik = null;
	private static Logger myLog = Logger.getLogger(ekptg.model.htp.rekod.FrmHakmilikRizabBorangKBean.class);
...
//			}   
	...
	public Vector getCarianSenaraiHakmilikRizab(String idJenisTanah
			,String idNegeri, String idDaerah, String idMukim
			,String noFail, String idJenisHakmilik
			,String noHakmilik, String noWarta
			,String idLot, String noLot, String idAgensi, String idKementerian
			,String idStatus)throws Exception {
	    Db db = null;
	    listCarianHakmilikDanRizab = new Vector();
	    try {
	    	...
	    	sql = "SELECT " +
	            ...
	            " ,PTH.ID_LOT "+
	            " ,CASE " +
	            "	WHEN length(LTRIM(RTRIM(PTH.NO_PT))) IS NULL THEN PTH.NO_LOT " +
	            " 	ELSE (SELECT UPPER(KETERANGAN) FROM TBLRUJLOT WHERE ID_LOT=PTH.ID_LOT) ||' '|| PTH.NO_PT " +
	            "	" +
	            " END AS NO_LOTPT "+
	            ",PP.TUJUAN KEGUNAAN_TANAH"+
	            " ,CASE  " +
	            "     WHEN (SELECT COUNT(*) FROM TBLHTPHAKMILIKURUSAN  " +
	            "             WHERE ID_NEGERI=PTH.ID_NEGERI AND ID_DAERAH=PTH.ID_DAERAH AND ID_MUKIM=PTH.ID_MUKIM  " +
	            "             AND NO_HAKMILIK=PTH.NO_HAKMILIK " +
	            "          )>0 " +
	            "         THEN 'PERMOHONAN'	 " +
	            "     WHEN (SELECT COUNT(*) FROM TBLPHPHAKMILIK  " +
	            "             WHERE ID_NEGERI=PTH.ID_NEGERI AND ID_DAERAH=PTH.ID_DAERAH AND ID_MUKIM=PTH.ID_MUKIM  " +
	            "             AND NO_HAKMILIK=PTH.NO_HAKMILIK " +
	            "          )>0  " +
	            "         THEN 'PENYEWAAN'  " +	            
	            ...
	            " TBLRUJKEMENTERIAN RK," +
	            " TBLRUJKEMENTERIANMAPPING RKME " +
	            " ,TBLPPTBORANGK PTK, TBLPPTHAKMILIKBORANGK PTHK "+
	            " WHERE PP.ID_PERMOHONAN = PTH.ID_PERMOHONAN "+
	            " AND PP.ID_FAIL = F.ID_FAIL "+
	            " AND PTH.ID_JENISHAKMILIK = RJH.ID_JENISHAKMILIK "+
	             //"AND A.ID_LOT = F.ID_LOT "+
	            " AND RKME.ID_KEMENTERIANLAMA = F.ID_KEMENTERIAN "+
	            " AND RKME.ID_KEMENTERIANBARU = RK.ID_KEMENTERIAN AND RKME.STATUS='A' " +
	            " AND PTH.ID_HAKMILIK = PTHK.ID_HAKMILIK " +
	            " AND PTH.flag_endosan IS NOT NULL " +        
//	            "AND A.ID_HAKMILIK IN " +
//	             "	(SELECT TPHA.ID_HAKMILIK FROM" +
...
//	             "	) "+
			...
			if (idJenisTanah != null) {
				if (!idJenisTanah.trim().equals("")) {
					if (Integer.parseInt(idJenisTanah) == 1){
						sql = sql + " AND PTH.NO_HAKMILIK  IN " +
			            "     (SELECT NO_HAKMILIK FROM TBLHTPHAKMILIKURUSAN  " +
			            "    	WHERE ID_NEGERI=PTH.ID_NEGERI AND ID_DAERAH=PTH.ID_DAERAH AND ID_MUKIM=PTH.ID_MUKIM  " +
			            "    	AND NO_HAKMILIK=PTH.NO_HAKMILIK" +
			            "	   ) " +
						"";
						...
						sql = sql + " AND PTH.NO_HAKMILIK  IN" +
			            "	(SELECT NO_HAKMILIK FROM TBLPHPHAKMILIK  " +
			            "   	WHERE ID_NEGERI=PTH.ID_NEGERI AND ID_DAERAH=PTH.ID_DAERAH AND ID_MUKIM=PTH.ID_MUKIM  " +
			            " 		AND NO_HAKMILIK=PTH.NO_HAKMILIK " +
			            "	)  " +
								"";
					}else{
						sql = sql + " AND PTH.NO_HAKMILIK  NOT IN " +
			            "     (SELECT NO_HAKMILIK FROM TBLHTPHAKMILIKURUSAN  " +
			            "    	WHERE ID_NEGERI=PTH.ID_NEGERI AND ID_DAERAH=PTH.ID_DAERAH AND ID_MUKIM=PTH.ID_MUKIM  " +
			            "    	AND NO_HAKMILIK=PTH.NO_HAKMILIK" +
			            "	   ) " +
			...
			if (idNegeri != null) {
				if (!idNegeri.trim().equals("")) {
					if (!idNegeri.trim().equals("99999")) {
						sql = sql + " AND PTH.ID_NEGERI = '" + idNegeri + "'";
					}
				}
			}
			//id daerah
			if (idDaerah != null) {
				if (!idDaerah.trim().equals("")) {
					if (!idDaerah.trim().equals("99999")) {
						sql = sql + " AND PTH.ID_DAERAH = '" + idDaerah + "'";
					}
				}
			}
			//id mukim
			if (idMukim != null) {
				if (!idMukim.trim().equals("")) {
					if (!idMukim.trim().equals("99999")) {
						sql = sql + " AND PTH.ID_MUKIM = '" + idMukim + "'";
					}
				}
			}	      
			//no fail
			...
			if (idJenisHakmilik != null) {
				if (!idJenisHakmilik.trim().equals("")) {
					if (!idJenisHakmilik.trim().equals("99999") && !idJenisHakmilik.trim().equals("00")) {
						sql = sql + " AND PTH.ID_JENISHAKMILIK = '" + idJenisHakmilik + "'";
					}
				}
			} 
			//no hakmilik
...
//			} 
			...
			if (noLot != null) {
				if (!noLot.trim().equals("")) {
					sql = sql + " AND UPPER(PTH.NO_LOT) LIKE '%' ||'" + noLot.toUpperCase() + "'|| '%'";
				}
			} 
			//id Kementerian
			if (idKementerian != null) {
				if (!idKementerian.trim().equals("")) {
					if (!idKementerian.trim().equals("99999")) {
						//20/09/2010
						sql = sql + " AND F.ID_KEMENTERIAN = '" + idKementerian + "'";
					}
				}
			}
			//no agensi
...
//			}
			...
			sql = sql +" ORDER BY F.NO_FAIL" +
					//",PTH.ID_NEGERI,PTH.ID_DAERAH,PTH.ID_MUKIM " +
					" DESC ";
			myLog.debug("getCarianSenaraiTanahBorangK:sql="+sql);
			ResultSet rs = stmt.executeQuery(sql);	    
			Hashtable h;
	      	int bil = 1;
	      	int count = 0;
...
//	      		//h.put("noWarta", rs.getString("NO_WARTA")== null?"":rs.getString("NO_WARTA"));
//	      		//h.put("kodLot", rs.getString("JENIS_LOT")== null?"":rs.getString("JENIS_LOT"));
	      		h.put("noLot", rs.getString("NO_LOTPT")== null?"":rs.getString("NO_LOTPT"));
//	      		//h.put("namaUrusan", rs.getString("NAMA_URUSAN")== null?"":rs.getString("NAMA_URUSAN"));
//	      		//h.put("statusSah", rs.getString("STATUS_HAKMILIK")== null?"":rs.getString("STATUS_HAKMILIK"));
	      		h.put("kegunaanTanah", rs.getString("KEGUNAAN_TANAH")== null?"":rs.getString("KEGUNAAN_TANAH").toUpperCase());
	      		h.put("status", rs.getString("STATUS_SEMASA")== null? "":rs.getString("STATUS_SEMASA"));
//	      		//h.put("tarikhTerima",rs.getDate("TARIKH_TERIMA")== null? "":sdf.format(rs.getDate("TARIKH_TERIMA")));
//	    	  
//	      		//if(rs.getString("NO_HAKMILIK")!= null){
	      			h.put("jenisTanah","M");
	...
	public Hashtable getCarianHakmilikRizab(String idHakmilik)throws Exception {
	    ...
	    try {
	    	...
	    	sql = "SELECT " +
	            ...
	            " END AS NO_LOTPT "+
	            " ,PP.TUJUAN KEGUNAAN_TANAH" +
	            " ,PTH.ID_JENISHAKMILIK " +
	            " ,PTH.ID_NEGERI,PTH.ID_DAERAH,PTH.ID_MUKIM " +
	            " ,PTK.ID_BORANGK,PTK.TARIKH_BORANGK" +
	            " ,PTH.ID_UNITLUASAMBIL,PTH.LUAS_AMBIL " +
	            " FROM TBLPPTHAKMILIK PTH, "+
	            " TBLPPTPERMOHONAN PP, "+
	            " TBLPFDFAIL F, "+
	            " TBLRUJJENISHAKMILIK RJH, "+
	            ...
	            " AND PTH.ID_HAKMILIK = PTHK.ID_HAKMILIK " +
	            " AND PTH.flag_endosan IS NOT NULL " +
	            " AND RN.ID_NEGERI = PTH.ID_NEGERI AND RD.ID_DAERAH = PTH.ID_DAERAH AND RM.ID_MUKIM = PTH.ID_MUKIM  " +        
//	            "AND A.ID_HAKMILIK IN " +
//	             "	(SELECT TPHA.ID_HAKMILIK FROM" +
//	             "	TBLPFDFAIL F,TBLPERMOHONAN P,TBLPHPHAKMILIKPERMOHONAN HPHP,TBLHTPHAKMILIKAGENSI TPHA " +
//	             "	WHERE P.ID_FAIL=F.ID_FAIL " +
...
//	             "	) "+
			...
			if (idHakmilik != null) {
				if (!idHakmilik.trim().equals("")) {
					sql = sql + " AND PTH.ID_HAKMILIK = '" + idHakmilik + "'";
				}
			}
			sql = sql +" ORDER BY F.NO_FAIL" +
					//",PTH.ID_NEGERI,PTH.ID_DAERAH,PTH.ID_MUKIM " +
					" DESC ";
			myLog.debug("getCarianTanahBorangK:sql="+sql);
			ResultSet rs = stmt.executeQuery(sql);	    
	      	int bil = 1;
	      	int count = 0;
	      	while (rs.next()) {
	      		...
	      		h.put("noLotPT", rs.getString("NO_LOTPT")== null?"":rs.getString("NO_LOTPT"));
	      		h.put("idNegeri", rs.getString("ID_NEGERI"));
	      		h.put("idDaerah", rs.getString("ID_DAERAH"));
	      		h.put("idMukim", rs.getString("ID_MUKIM"));
	      		h.put("idBorangK",Utils.isNull(rs.getString("ID_BORANGK")));      		
	      		h.put("tarikhBorangK",Utils.isNull(rs.getString("TARIKH_BORANGK")));    
	      		h.put("idLuas",Utils.isNull(rs.getString("ID_UNITLUASAMBIL")));    
	      		h.put("Luas",rs.getString("LUAS_AMBIL")==null?"0":rs.getString("LUAS_AMBIL"));          		
	      		h.put("kegunaanTanah", rs.getString("KEGUNAAN_TANAH")== null?"":rs.getString("KEGUNAAN_TANAH").toUpperCase());
//	      		//h.put("tarikhTerima",rs.getDate("TARIKH_TERIMA")== null? "":sdf.format(rs.getDate("TARIKH_TERIMA")));
//	    	  
//	      		//if(rs.getString("NO_HAKMILIK")!= null){
