...
public class FrmHakmilikRizabUrusanBean implements IHakmilikRizab {
	
	private HakmilikInterface iHakmilik = null;
	private static Logger myLog = Logger.getLogger(ekptg.model.htp.rekod.FrmHakmilikRizabUrusanBean.class);
			...
			if (idJenisTanah != null) {
				if (!idJenisTanah.trim().equals("")) {
					if (Integer.parseInt(idJenisTanah) == 1){
						sql = sql + " AND A.ID_HAKMILIK IN  "+
			             " ( SELECT HPH.ID_HAKMILIK "+
			             " FROM TBLPHPHAKMILIKPERMOHONAN HPH, TBLPFDFAIL F, TBLPERMOHONAN P, TBLPHPPERJANJIAN HPP "+
			             " WHERE "+
			             " HPH.ID_PERMOHONAN = P.ID_PERMOHONAN "+
						...
						sql = sql + " AND A.PEGANGAN_HAKMILIK IN  "+
			             " ( SELECT TPHU.PEGANGAN_HAKMILIK "+
					     " FROM TBLHTPHAKMILIKURUSAN TPHU, TBLPFDFAIL F, TBLPERMOHONAN P, TBLHTPPAJAKAN TPP " +
			             " WHERE  "+
			             " TPHU.ID_PERMOHONAN = P.ID_PERMOHONAN "+
			             " AND P.ID_FAIL = F.ID_FAIL "+
			             " AND F.ID_URUSAN = '3' "+
					...
					}else if (Integer.parseInt(idJenisTanah) == 3){
						sql = sql + " AND A.STATUS_PELEPASAN = 'Y' "+
						"";
					}
