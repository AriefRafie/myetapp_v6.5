...
public class FrmCukaiBayaranBean implements ICukai {
	
	static Logger myLog = Logger.getLogger(ekptg.model.htp.cukai.FrmCukaiBayaranBean.class);
...
// 	    	    " AND LTRIM(TBLTEMP.NO_HAKMILIK,0) = LTRIM(THHCT.NO_HAKMILIK,0) "+ 
	...
	@Override
 	public Vector senaraiHakmilik(String idNegeri
 	    ...
 	    if (noLot.equals("")||noLot.equals("0")){
 	    	noLot = null;
 	    } 	   
	...
	public HakMilik getCukaiHakmilik(String idCukaiTemp){
		...
		SQLRenderer r = null;
 	    try {
			...
			r.add("TPC.CUKAI_PERLU_BAYAR CUKAI_TERKINI");
			r.add("TPC.CUKAI_TALIAIR");
			r.add("TPC.CUKAI_PARIT");
			r.add("TPC.DENDA");
			r.add("TPC.BAYARAN_LAIN");
			...
			r.add("TPC.LEBIHAN"); //TERLEBIH BAYAR
			r.add("TPC.CUKAI_KENA_BAYAR");
			...
			while(rs.next()){
				...
				cukai.setPengecualian(rs.getDouble("PENGECUALIAN"));
				cukai.setJumlah(rs.getDouble("CUKAI_KENA_BAYAR"));
				cukai.setIdHakmilikCukai(rs.getLong("ID_HAKMILIKCUKAI"));
...
//	}
	...
	public void kemaskiniHakmilikCukaiTemp(CukaiTemp data) throws SQLException, DbException{		
		...
		double bayaranLain = (double)data.getBayaranLain();
	    ...
	    try{
	    	...
	    	r.add("BAYARAN_LAIN", Utils.RemoveComma(String.valueOf(bayaranLain)));
...
//	    	r.add("LEBIHAN", Utils.RemoveComma(String.valueOf(lebihan)));
	    	...
	    	ResultSet rs1 = stmt.executeQuery(sql1);	      
	 	    String no_hakmilik = "",no_lot = "",id_permohonan = "";
	 	    while (rs1.next()) {
	    	...
	    	String sql2 = " SELECT * "+
	    			...
	    			" WHERE STATUS = 'S' "+
	    			" AND ID_HAKMILIK = (SELECT A1.ID_HAKMILIK "+ 
	    			" FROM TBLHTPHAKMILIK A1 "+ 
	    			" WHERE UPPER (A1.NO_HAKMILIK) LIKE '%' || '"+no_hakmilik+"' || '%' "+
	    			" AND A1.NO_LOT = '"+no_lot+"' "+ 
	    			" AND A1.ID_PERMOHONAN = '"+id_permohonan+"') ";
	    	
	    	ResultSet rs2 = stmt.executeQuery(sql2);	      
	 	    String id_hakmilikcukai = "",id_hakmilik = "",cukai_terkini = "";
	 	    while (rs2.next()) {
	    	...
	    	if(jumlah != (Utils.parseDouble(cukai_terkini))){
		    	...
		    	r1.add("STATUS","N");
		    	r1.add("ID_KEMASKINI",r1.unquote(idkemaskini));
			    ...
			    r1.add("ID_HAKMILIKCUKAI",idHakmilikCukai); 
			    r1.add("ID_HAKMILIK",id_hakmilik); 
				r1.add("KOD_BAYARAN_CUKAI","P");
				r1.add("CUKAI",cukai_terkini);
				r1.add("CUKAI_TERKINI",Utils.RemoveComma(String.valueOf(jumlah)));
				r1.add("STATUS","S");
				r1.add("ID_MASUK",idkemaskini);
				...
				r1 = new SQLRenderer();
				r1.update("ID_HAKMILIK",r1.unquote(id_hakmilik)); 
		    	r1.add("CUKAI_TERKINI",Utils.RemoveComma(String.valueOf(jumlah)));
		    	r1.add("ID_KEMASKINI",idkemaskini);
		    	r1.add("TARIKH_KEMASKINI",r1.unquote("SYSDATE"));
