...
public class FrmPerletakhakanSemakanData {
	...
	public static void setSemakBaru(int idFail)throws Exception {
	    Db db = null;
	    list.clear();
	    String sql = "";
	    try {
...
//	    	  h.put("idAgensi", rs.getString("id_Agensi"));
	    	  ...
	    	  h.put("noFail", rs.getString("no_Fail"));
	    	  //rosli
//	    	  h.put("tAgihan", Format.format(rs.getDate("tarikh_Daftar_Fail")));
	    	  list.addElement(h);
	      }
	    } finally {
	      if (db != null) db.close();
	  ...
	  public static Vector getSemakBaru(){
		  System.out.println(list.size());
		  return list;
	  }
	...
	public static int update(Hashtable data) throws Exception {
		...
		try {
			...
			System.out.println("FrmPerletakhakanSemakanData::Update::TBLPERMOHONAN = "+ sql);
			...
			rHP.add("tarikh_Terima", r.unquote(TP));
			rHP.add("tarikh_Agihan", r.unquote(TBF));
			sql = rHP.getSQLUpdate("Tblhtppermohonan");
			System.out.println("FrmPerletakhakanSemakanData::Update::tarikh_Agihan = "+ TSKJP);
			System.out.println("FrmPerletakhakanSemakanData::Update::TBLHTPPERMOHONAN = "+ sql);
	...
	public static int simpan(Hashtable data) throws Exception {
		...
		try {
			...
			int idUrusan = Integer.parseInt("3");
			int idSuburusan = (Integer) data.get("idSuburusan");
			int idNegeri = (Integer) data.get("idNegeri");
			int idKementerian = (Integer) data.get("idKementerian");
			...
			String noFail = kodJabatan
					...
					+ kodMampu
					+ "-"
					+ File.getSeqNo(idSeksyen, idUrusan, idKementerian,
							idNegeri);
	...
	// public static void StatusChange_Action(long idPermohonan, int
	// idSuburusan) {
	// int PPP = Integer.parseInt("31"); // PINJAMAN PERUMAHAN PERBENDAHARAAN
	// int PKRJPN = Integer.parseInt("57"); // PINJAMAN KOS RENDAH JABATAN
	// // PERUMAHAN NEGARA
	...
	// String sekarang = "to_date('" + formatter.format(now)
	...
	// System.out
	// .println("FrmGadaianSemakanData::StatusChange_Action::TBLRUJSUBURUSANSTATUSFAIL = "
	// + sql);
	// stmt.executeUpdate(sql);
	// } catch (Exception ex) {
	// System.out
	// .println("FrmGadaianSemakanData::StatusChange_Action::ex = "
	// + ex);
	// } finally {
	// if (db != null)
	// db.close();
