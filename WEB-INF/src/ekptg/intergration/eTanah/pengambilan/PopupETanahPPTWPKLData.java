...
public class PopupETanahPPTWPKLData {

	protected Db db;
	SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
	SimpleDateFormat dtf = new SimpleDateFormat("dd/MM/yyyy hh:mm:ss a");
	private static Logger myLog = Logger.getLogger(PopupETanahPPTWPKLData.class);
		...
		try {
			...
			if ("D".equals(flagUrusan)) {
				sql = "SELECT ID_PERMOHONAN FROM INT_PPTPERMOHONAN"
						+ " WHERE ID_WARTA = '" + idPPTWarta + "' AND FLAG_URUSAN = '" + flagUrusan + "'";
			}
	...
	public Hashtable getMaklumatPermohonan(String idPermohonanIntegrasi, Db db) {
		String sql = "";
		...
		try {
			...
			sql = "SELECT * FROM INT_PPTPERMOHONAN WHERE ID_PERMOHONAN = '" + idPermohonanIntegrasi + "'";
			ResultSet rs = stmt.executeQuery(sql);
			...
			if (rs.next()) {
				...
				maklumatPermohonan.put("flagHantar", rs.getString("FLAG_HANTAR") == null ? "" : rs.getString("FLAG_HANTAR"));
	...
	public Vector getSenaraiHakmilik(String idPermohonanIntegrasi, Db db) {
		String sql = "";
		...
		try {
			...
			sql = "SELECT * FROM INT_PPTHAKMILIKPERMOHONAN WHERE ID_PERMOHONAN = '" + idPermohonanIntegrasi + "'";
			sql = sql + " ORDER BY ID_HAKMILIK ASC";
			ResultSet rs = stmt.executeQuery(sql);
	...
	public Vector getSenaraiDokumen(String idPermohonanIntegrasi, Db db) {
		String sql = "";
		...
		try {
			...
			sql = "SELECT * FROM INT_PPTDOKUMENPERMOHONAN WHERE ID_PERMOHONAN = '" + idPermohonanIntegrasi + "'";
			sql = sql + " ORDER BY ID_DOKUMENPERMOHONAN ASC";
			ResultSet rs = stmt.executeQuery(sql);

			while (rs.next()) {
				h = new Hashtable();
				h.put("idDokumenPermohonan", rs.getString("ID_DOKUMENPERMOHONAN") == null ? "" : rs.getString("ID_DOKUMENPERMOHONAN"));
	...
	public Vector getSenaraiDokumenEndorsan(String idPermohonanIntegrasi, Db db) {
		...
		try {
			...
			sql = "SELECT * FROM INT_PPTDOKUMENENDORSAN WHERE ID_PERMOHONAN = '" + idPermohonanIntegrasi + "'";
